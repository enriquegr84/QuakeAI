...
public:
	Point(int pointId, std::vector<float>& values)
	{
		mPointId = pointId;
		mDimension = values.size();

		for (int i = 0; i < (int)mDimension; i++)
	...
	size_t GetDimension() { return mDimension; }
...
public:
	Clustering(int clusterId, Point point)
	{
		...
		int dimension = (int)point.GetDimension();
	...
	size_t GetSize() { return mPoints.size(); }
...
public:
	...
	const std::vector<Clustering>& GetClusters() { return mClusters; }
...
private:
	...
	size_t mDimension;
	int mIterations; 
	std::vector<Clustering> mClusters;
