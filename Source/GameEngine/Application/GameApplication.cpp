...
bool GameApplication::OnInitialize()
{
	...
	mSystem = std::make_shared<WindowsSystem>(mWidth, mHeight);
	mSystem->SetEventListener(this);
	HWND handle = reinterpret_cast<HWND>(mSystem->GetID());

#ifdef USE_DX11

	mRenderer = std::make_shared<Dx11Renderer>(
		handle, mWidth, mHeight, D3D_FEATURE_LEVEL_11_0);
	mProgramFactory = std::make_shared<HLSLProgramFactory>();

	if (mRenderer == 0)
	{
		LogError("Failed to initialize the rendering system");
		return false;
	}

#elif _OPENGL_

	mRenderer = std::make_shared<WGLRenderer>(handle);
	...
	if (mSystem == 0)
	{
		LogError("Failed to initialize the game system");
		return false;
	}
	...
	{
		LogError("There is already another application running");
		return false;
	}
	...
	std::wstring gametitle(GetGameTitle());
	...
	mSoundSystem = std::make_shared<OpenALSoundSystem>();
	if (!mSoundSystem->Init())
	{
	...
	*/
	...
	mResCache = std::make_shared<ResCache>(200, mountPointFile);
	...
	extern std::shared_ptr<BaseResourceLoader> CreateWAVResourceLoader();
	extern std::shared_ptr<BaseResourceLoader> CreateOGGResourceLoader();
	extern std::shared_ptr<BaseResourceLoader> CreateMeshResourceLoader();
	extern std::shared_ptr<BaseResourceLoader> CreateXmlResourceLoader();
	extern std::shared_ptr<BaseResourceLoader> CreateImageResourceLoader();
	extern std::shared_ptr<BaseResourceLoader> CreatePhysicResourceLoader();
	extern std::shared_ptr<BaseResourceLoader> CreateShaderResourceLoader();
	...
	mResCache->RegisterLoader(CreateShaderResourceLoader());

    Settings::CreateLayer(SL_GLOBAL);
    Settings::CreateLayer(SL_DEFAULTS);
    Settings::Get()->Init(SL_DEFAULTS, L"Config/GameSettings.xml");

	// The event manager should be created next so that subsystems can hook in as desired.
	mEventManager = std::make_shared<EventManager>("GameEngine EventMgr", true);
	if (!mEventManager)
	{
...
//----------------------------------------------------------------------------
bool GameApplication::OnEvent(const Event& evt)
{
	bool absorbed = false;

	std::list<std::shared_ptr<BaseGameView>>::reverse_iterator i = mGameViews.rbegin();
...
{
	std::list<std::shared_ptr<BaseGameView>>::iterator it = mGameViews.begin();
...
void GameApplication::AddView(const std::shared_ptr<BaseGameView>& pView)
{
...
HumanView* GameApplication::GetHumanView()
{
	...
	for (auto it = mGameViews.begin(); it != mGameViews.end(); ++it)
	{
		...
		{
			std::shared_ptr<BaseGameView> pBaseGameView(*it);
...
const std::shared_ptr<BaseGameView>& GameApplication::GetGameView(GameViewId viewId)
{
	for (auto it = mGameViews.begin(); it != mGameViews.end(); ++it)
		if ((*it)->GetId() == viewId)
			return (*it);
...
const std::shared_ptr<BaseGameView>& GameApplication::GetGameView(GameViewType viewType)
{ 
	for (auto it = mGameViews.begin(); it != mGameViews.end(); ++it)
		if ((*it)->GetType() == viewType)
			return (*it);
...
//remove game view
void GameApplication::RemoveView(const std::shared_ptr<BaseGameView>& pView)
{
...
bool GameApplication::AttachAsClient()
{
	ClientSocketManager *pClient = new ClientSocketManager(
        Settings::Get()->Get("selected_game"), Settings::Get()->GetInt("port"));
	if (!pClient->Connect())
		return false;
