...
public:
	...
	const std::shared_ptr<BaseGameView>& GetGameView(GameViewId viewId);
	const std::shared_ptr<BaseGameView>& GetGameView(GameViewType view);
	const GameViewList& GetGameViews() { return mGameViews; }
	
	virtual void AddView(const std::shared_ptr<BaseGameView>& pView);
	virtual void RemoveView(const std::shared_ptr<BaseGameView>& pView);
	...
	void SetEditorRunning(bool isRunning) { mIsEditorRunning = isRunning; }
	bool IsEditorRunning() { return mIsEditorRunning; }

	bool AttachAsClient();

	// Getter for the game application.
	static GameApplication* Get(void) { return (GameApplication*)Application::App; }

	// Sound System
	std::shared_ptr<SoundSystem> mSoundSystem;

	// File and Resource System
	std::shared_ptr<ResCache> mResCache;

	// Event manager
	std::shared_ptr<EventManager> mEventManager;

	// Socket manager - could be server or client
	std::shared_ptr<BaseSocketManager> mBaseSocketManager;
	std::shared_ptr<NetworkEventForwarder> mNetworkEventForwarder;
