...
std::map<HWND, System*> WindowsSystem::mHandleSystems = {};
...
{
	std::map<HWND, System*>::iterator it = mHandleSystems.begin();
...
LRESULT CALLBACK WindowsSystem::WndProc (HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if ( m->group >= 0 )
	{
        ...
        evt.mMouseInput.X = GET_X_LPARAM(lParam);
        evt.mMouseInput.Y = GET_Y_LPARAM(lParam);
    ...
    switch (iMsg)											
	{
		...
		case WM_KEYUP:
		{
			...
			int conversionResult = 
				ToAsciiEx((UINT)wParam,scanCode,allKeys,keyChars,0,KEYBOARD_INPUT_HKL);
...
//! by the gfx adapter.
std::vector<Vector2<unsigned int>> WindowsSystem::GetVideoResolutions()
{
	...
	std::vector<Vector2<unsigned int>> resolutions;
...
float WindowsSystem::GetDisplayDensity()
{
    HDC hdc = GetDC((HWND)IntToPtr(mWindowID));
    int dpi = GetDeviceCaps(hdc, LOGPIXELSX);
    ReleaseDC((HWND)IntToPtr(mWindowID), hdc);
    return dpi / 96.0f;
}
...
void WindowsSystem::GetSystemVersion(std::string& out)
{
	...
	std::string osviszCSDVersion = ToString(osvi.szCSDVersion);
...
void WindowsSystem::CursorControl::SetPosition(int x, int y)
{
	if (mUseReferenceRect)
	{
        Vector2<int> upperLeftCorner = mReferenceRect.GetVertice(RVP_UPPERLEFT);
		SetCursorPos(upperLeftCorner[0] + x, upperLeftCorner[1] + y);
	}
	else
	{
		RECT rect;
...
void WindowsSystem::CursorControl::UpdateInternalCursorPosition()
{
	...
	{
        Vector2<int> upperLeftCorner = mReferenceRect.GetVertice(RVP_UPPERLEFT);
        mCursorPos[0] = p.x - upperLeftCorner[0];
        mCursorPos[1] = p.y - upperLeftCorner[1];
	}
	else
	{
		RECT rect;
