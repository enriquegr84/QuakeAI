...
bool WaveResourceLoader::IsALoadableFileExtension(const std::wstring& fileName) const
{
...
bool WaveResourceLoader::LoadResource(void *rawBuffer, unsigned int rawSize, const std::shared_ptr<ResHandle>& handle)
{
	std::shared_ptr<SoundResourceExtraData> extra(new SoundResourceExtraData());
...
std::shared_ptr<BaseResourceLoader> CreateWAVResourceLoader()
{
	return std::shared_ptr<BaseResourceLoader>(new WaveResourceLoader());
}

std::shared_ptr<BaseResourceLoader> CreateOGGResourceLoader()
{
	return std::shared_ptr<BaseResourceLoader>(new OggResourceLoader());
}
...
bool OggResourceLoader::IsALoadableFileExtension(const std::wstring& fileName) const
{
	if (fileName.rfind('.') != std::string::npos)
	{
		std::wstring fileExtension = fileName.substr(fileName.rfind('.') + 1);
...
bool OggResourceLoader::LoadResource(void *rawBuffer, unsigned int rawSize, const std::shared_ptr<ResHandle>& handle)
{
	std::shared_ptr<SoundResourceExtraData> extra = 
		std::shared_ptr<SoundResourceExtraData>(new SoundResourceExtraData());
