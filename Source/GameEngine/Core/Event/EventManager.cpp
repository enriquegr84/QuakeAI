...
{
	//LogInformation("Events " + std::string("Attempting to add delegate function for event type: ") + std::to_string(type));
	...
	//LogInformation("Events " + std::string("Successfully added delegate for event type: ") + std::to_string(type));
...
{
	//LogInformation("Events " + std::string("Attempting to remove delegate function from event type: ") + std::to_string(type));
	bool success = false;
	...
	if (findIt != mEventListeners.end())
	{
		...
		for (auto it = listeners.begin(); it != listeners.end(); ++it)
		{
			...
			{
				listeners.erase(it);
				//LogInformation("Events " + std::string("Successfully removed delegate function from event type: ") + std::to_string(type));
				success = true;
...
{
	//LogInformation("Events " + std::string("Attempting to trigger event ") + std::string(pEvent->GetName()));
	...
	if (findIt != mEventListeners.end())
	{
		...
		for (EventListenerList::const_iterator it = eventListenerList.begin(); it != eventListenerList.end(); ++it)
		{
			...
			//LogInformation("Events " + std::string("Sending Event ") + std::string(pEvent->GetName()) + std::string(" to delegate."));
...
bool EventManager::QueueEvent(const BaseEventDataPtr& pEvent)
{
	...
	//LogInformation("Events " + std::string("Attempting to queue event: ") + std::string(pEvent->GetName()));
	auto findIt = mEventListeners.find(pEvent->GetEventType());
	if (findIt != mEventListeners.end())
	{
		MutexAutoLock lock(mMutex);
		mQueues[mActiveQueue].push_back(pEvent);
		//LogInformation("Events " + std::string("Successfully queued event: ") + std::string(pEvent->GetName()));
		return true;
	}
	else
	{
		//LogInformation("Events " + std::string("Skipping event since there are no delegates registered to receive it: ") + std::string(pEvent->GetName()));
		return false;
	}
}

void EventManager::ClearQueue(int queueToClear)
{
	MutexAutoLock lock(mMutex);
	mQueues[mActiveQueue].clear();
}

bool EventManager::ProccessQueue(BaseEventDataPtr& pEvent, int queueToProcess)
{
	MutexAutoLock lock(mMutex);
	...
	if (!mQueues[queueToProcess].empty())
	{
		...
		pEvent = mQueues[queueToProcess].front();
...
bool EventManager::Update(unsigned long maxTime)
{
	...
	ClearQueue(mActiveQueue);
	/*
	LogInformation("EventLoop " + std::string("Processing Event Queue ") + std::to_string(queueToProcess) + "; "
		+ std::to_string((unsigned long)mQueues[queueToProcess].size()) + std::string(" events to process"));
	*/
	// Process the queue
	BaseEventDataPtr pEvent;
	while (ProccessQueue(pEvent, queueToProcess))
	{
		//LogInformation("EventLoop " + std::string("\t\tProcessing Event ") + std::string(pEvent->GetName()));
		...
		if (findIt != mEventListeners.end())
		{
			...
			/*
			LogInformation("EventLoop " + std::string("\t\tFound ") + std::to_string((unsigned long)eventListeners.size())
			...
			*/
			...
			for (auto it = eventListeners.begin(); it != eventListeners.end(); ++it)
			{
				EventListenerDelegate listener = (*it);
				/*
				LogInformation("EventLoop " + std::string("\t\tSending event ") + std::string(pEvent->GetName())
					+ std::string(" to delegate"));
				*/
				listener(pEvent);
			}
		}
