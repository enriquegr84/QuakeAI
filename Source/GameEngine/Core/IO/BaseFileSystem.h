...
public:
	...
	*/
	virtual BaseReadFile* CreateMemoryReadFile(const void* memory, int len, 
		const std::wstring& fileName, bool deleteMemoryWhenDropped = false) = 0;
	...
	See IReferenceCounted::drop() for more information.
	*/
	virtual BaseReadFile* CreateLimitReadFile(const std::wstring& fileName, 
	...
	*/
	...
	//	const std::string& fileName, bool deleteMemoryWhenDropped=false) =0;
	...
	virtual BaseFileList* CreateEmptyFileList(
		const std::wstring& path, bool ignoreCase, bool ignorePaths) =0;
	...
	virtual BaseFileArchive* CreateMountPointFileArchive(
		const std::wstring& filename, bool ignoreCase=true, bool ignorePaths=true) = 0;

	//! Get the current working directory.
	/** \return Current working directory as a string. */
	virtual const std::wstring& GetWorkingDirectory() =0;
	...
	virtual bool ChangeWorkingDirectoryTo(const std::wstring& newDirectory) =0;
    ...
    virtual void GetFileList(
		std::vector<std::wstring>& result, const std::wstring& dir, bool makeFullPath=false) = 0;
	...
	virtual bool ExistDirectory(const std::wstring& dirname) =0;

	//! Determines if a file exists and could be opened.
	/** \param filename is the string identifying the file which should be tested for existence.
	\return True if file exists, and false if it does not exist or an error occured. */
	virtual bool ExistFile(const std::wstring& filename) =0;

	//! Converts a relative path to an absolute (unique) path, resolving symbolic links if required
	/** \param filename Possibly relative file or directory name to query.
	\result Absolute filename which points to the same file. */
	virtual std::wstring GetAbsolutePath(const std::wstring& filename) const =0;

	//! Get the directory a file is located in.
	/** \param filename: The file to get the directory from.
	\return String containing the directory of the file. */
	virtual std::wstring GetFileDirectory(const std::wstring& filename) const =0;

    /* 
        Returns a list of subdirectories, including the path itself, but excluding
        hidden directories (whose names start with . or _)
    */
    virtual std::vector<std::wstring> GetRecursiveDirectories(const std::wstring& dir) =0;
    virtual void GetRecursiveDirectories(std::vector<std::wstring>& dirs, const std::wstring& dir) =0;
    ...
    */
    virtual void GetRecursiveSubPaths(const std::wstring &path,
        std::vector<std::wstring>& dst, bool listPaths, const std::set<char>& ignore) =0;

    //! returns the filename of a filepath. If no directory path is prefixed,
    //! the file path is returned.
    virtual std::wstring GetFileName(const std::wstring& filepath) const =0;
    ...
    //! Write/Update safetly a file
    virtual bool SafeWriteToFile(const std::string& path, const std::string& content) =0;
	...
	virtual bool InsertDirectory(const std::string& directory) =0;
	virtual bool DeleteDirectory(const std::string& directory) =0;
    virtual bool CreateAllDirectories(const std::string& path) =0;
	...
	virtual std::string GetPath(const std::string& filename) =0;
};
