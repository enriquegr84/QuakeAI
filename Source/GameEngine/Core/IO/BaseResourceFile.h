...
public:
	std::wstring mName;
	BaseResource(const std::wstring& name);
};
...
public:
	...
	virtual std::wstring GetResourceName(unsigned int num) const = 0;
	virtual bool IsUsingDevelopmentDirectories(void) const = 0;

	//! determines if a file exists and would be able to be opened.
	virtual bool ExistFile(const std::wstring& filename) const = 0;

	//! determines if a directory exists and would be able to be opened.
	virtual bool ExistDirectory(const std::wstring& dirname) const = 0;

	//! Check if the file might be loaded by this class
	/** Check is based on the file extension (e.g. ".tga")
	\param filename Name of file to check.
	\return True if file seems to be loadable. */
	virtual bool IsALoadableFileFormat(const std::wstring& filename) const = 0;
