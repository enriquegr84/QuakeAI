...
static const std::wstring emptyFileListEntry;

FileList::FileList(const std::wstring& filepath, bool ignoreCase, bool ignorePaths)
 : mIgnorePaths(ignorePaths), mIgnoreCase(ignoreCase), mFileListPath(filepath)
{
	std::replace(mFileListPath.begin(), mFileListPath.end(), L'\\', L'/');
}
...
const std::wstring& FileList::GetFileName(unsigned int index) const
{
...
const std::wstring& FileList::GetFullFileName(unsigned int index) const
{
	if (index >= mFiles.size())
		return emptyFileListEntry;
...
unsigned int FileList::AddItem(const std::wstring& fullPath, 
	unsigned int offset, unsigned int size, bool isDirectory, unsigned int id)
{
	FileListEntry entry;
	entry.mID   = id ? id : (unsigned int)mFiles.size();
	entry.mOffset = offset;
	entry.mSize = size;
	entry.mName = fullPath;
	std::replace(entry.mName.begin(), entry.mName.end(), L'\\', L'/');
	...
	if (mIgnoreCase)
		std::transform(entry.mName.begin(), entry.mName.end(), entry.mName.begin(), ::tolower);
	...
	if (entry.mName.rfind('/') != std::string::npos)
		entry.mName = entry.mName.substr(entry.mName.rfind('/') + 1);

	//LogInformation(mPath entry.mFullName);
	mFiles.push_back(entry);

	return (unsigned int)(mFiles.size() - 1);
}
...
int FileList::FindFile(const std::wstring& filename, bool isDirectory = false) const
{
	FileListEntry entry;
	...
	// exchange
	std::replace(entry.mFullName.begin(), entry.mFullName.end(), L'\\', L'/');
	...
	if (mIgnoreCase)
		std::transform(entry.mFullName.begin(), entry.mFullName.end(), entry.mFullName.begin(), ::tolower);
...
const std::wstring& FileList::GetPath() const
{
