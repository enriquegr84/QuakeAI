...
public:
	...
	FileList(const std::wstring& filepath, bool ignoreCase, bool ignorePaths);
	...
	virtual const std::wstring& GetFileName(unsigned int index) const;

	//! Gets the full name of a file in the list including the path, based on an index.
	/** \param index is the zero based index of the file which name should
	be returned. The index must be less than the amount getFileCount() returns.
	\return File name of the file. Returns 0 if an error occured. */
	virtual const std::wstring& GetFullFileName(unsigned int index) const;
	...
	virtual int FindFile(const std::wstring& filename, bool isFolder) const;
	...
	virtual unsigned int AddItem(const std::wstring& fullPath, 
...
protected:
	...
	std::wstring mFileListPath;

	//! List of files
	std::vector<FileListEntry> mFiles;
};
