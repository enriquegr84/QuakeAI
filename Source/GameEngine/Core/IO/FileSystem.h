...
public:
	...
	virtual BaseReadFile* CreateMemoryReadFile(const void* memory, int len, 
		const std::wstring& fileName, bool deleteMemoryWhenDropped = false);
	...
	\param fileName: The name given to this file
	\return A pointer to the created file interface.*/
	virtual BaseReadFile* CreateReadFile(const std::wstring& fileName);
	...
	virtual BaseFileList* CreateEmptyFileList(
		const std::wstring& path, bool ignoreCase, bool ignorePaths);
	...
	virtual BaseFileArchive* CreateMountPointFileArchive(
		const std::wstring& filename, bool ignoreCase, bool ignorePaths);

	//! Get the current working directory.
	/** \return Current working directory as a string. */
	virtual const std::wstring& GetWorkingDirectory();
	...
	virtual bool ChangeWorkingDirectoryTo(const std::wstring& newDirectory);
    ...
    virtual void GetFileList(
		std::vector<std::wstring>& result, const std::wstring& dir, bool makeFullPath=false);
	...
	virtual bool ExistDirectory(const std::wstring& dirname);

	//! Determines if a file exists and could be opened.
	/** \param filename is the string identifying the file which should be tested for existence.
	\return True if file exists, and false if it does not exist or an error occured. */
	virtual bool ExistFile(const std::wstring& filename);

	//! Converts a relative path to an absolute (unique) path, resolving symbolic links if required
	/** \param filename Possibly relative file or directory name to query.
	\result Absolute filename which points to the same file. */
	virtual std::wstring GetAbsolutePath(const std::wstring& filename) const;

	//! Get the directory a file is located in.
	/** \param filename: The file to get the directory from.
	\return String containing the directory of the file. */
	virtual std::wstring GetFileDirectory(const std::wstring& filename) const;

    /*
        Returns a list of subdirectories, including the path itself, but excluding
        hidden directories (whose names start with . or _)
    */
    virtual std::vector<std::wstring> GetRecursiveDirectories(const std::wstring& dir);
    virtual void GetRecursiveDirectories(std::vector<std::wstring>& dirs, const std::wstring& dir);
    ...
    */
    ...
    virtual std::wstring GetFileName(const std::wstring& filepath) const;
    ...
    virtual bool SafeWriteToFile(const std::string& path, const std::string& content);
	...
	virtual bool InsertDirectory(const std::string& directory);
	virtual bool DeleteDirectory(const std::string& directory);
    virtual bool CreateAllDirectories(const std::string& path);
...
private:
	...
	std::wstring mWorkingDirectory[2];
	...
	std::vector<std::string> msDirectories;
};
