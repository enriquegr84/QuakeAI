...
void EnrichedString::Clear()
{
	...
	mDefaultLength = 0;
	mDefaultColor = SColor(255, 255, 255, 255);
	mBackground = SColor();
}
...
void EnrichedString::AddAtEnd(const std::wstring &s, SColor initialColor)
{
	SColor color(initialColor);
	...
	while (i < s.length()) 
    {
		...
		std::wstring escapeSequence(s, startIndex, length);
		...
		if (parts[0] == L"c") 
        {
			...
			ParseColorString(ToString(parts[1]), color, true);
        ...
        else if (parts[0] == L"b") 
        {
			...
			ParseColorString(ToString(parts[1]), mBackground, true);
			mHasBackground = true;
		}
	}
...
EnrichedString EnrichedString::Substr(size_t pos, size_t len) const
{
	...
	if (len == std::string::npos || pos + len > mString.length())
		len = mString.length() - pos;

	EnrichedString str(mString.substr(pos, len),
		std::vector<SColor>(mColors.begin() + pos, mColors.begin() + pos + len)
	);
	...
	if (pos < mDefaultLength)
		str.mDefaultLength = std::min(mDefaultLength - pos, str.Size());
...
{
	LogAssert(mDefaultLength <= mColors.size(), "Invalid color length");
