...
public:
	...
	EnrichedString(const wchar_t *str, const SColor &color = SColor(255, 255, 255, 255));
	EnrichedString(const std::wstring &string, const std::vector<SColor> &colors);
	void operator=(const wchar_t *str);

	void Clear();

	void AddAtEnd(const std::wstring &s, SColor color);
	...
	inline void SetDefaultColor(SColor color)
	{
		mDefaultColor = color;
		UpdateDefaultColor();
	}
	void UpdateDefaultColor();
	inline SColorF GetDefaultColor() const
	{
	...
	inline SColor GetBackground() const
	{
		return mBackground;
	}
	inline void SetBackground(SColor color)
	{
		mBackground = color;
		mHasBackground = true;
	}
...
private:
	std::wstring mString;
	std::vector<SColor> mColors;
	bool mHasBackground;
	SColor mDefaultColor;
	SColor mBackground;
