...
Actor::~Actor(void)
{
    LogInformation("Destroying Actor " + std::to_string(mID));
...
bool Actor::Init(tinyxml2::XMLElement* pData)
{
    ...
    if (pData->Attribute("resource"))
        mResource = pData->Attribute("resource");
    LogInformation("Initializing Actor " + std::to_string(mID) + " " + mType);
...
std::string Actor::ToXML()
{
    ...
    {
        std::shared_ptr<ActorComponent> pComponent = it->second;
...
void Actor::AddComponent(std::shared_ptr<ActorComponent> pComponent)
{
    std::pair<ActorComponents::iterator, bool> success = 
		mComponents.insert(std::make_pair(pComponent->GetId(), pComponent));
...
void Actor::RemoveComponent(ComponentId id)
{
    ...
    LogAssert(findIt != mComponents.end(), "error remove component");
