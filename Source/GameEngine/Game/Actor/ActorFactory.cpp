...
std::shared_ptr<Actor> ActorFactory::CreateActor(const wchar_t* actorResource, 
...
std::shared_ptr<ActorComponent> ActorFactory::CreateComponent(
	std::shared_ptr<Actor> pActor, tinyxml2::XMLElement* pData)
{
    ...
    if (pComponent)
    {
        ...
        {
            LogError(std::string("Component failed to initialize: ") + std::string(name));
    ...
    {
        LogError(std::string("Couldn't find ActorComponent named ") + std::string(name));
