...
bool PhysicComponent::Init(tinyxml2::XMLElement* pData)
{
	...
	// physic surface
	tinyxml2::XMLElement* pConvexSurface = pData->FirstChildElement("ConvexSurface");
	if (pConvexSurface)
	{
		for (auto const& convexSurface : StringSplit(Trim(pConvexSurface->FirstChild()->Value()), ','))
	...
	{
		for (auto const& ignoreBSPSurface : StringSplit(Trim(pIgnoreBSPSurface->FirstChild()->Value()), ','))
			mIgnoreBSPSurfaces.insert(std::atoi(ignoreBSPSurface.c_str()));
	}

	tinyxml2::XMLElement* pIgnorePhysSurface = pData->FirstChildElement("IgnorePhysSurface");
	if (pIgnorePhysSurface)
	{
		for (auto const& ignorePhysSurface : StringSplit(Trim(pIgnorePhysSurface->FirstChild()->Value()), ','))
			mIgnorePhysSurfaces.insert(std::atoi(ignorePhysSurface.c_str()));
	}
...
tinyxml2::XMLElement* PhysicComponent::GenerateXml(void)
{
    ...
    pPosition->SetAttribute("x", std::to_string(mRigidBodyLocation[0]).c_str());
    pPosition->SetAttribute("y", std::to_string(mRigidBodyLocation[1]).c_str());
    pPosition->SetAttribute("z", std::to_string(mRigidBodyLocation[2]).c_str());
    ...
    pOrientation->SetAttribute("x", std::to_string(mRigidBodyOrientation[0]).c_str());
    pOrientation->SetAttribute("y", std::to_string(mRigidBodyOrientation[1]).c_str());
    pOrientation->SetAttribute("z", std::to_string(mRigidBodyOrientation[2]).c_str());
    pInitialTransform->LinkEndChild(pOrientation);

	// initial transform -> scale 
	tinyxml2::XMLElement* pScale = doc.NewElement("Scale");
    pScale->SetAttribute("x", std::to_string(mRigidBodyScale[0]).c_str());
    pScale->SetAttribute("y", std::to_string(mRigidBodyScale[1]).c_str());
    pScale->SetAttribute("z", std::to_string(mRigidBodyScale[2]).c_str());
	...
	// surface
	tinyxml2::XMLElement* pConvexSurface = doc.NewElement("ConvexSurface");
	for (int convexSurface : mConvexSurfaces)
	{
		tinyxml2::XMLText* pConvexSurfaceText = doc.NewText(std::to_string(convexSurface).c_str());
		pConvexSurface->LinkEndChild(pConvexSurfaceText);
	}
	...
	{
		tinyxml2::XMLText* pIgnoreBSPSurfaceText = doc.NewText(std::to_string(ignoreBSPSurface).c_str());
		pIgnoreBSPSurface->LinkEndChild(pIgnoreBSPSurfaceText);
	}
	pBaseElement->LinkEndChild(pIgnoreBSPSurface);

	tinyxml2::XMLElement* pIgnorePhysSurface = doc.NewElement("IgnorePhysSurface");
	for (int ignorePhysSurface : mIgnorePhysSurfaces)
	{
		tinyxml2::XMLText* pIgnorePhysSurfaceText = doc.NewText(std::to_string(ignorePhysSurface).c_str());
		pIgnorePhysSurface->LinkEndChild(pIgnorePhysSurfaceText);
	}
	pBaseElement->LinkEndChild(pIgnorePhysSurface);
    ...
    if (mOwner)
    {
		...
		else if (mShape == "BSP")
		{
			std::shared_ptr<ResHandle>& resHandle =
				ResCache::Get()->GetHandle(&BaseResource(ToWideString(mMesh)));
			if (resHandle)
			{
				const std::shared_ptr<BspResourceExtraData>& extra =
					std::static_pointer_cast<BspResourceExtraData>(resHandle->GetExtra());
				gamePhysics->AddBSP(extra->GetLoader(), mConvexSurfaces, mIgnoreBSPSurfaces, mIgnorePhysSurfaces, mOwner, mDensity, mMaterial);
			}
		}
...
{
	BaseGamePhysic* gamePhysics = GameLogic::Get()->GetGamePhysics().get();
	gamePhysics->Move(mOwner->GetId(), direction);
}

void PhysicComponent::KinematicFall(const Vector3<float>& direction)
{
	BaseGamePhysic* gamePhysics = GameLogic::Get()->GetGamePhysics().get();
	gamePhysics->Fall(mOwner->GetId(), direction);
}
...
{
	BaseGamePhysic* gamePhysics = GameLogic::Get()->GetGamePhysics().get();
	gamePhysics->SetCollisionFlags(mOwner->GetId(), collisionFlags);
}
