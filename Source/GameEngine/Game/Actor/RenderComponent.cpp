...
const char* CloudRenderComponent::Name = "CloudRenderComponent";
...
std::shared_ptr<Node> MeshRenderComponent::CreateSceneNode(void)
{
	...
	if (pTransformComponent)
	{
		...
		for (std::string meshName : mMeshes)
		{
			std::shared_ptr<ResHandle>& resHandle =
				ResCache::Get()->GetHandle(&BaseResource(ToWideString(meshName)));
			if (resHandle)
			{
	...
	tinyxml2::XMLDocument doc, tinyxml2::XMLElement* pBaseElement)
{
	...
	{
		tinyxml2::XMLText* pMeshFile = doc.NewText(mesh.c_str());
	...
	{
		tinyxml2::XMLText* pTexureFile = doc.NewText(texture.c_str());
		pTexture->LinkEndChild(pTexureFile);
	}
...
std::shared_ptr<Node> SphereRenderComponent::CreateSceneNode(void)
{
    // get the transform component
    const std::shared_ptr<TransformComponent>& pTransformComponent(
		mOwner->GetComponent<TransformComponent>(TransformComponent::Name).lock());
	if (pTransformComponent)
	{
		...
		std::shared_ptr<ResHandle>& resHandle =
			ResCache::Get()->GetHandle(&BaseResource(ToWideString(mTextureResource)));
		if (resHandle)
		{
			const std::shared_ptr<ImageResourceExtraData>& extra =
				std::static_pointer_cast<ImageResourceExtraData>(resHandle->GetExtra());
	...
	tinyxml2::XMLDocument doc, tinyxml2::XMLElement* pBaseElement)
{
	...
	pMesh->SetAttribute("radius", std::to_string(mRadius).c_str());
    pMesh->SetAttribute("segments", std::to_string(mSegments).c_str());
    pBaseElement->LinkEndChild(pBaseElement);

	tinyxml2::XMLElement* pTexture = doc.NewElement("Texture");
	pTexture->SetAttribute("file", mTextureResource.c_str());
	pTexture->SetAttribute("x", std::to_string(mTextureSize[0]).c_str());
	pTexture->SetAttribute("y", std::to_string(mTextureSize[1]).c_str());
	pBaseElement->LinkEndChild(pTexture);
...
std::shared_ptr<Node> BoxRenderComponent::CreateSceneNode(void)
{
	const std::shared_ptr<TransformComponent>& pTransformComponent(
		mOwner->GetComponent<TransformComponent>(TransformComponent::Name).lock());
	if (pTransformComponent)
	{
		...
		if (resHandle)
		{
			...
			std::shared_ptr<Node> boxNode = pScene->AddBoxNode(0, 
			...
			if (boxNode)
			{
				...
				if (mMaterialType == MaterialType::MT_TRANSPARENT)
				{
					...
					{
						std::shared_ptr<Material> material = boxNode->GetMaterial(i);
						material->mBlendTarget.enable = true;
						material->mBlendTarget.srcColor = BlendState::BM_ONE;
						material->mBlendTarget.dstColor = BlendState::BM_INV_SRC_COLOR;
						material->mBlendTarget.srcAlpha = BlendState::BM_SRC_ALPHA;
	...
	tinyxml2::XMLDocument doc, tinyxml2::XMLElement *pBaseElement)
{
	...
	pBox->SetAttribute("x", std::to_string(mSize[0]).c_str());
	pBox->SetAttribute("y", std::to_string(mSize[1]).c_str());
	pBox->SetAttribute("z", std::to_string(mSize[2]).c_str());
...
//---------------------------------------------------------------------------------------------------------------------
// CloudRenderComponent
//---------------------------------------------------------------------------------------------------------------------
CloudRenderComponent::CloudRenderComponent(void)
{
    mMaterialType = 0;
    mSize = 0;
}

bool CloudRenderComponent::DelegateInit(tinyxml2::XMLElement* pData)
{
    tinyxml2::XMLElement* pCloud = pData->FirstChildElement("Cloud");
    if (pCloud)
    {
        float x = 1.f;
        float y = 1.f;
        float z = 1.f;
        x = pCloud->FloatAttribute("x", x);
        y = pCloud->FloatAttribute("y", y);
        z = pCloud->FloatAttribute("z", z);
        mSize = Vector3<float>{ x, y, z };
    }
...
std::shared_ptr<Node> CloudRenderComponent::CreateSceneNode(void)
{
    const std::shared_ptr<TransformComponent>& pTransformComponent(
        mOwner->GetComponent<TransformComponent>(TransformComponent::Name).lock());
    if (pTransformComponent)
    {
        ...
        // create an animated mesh scene node with specified mesh.
        std::shared_ptr<Node> cloudSystemNode = pScene->AddCloudSystemNode(0, mOwner->GetId());
        ...
        if (cloudSystemNode)
        {
            cloudSystemNode->GetRelativeTransform() = transform;
			cloudSystemNode->UpdateAbsoluteTransform();

            if (mMaterialType == MaterialType::MT_TRANSPARENT)
            {
                for (unsigned int i = 0; i < cloudSystemNode->GetMaterialCount(); ++i)
                {
                    std::shared_ptr<Material> material = cloudSystemNode->GetMaterial(i);
                    material->mBlendTarget.enable = true;
					material->mBlendTarget.srcColor = BlendState::BM_ONE;
					material->mBlendTarget.dstColor = BlendState::BM_INV_SRC_COLOR;
                    material->mBlendTarget.srcAlpha = BlendState::BM_SRC_ALPHA;
            ...
            for (unsigned int i = 0; i < cloudSystemNode->GetMaterialCount(); ++i)
                cloudSystemNode->GetMaterial(i)->mLighting = false;
            cloudSystemNode->SetMaterialType((MaterialType)mMaterialType);
        }

        return cloudSystemNode;
    }

    return std::shared_ptr<Node>();
}

void CloudRenderComponent::CreateInheritedXMLElements(
    tinyxml2::XMLDocument doc, tinyxml2::XMLElement *pBaseElement)
{
    tinyxml2::XMLElement* pCloud = doc.NewElement("Cloud");
    pCloud->SetAttribute("x", std::to_string(mSize[0]).c_str());
    pCloud->SetAttribute("y", std::to_string(mSize[1]).c_str());
    pCloud->SetAttribute("z", std::to_string(mSize[2]).c_str());
    pBaseElement->LinkEndChild(pCloud);
...
std::shared_ptr<Node> GridRenderComponent::CreateSceneNode(void)
{
    const std::shared_ptr<TransformComponent>& pTransformComponent(
		mOwner->GetComponent<TransformComponent>(TransformComponent::Name).lock());
    if (pTransformComponent)
    {
		...
		if (resHandle)
		{
			...
			// create an animated mesh scene node with specified mesh.
			std::shared_ptr<Node> gridNode = pScene->AddRectangleNode(
			...
			if (gridNode)
			{
				...
				gridNode->UpdateAbsoluteTransform();

				if (mMaterialType == MaterialType::MT_TRANSPARENT)
				{
					for (unsigned int i = 0; i<gridNode->GetMaterialCount(); ++i)
					{
						std::shared_ptr<Material> material = gridNode->GetMaterial(i);
						material->mBlendTarget.enable = true;
						material->mBlendTarget.srcColor = BlendState::BM_ONE;
						material->mBlendTarget.dstColor = BlendState::BM_INV_SRC_COLOR;
						material->mBlendTarget.srcAlpha = BlendState::BM_SRC_ALPHA;
	...
	tinyxml2::XMLDocument doc, tinyxml2::XMLElement *pBaseElement)
{
	...
	pSegment->SetAttribute("x", std::to_string(mSegments[0]).c_str());
	pSegment->SetAttribute("y", std::to_string(mSegments[1]).c_str());
	pBaseElement->LinkEndChild(pSegment);

	tinyxml2::XMLElement* pExtent = doc.NewElement("Extent");
	pExtent->SetAttribute("x", std::to_string(mExtent[0]).c_str());
	pExtent->SetAttribute("y", std::to_string(mExtent[1]).c_str());
...
bool BillboardRenderComponent::DelegateInit(tinyxml2::XMLElement* pData)
{
	...
	if (pAnimation)
	{
		...
		unsigned int time = 0;
		mAnimationTime = pAnimation->IntAttribute("time", time);
	}
...
std::shared_ptr<Node> BillboardRenderComponent::CreateSceneNode(void)
{
	const std::shared_ptr<TransformComponent>& pTransformComponent(
		mOwner->GetComponent<TransformComponent>(TransformComponent::Name).lock());
	if (pTransformComponent)
	{
		...
		std::shared_ptr<ResHandle>& resHandle =
			ResCache::Get()->GetHandle(&BaseResource(ToWideString(mTextures.front())));
		if (resHandle)
		{
			const std::shared_ptr<ImageResourceExtraData>& extra =
				std::static_pointer_cast<ImageResourceExtraData>(resHandle->GetExtra());
			...
			// Add billboard
			std::shared_ptr<Node> billboardNode =
				pScene->AddBillboardNode(0, extra->GetImage(), mTextureSize, mOwner->GetId());
			if (billboardNode)
			{
				billboardNode->GetRelativeTransform() = transform;
				billboardNode->UpdateAbsoluteTransform();

				if (mAnimationType & NAT_TEXTURE)
				{
					std::vector<std::shared_ptr<Texture2>> textures;
					for (auto const& texture : mTextures)
					{
						std::shared_ptr<ResHandle>& resHandle = 
							ResCache::Get()->GetHandle(&BaseResource(ToWideString(texture)));
					...
					std::shared_ptr<NodeAnimator> anim = 0;
					anim = pScene->CreateTextureAnimator(textures, mAnimationTime / (unsigned int)textures.size(), false);
					billboardNode->AttachAnimator(anim);
				}

				if (mAnimationType & NAT_DELETION)
				{
					std::shared_ptr<NodeAnimator> anim = pScene->CreateDeleteAnimator(mAnimationTime);
					billboardNode->AttachAnimator(anim);
				}
	...
	tinyxml2::XMLDocument doc, tinyxml2::XMLElement *pBaseElement)
{
	...
	tinyxml2::XMLElement* pAnimation = doc.NewElement("Animation");
	pAnimation->SetAttribute("type", std::to_string(mAnimationType).c_str());
	pAnimation->SetAttribute("time", std::to_string(mAnimationTime).c_str());
	pBaseElement->LinkEndChild(pAnimation);
	...
	for (std::string texture : mTextures)
	{
		tinyxml2::XMLText* pTextureFile = doc.NewText(texture.c_str());
		pTexture->LinkEndChild(pTextureFile);
	}
	pBaseElement->LinkEndChild(pTexture);

	tinyxml2::XMLElement* pColor = doc.NewElement("TextureColor");
	pColor->SetAttribute("r", std::to_string(mTextureColor[0]).c_str());
	pColor->SetAttribute("g", std::to_string(mTextureColor[1]).c_str());
	pColor->SetAttribute("b", std::to_string(mTextureColor[2]).c_str());
	pColor->SetAttribute("a", std::to_string(mTextureColor[3]).c_str());
	pBaseElement->LinkEndChild(pColor);

	tinyxml2::XMLElement* pSize = doc.NewElement("TextureSize");
	pSize->SetAttribute("x", std::to_string(mTextureSize[0]).c_str());
	pSize->SetAttribute("y", std::to_string(mTextureSize[1]).c_str());
...
std::shared_ptr<Node> VolumeLightRenderComponent::CreateSceneNode(void)
{
	const std::shared_ptr<TransformComponent>& pTransformComponent(
		mOwner->GetComponent<TransformComponent>(TransformComponent::Name).lock());
	if (pTransformComponent)
	{
		...
		if (resHandle)
		{
			...
			// Add volumelight
			std::shared_ptr<Node> volumeLightNode = pScene->AddVolumeLightNode(0, 
			...
			if (volumeLightNode)
			{
				...
				volumeLightNode->UpdateAbsoluteTransform();
				...
				if (mMaterialType == MT_TRANSPARENT)
				{
					for (unsigned int i = 0; i < volumeLightNode->GetMaterialCount(); ++i)
					{
						std::shared_ptr<Material> material = volumeLightNode->GetMaterial(i);
						material->mBlendTarget.enable = true;
						material->mBlendTarget.srcColor = BlendState::BM_ONE;
						material->mBlendTarget.dstColor = BlendState::BM_INV_SRC_COLOR;
						material->mBlendTarget.srcAlpha = BlendState::BM_SRC_ALPHA;
	...
	tinyxml2::XMLDocument doc, tinyxml2::XMLElement *pBaseElement)
{
	...
	pSubdivision->SetAttribute("u", std::to_string(mSubdivision[0]).c_str());
	pSubdivision->SetAttribute("v", std::to_string(mSubdivision[1]).c_str());
	pBaseElement->LinkEndChild(pSubdivision);

	tinyxml2::XMLElement* pFootColor = doc.NewElement("FootColor");
	pFootColor->SetAttribute("r", std::to_string(mFootColor.GetRed()).c_str());
	pFootColor->SetAttribute("g", std::to_string(mFootColor.GetGreen()).c_str());
	pFootColor->SetAttribute("b", std::to_string(mFootColor.GetBlue()).c_str());
	pFootColor->SetAttribute("a", std::to_string(mFootColor.GetAlpha()).c_str());
	pBaseElement->LinkEndChild(pFootColor);

	tinyxml2::XMLElement* pTailColor = doc.NewElement("FootColor");
	pTailColor->SetAttribute("r", std::to_string(mTailColor.GetRed()).c_str());
	pTailColor->SetAttribute("g", std::to_string(mTailColor.GetGreen()).c_str());
	pTailColor->SetAttribute("b", std::to_string(mTailColor.GetBlue()).c_str());
	pTailColor->SetAttribute("a", std::to_string(mTailColor.GetAlpha()).c_str());
...
#if defined(_OPENGL_)
	mLightData = std::make_shared<Light>(true, false);
#else
	mLightData = std::make_shared<Light>(true, true);
#endif
	mLightData->mLighting = std::make_shared<Lighting>();
}
...
bool LightRenderComponent::DelegateInit(tinyxml2::XMLElement* pData)
{
	...
	if (pAnimator)
	{
		...
		if (pAnimation)
		{
			...
			float radius = 0;
			mAnimationRadius = pAnimation->FloatAttribute("radius", radius);
		}
		...
		if (pCenter)
		{
			...
			mAnimationCenter[0] = temp;

			temp = pCenter->FloatAttribute("y", temp);
			mAnimationCenter[1] = temp;

			temp = pCenter->FloatAttribute("z", temp);
			mAnimationCenter[2] = temp;
		}
	}
...
std::shared_ptr<Node> LightRenderComponent::CreateSceneNode(void)
{
    const std::shared_ptr<TransformComponent>& pTransformComponent(
		mOwner->GetComponent<TransformComponent>(TransformComponent::Name).lock());
    if (pTransformComponent)
    {
		...
		// Add light
		std::shared_ptr<Node> lightNode = pScene->AddLightNode(0, mLightData, mOwner->GetId());
		if (lightNode)
		{
			lightNode->GetRelativeTransform() = transform;
			lightNode->UpdateAbsoluteTransform();

			if (mAnimationType & NAT_FLY_CIRCLE)
			{
				std::shared_ptr<NodeAnimator> anim = 0;
				anim = pScene->CreateFlyCircleAnimator(mAnimationCenter, mAnimationRadius);
	...
	tinyxml2::XMLDocument doc, tinyxml2::XMLElement *pBaseElement)
{
	...
	pLighting->SetAttribute("type", std::to_string(mLightData->mLighting->mType).c_str());
	...
	pDirection->SetAttribute("x", std::to_string(mLightData->mLighting->mDirection[0]).c_str());
	pDirection->SetAttribute("y", std::to_string(mLightData->mLighting->mDirection[1]).c_str());
	pDirection->SetAttribute("z", std::to_string(mLightData->mLighting->mDirection[2]).c_str());
	pLightElement->LinkEndChild(pDirection);

	// Ambient
	tinyxml2::XMLElement* pAmbient = doc.NewElement("Ambient");
	pAmbient->SetAttribute("r", std::to_string(mLightData->mLighting->mAmbient[0]).c_str());
	pAmbient->SetAttribute("g", std::to_string(mLightData->mLighting->mAmbient[1]).c_str());
	pAmbient->SetAttribute("b", std::to_string(mLightData->mLighting->mAmbient[2]).c_str());
	pAmbient->SetAttribute("a", std::to_string(mLightData->mLighting->mAmbient[3]).c_str());
	pLightElement->LinkEndChild(pAmbient);

	// Diffuse
	tinyxml2::XMLElement* pDiffuse = doc.NewElement("Diffuse");
	pDiffuse->SetAttribute("r", std::to_string(mLightData->mLighting->mDiffuse[0]).c_str());
	pDiffuse->SetAttribute("g", std::to_string(mLightData->mLighting->mDiffuse[1]).c_str());
	pDiffuse->SetAttribute("b", std::to_string(mLightData->mLighting->mDiffuse[2]).c_str());
	pDiffuse->SetAttribute("a", std::to_string(mLightData->mLighting->mDiffuse[3]).c_str());
	pLightElement->LinkEndChild(pDiffuse);

	// Specular
	tinyxml2::XMLElement* pSpecular = doc.NewElement("Specular");
	pSpecular->SetAttribute("r", std::to_string(mLightData->mLighting->mSpecular[0]).c_str());
	pSpecular->SetAttribute("g", std::to_string(mLightData->mLighting->mSpecular[1]).c_str());
	pSpecular->SetAttribute("b", std::to_string(mLightData->mLighting->mSpecular[2]).c_str());
	pSpecular->SetAttribute("a", std::to_string(mLightData->mLighting->mSpecular[3]).c_str());
	pLightElement->LinkEndChild(pSpecular);

    // attenuation
	tinyxml2::XMLElement* pAttenuation = doc.NewElement("Attenuation");
    pAttenuation->SetAttribute("const", std::to_string(mLightData->mLighting->mAttenuation[0]).c_str());
    pAttenuation->SetAttribute("linear", std::to_string(mLightData->mLighting->mAttenuation[1]).c_str());
    pAttenuation->SetAttribute("exp", std::to_string(mLightData->mLighting->mAttenuation[2]).c_str());
	pAttenuation->SetAttribute("int", std::to_string(mLightData->mLighting->mAttenuation[3]).c_str());
    pLightElement->LinkEndChild(pAttenuation);

    // spotcutoff
	tinyxml2::XMLElement* pSpotCutoff = doc.NewElement("SpotCutoff");
	pSpotCutoff->SetAttribute("angle", std::to_string(mLightData->mLighting->mSpotCutoff[0]).c_str());
	pSpotCutoff->SetAttribute("exponent", std::to_string(mLightData->mLighting->mSpotCutoff[3]).c_str());
	...
	tinyxml2::XMLElement* pAnimation = doc.NewElement("Animation");
	pAnimation->SetAttribute("type", std::to_string(mAnimationType).c_str());
	pAnimation->SetAttribute("radius", std::to_string(mAnimationRadius).c_str());
	pAnimatorElement->LinkEndChild(pAnimation);

	tinyxml2::XMLElement* pCenter = doc.NewElement("Center");
	pCenter->SetAttribute("x", std::to_string(mAnimationCenter[0]).c_str());
	pCenter->SetAttribute("y", std::to_string(mAnimationCenter[1]).c_str());
	pCenter->SetAttribute("z", std::to_string(mAnimationCenter[2]).c_str());
...
bool ParticleEffectRenderComponent::DelegateInit(tinyxml2::XMLElement* pData)
{
	...
	if (pColorMinimum)
	{
		...
		a = pColorMinimum->FloatAttribute("a", a);
		r = pColorMinimum->FloatAttribute("r", r);
		g = pColorMinimum->FloatAttribute("g", g);
		b = pColorMinimum->FloatAttribute("b", b);
		mMinStartColor.Set(a, r, g, b);
	}
	...
	if (pColorMaximum)
	{
		...
		a = pColorMaximum->FloatAttribute("a", a);
		r = pColorMaximum->FloatAttribute("r", r);
		g = pColorMaximum->FloatAttribute("g", g);
		b = pColorMaximum->FloatAttribute("b", b);
		mMaxStartColor.Set(a, r, g, b);
	}
...
std::shared_ptr<Node> ParticleEffectRenderComponent::CreateSceneNode(void)
{
    const std::shared_ptr<TransformComponent>& pTransformComponent(
		mOwner->GetComponent<TransformComponent>(TransformComponent::Name).lock());
    if (pTransformComponent)
    {
		...
		std::shared_ptr<Node> node = pScene->AddParticleSystemNode(0, mOwner->GetId(), false);
		if (node)
		{
			std::shared_ptr<ParticleSystemNode> particleSystem = 
				std::dynamic_pointer_cast<ParticleSystemNode>(node);
			particleSystem->GetRelativeTransform() = transform;
			particleSystem->UpdateAbsoluteTransform();

			switch (mEmitterType)
			{
				case PET_POINT:
				{
					std::shared_ptr<BaseParticleEmitter> em(particleSystem->CreatePointEmitter(
				...
				case PET_BOX:
				{
					std::shared_ptr<BaseParticleEmitter> em(particleSystem->CreateBoxEmitter(
				...
				case PET_CYLINDER:
				{
					std::shared_ptr<BaseParticleEmitter> em(particleSystem->CreateCylinderEmitter(
						Vector3<float>::Zero(), 0.0f, Vector3<float>::Zero(), 0.0f, true,
						mDirection, mMinParticlesPerSecond, mMaxParticlesPerSecond,
						mMinStartColor, mMaxStartColor, mMinLifeTime, mMaxLifeTime,
						mMaxAngle, mMinStartSize, mMaxStartSize));
					particleSystem->SetEmitter(em); // this grabs the emitter
				...
				case PET_MESH:
				{
					std::shared_ptr<BaseParticleEmitter> em(particleSystem->CreateMeshEmitter(
						std::shared_ptr<BaseMesh>(), false, mDirection, 0, 0, false,
				...
				case PET_RING:
				{
					std::shared_ptr<BaseParticleEmitter> em(particleSystem->CreateRingEmitter(
						Vector3<float>::Zero(), 0.0f, 0.0f, mDirection, mMinParticlesPerSecond, mMaxParticlesPerSecond,
						mMinStartColor, mMaxStartColor, mMinLifeTime, mMaxLifeTime,
						mMaxAngle, mMinStartSize, mMaxStartSize));
					particleSystem->SetEmitter(em); // this grabs the emitter
					break;
				...
				case PET_SPHERE:
				{
					std::shared_ptr<BaseParticleEmitter> em(particleSystem->CreateSphereEmitter(
						Vector3<float>::Zero(), 0.0f, mDirection, mMinParticlesPerSecond, mMaxParticlesPerSecond,
						mMinStartColor, mMaxStartColor, mMinLifeTime, mMaxLifeTime,
						mMaxAngle, mMinStartSize, mMaxStartSize));
					particleSystem->SetEmitter(em); // this grabs the emitter
					break;
			...
			switch (mAffectorType)
			{
				case PAT_ATTRACT:
				{
					std::shared_ptr<BaseParticleAffector> particleAffector(
			...
			if (resHandle)
			{
				...
				if (mMaterialType == MaterialType::MT_TRANSPARENT)
				{
					for (unsigned int i = 0; i<particleSystem->GetMaterialCount(); ++i)
					{
						std::shared_ptr<Material> material = particleSystem->GetMaterial(i);
	...
	tinyxml2::XMLDocument doc, tinyxml2::XMLElement *pBaseElement)
{
	...
	pAffector->SetAttribute("type", std::to_string(mAffectorType).c_str());
	pBaseElement->LinkEndChild(pAffector);

	tinyxml2::XMLElement* pEmitter = doc.NewElement("Emitter");
	pEmitter->SetAttribute("minx", std::to_string(mEmitter.mMin[0]).c_str());
	pEmitter->SetAttribute("miny", std::to_string(mEmitter.mMin[1]).c_str());
	pEmitter->SetAttribute("minz", std::to_string(mEmitter.mMin[2]).c_str());
	pEmitter->SetAttribute("maxx", std::to_string(mEmitter.mMax[0]).c_str());
	pEmitter->SetAttribute("maxy", std::to_string(mEmitter.mMax[1]).c_str());
	pEmitter->SetAttribute("maxz", std::to_string(mEmitter.mMax[2]).c_str());
	pBaseElement->LinkEndChild(pEmitter);

	tinyxml2::XMLElement* pDirection = doc.NewElement("Direction");
	pDirection->SetAttribute("x", std::to_string(mDirection[0]).c_str());
	pDirection->SetAttribute("y", std::to_string(mDirection[1]).c_str());
	pDirection->SetAttribute("z", std::to_string(mDirection[2]).c_str());
	pBaseElement->LinkEndChild(pDirection);

	tinyxml2::XMLElement* pEmitRate = doc.NewElement("EmitRate");
	pEmitRate->SetAttribute("min", std::to_string(mMinParticlesPerSecond).c_str());
	pEmitRate->SetAttribute("max", std::to_string(mMaxParticlesPerSecond).c_str());
	pBaseElement->LinkEndChild(pEmitRate);

	tinyxml2::XMLElement* pColorMinimum = doc.NewElement("ColorMinimum");
	pColorMinimum->SetAttribute("r", std::to_string(mMinStartColor.GetRed()).c_str());
	pColorMinimum->SetAttribute("g", std::to_string(mMinStartColor.GetGreen()).c_str());
	pColorMinimum->SetAttribute("b", std::to_string(mMinStartColor.GetBlue()).c_str());
	pColorMinimum->SetAttribute("a", std::to_string(mMinStartColor.GetAlpha()).c_str());
	pBaseElement->LinkEndChild(pColorMinimum);

	tinyxml2::XMLElement* pColorMaximum = doc.NewElement("ColorMaximum");
	pColorMaximum->SetAttribute("r", std::to_string(mMaxStartColor.GetRed()).c_str());
	pColorMaximum->SetAttribute("g", std::to_string(mMaxStartColor.GetGreen()).c_str());
	pColorMaximum->SetAttribute("b", std::to_string(mMaxStartColor.GetBlue()).c_str());
	pColorMaximum->SetAttribute("a", std::to_string(mMaxStartColor.GetAlpha()).c_str());
	pBaseElement->LinkEndChild(pColorMaximum);

	tinyxml2::XMLElement* pLifeTime = doc.NewElement("LifeTime");
	pLifeTime->SetAttribute("min", std::to_string(mMinLifeTime).c_str());
	pLifeTime->SetAttribute("max", std::to_string(mMaxLifeTime).c_str());
	pBaseElement->LinkEndChild(pLifeTime);

	tinyxml2::XMLElement* pAngleMaximum = doc.NewElement("AngleMaximum");
	pAngleMaximum->SetAttribute("angle", std::to_string(mMaxAngle).c_str());
	pBaseElement->LinkEndChild(pAngleMaximum);

	tinyxml2::XMLElement* pSizeMinimum = doc.NewElement("SizeMinimum");
	pSizeMinimum->SetAttribute("x", std::to_string(mMinStartSize[0]).c_str());
	pSizeMinimum->SetAttribute("y", std::to_string(mMinStartSize[1]).c_str());
	pBaseElement->LinkEndChild(pSizeMinimum);

	tinyxml2::XMLElement* pSizeMaximum = doc.NewElement("SizeMaximum");
	pSizeMaximum->SetAttribute("x", std::to_string(mMaxStartSize[0]).c_str());
	pSizeMaximum->SetAttribute("y", std::to_string(mMaxStartSize[1]).c_str());
...
std::shared_ptr<Node> SkyRenderComponent::CreateSceneNode(void)
{
	const std::shared_ptr<ScreenElementScene>& pScene = 
		GameApplication::Get()->GetHumanView()->mScene;

	std::shared_ptr<Texture2> skyDome;
	std::shared_ptr<ResHandle>& resHandle = 
		ResCache::Get()->GetHandle(&BaseResource(ToWideString(mTextureResource)));
	if (resHandle)
	{
	...
	// add skydome
	std::shared_ptr<Node> sky = pScene->AddSkyDomeNode(0,
	...
	tinyxml2::XMLDocument doc, tinyxml2::XMLElement *pBaseElement)
{
	tinyxml2::XMLElement* pTexture = doc.NewElement("Texture");
	pTexture->SetAttribute("file", mTextureResource.c_str());
	pTexture->SetAttribute("horiRes", std::to_string(mHoriRes).c_str());
	pTexture->SetAttribute("vertRes", std::to_string(mVertRes).c_str());
	pTexture->SetAttribute("texturePercentage", std::to_string(mTexturePercentage).c_str());
	pTexture->SetAttribute("spherePercentage", std::to_string(mSpherePercentage).c_str());
	pTexture->SetAttribute("radius", std::to_string(mRadius).c_str());
	pBaseElement->LinkEndChild(pTexture);
}
