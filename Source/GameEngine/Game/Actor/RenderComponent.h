...
class MeshRenderComponent : public BaseRenderComponent
{
	...
	std::map<std::string, Vector2<short>> mAnimationFrames;
...
public:
	...
	std::vector<std::string> GetMeshes() { return mMeshes; }
	std::vector<std::string> GetTextures() { return mTextures; }
	std::map<std::string, Vector2<short>> GetAnimationFrames() { return mAnimationFrames; }
...
protected:
    ...
    virtual std::shared_ptr<Node> CreateSceneNode(void) override;  // factory method to create the appropriate scene node
...
// Cloud
//---------------------------------------------------------------------------------------------------------------------
class CloudRenderComponent : public BaseRenderComponent
{
    unsigned int mMaterialType;
    Vector3<float> mSize;
...
class LightRenderComponent : public BaseRenderComponent
{
	...
	Vector3<float> mAnimationCenter;

	Vector2<float> mTextureSize;
	std::string mTextureResource;

	std::shared_ptr<Light> mLightData;
