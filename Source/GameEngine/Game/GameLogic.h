...
{
public:
	virtual std::weak_ptr<Actor> GetActor(const ActorId id) = 0;
	virtual std::shared_ptr<Actor> CreateActor(const std::string &actorResource, 
...
public:
	...
	ActorId GetLastActorID(void) { return mLastActorId; }
	ActorId GetNewActorID(void) { return ++mLastActorId; }
	...
	virtual std::weak_ptr<Actor> GetActor(const ActorId actorId);
...
protected:
	...
	std::shared_ptr<BaseGamePhysic> mPhysics;
};
