...
std::vector<std::wstring> LevelManager::mLevelSearchPaths;
 ...
 */
void LevelManager::AddLevelSearchDir(const std::wstring &dir)
{
 ...
 */
Level* LevelManager::GetLevel(const std::wstring& id) const
{
 ...
 */
void LevelManager::SetUnavailableLevels(const std::vector<std::wstring> &levels)
{
    for(LevelList::const_iterator i = mLevels.begin(); i != mLevels.end(); ++i)
    {
        if(!mLevelAvailables[i-mLevels.begin()]) continue;
        const std::wstring id=(*i)->GetId();
 ...
 */
std::vector<std::wstring> LevelManager::GetAllLevelIds()
{
 ...
 */
void LevelManager::LoadLevelList(const std::wstring& levelname)
{
    mAllLevelDirs.clear();
    ...
    {
        const std::wstring &dir = mLevelSearchPaths[i];
		...
		std::vector<std::wstring> files = ResCache::Get()->Match(dir + levelname);
		std::vector<std::wstring>::iterator itFile = files.begin();
 ...
 */
bool LevelManager::LoadLevel(const std::wstring& levelname)
{
 ...
 */
void LevelManager::RemoveLevel(const std::wstring& id)
{
    ...
    std::vector<Level*>::iterator it = std::find(mLevels.begin(), mLevels.end(), level);
    if (it == mLevels.end())
    {
		wchar_t error[128];
