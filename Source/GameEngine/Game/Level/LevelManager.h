...
public:
    ...
    static void AddLevelSearchDir(const std::wstring &dir);
    /** Returns a list of all level ids. */
    std::vector<std::wstring> GetAllLevelIds();

	/* Level */
	virtual bool LoadLevel(const std::wstring& dirname);
	Level* GetLevel(const std::wstring& id) const;
	void RemoveLevel(const std::wstring &id);

    /** Load all level files from all directories */
    void  LoadLevelList(const std::wstring& levelname);
    void  RemoveAllCachedData();
    // ------------------------------------------------------------------------
    /** Sets a list of levels as being unavailable
     *  \param level List of levels to mark as unavilable. */
    void SetUnavailableLevels(const std::vector<std::wstring> &levels);
    // ------------------------------------------------------------------------
    /** \brief Returns a list of all directories that contain a level. */
    const std::vector<std::wstring>* GetAllLevelDirs() const
    {
	...
	// ------------------------------------------------------------------------
	/** Returns the number of levels. */
	std::vector<Level*> GetLevels() const { return mLevels; }
	// ------------------------------------------------------------------------
...
protected:
	...
	static std::vector<std::wstring> mLevelSearchPaths;

	/** All directories in which levels were found. */
	std::vector<std::wstring> mAllLevelDirs;
	...
	*/
	std::vector<bool> mLevelAvailables;
