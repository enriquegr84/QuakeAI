...
Font::Font(std::shared_ptr<VisualProgram> const& program, 
    ...
    : mMaxMessageLength(maxMessageLength)
{
    ...
    mTextEffect = std::make_shared<TextEffect>(program, mTexture);
}

Vector2<int> Font::GetDimension(std::wstring const& message) const
{
    Vector2<float> lineDimension = Vector2<float>::Zero();
    Vector2<float> textDimension = Vector2<float>::Zero();
	...
	unsigned int const length = (unsigned int)message.length();
	for (unsigned int i = 0; i < length; ++i, ++lineLength)
	{
		// Get character data.
        ...
        if (lineLength >= mMaxMessageLength)
            break;
		...
		if (lineBreak)
		{
			textDimension[1] += th;  // height in pixels
            if (textDimension[0] < lineDimension[0])
                textDimension[0] = lineDimension[0];
            lineDimension[0] = 0;
            lineDimension[1] = 0;
            lineLength = 0;
			continue;
        ...
        lineDimension[0] += (tx1 - tx0)*tw - 1.0f;  // width in pixels
	}
    if (textDimension[0] < lineDimension[0])
        textDimension[0] = lineDimension[0];

	return Vector2<int>{(int)round(textDimension[0]), (int)round(textDimension[1])};
}

Vector2<int> Font::GetDimension(std::wstring const& message, unsigned int threshold) const
{
    ...
    for (; i < length; ++i)
    {
        ...
        if (round(width) >= threshold)
            break;
    }

    return Vector2<int>{(int)round(width), (int)round(height)};
}

unsigned int Font::GetLength(std::wstring const& message, unsigned int threshold) const
{
    float width = 0, height = 0;
...
std::wstring Font::GetText(std::wstring const& message, unsigned int offset, unsigned int dimension) const
{
    std::wstring text;
    float width = 0, height = 0;
    ...
    for (; i < length; ++i)
    {
        ...
        width += (tx1 - tx0)*tw - 1.0f;  // width in pixels
        if (floor(width) <= offset)
            continue;

        if (floor(width) <= dimension + offset)
            text += message[i];
    }
