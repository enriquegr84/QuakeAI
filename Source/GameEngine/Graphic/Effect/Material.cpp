...
bool Material::IsTransparent() const
{
	return	mType == MT_TRANSPARENT ||
			mType == MT_TRANSPARENT_ADD_COLOR ||
			mType == MT_TRANSPARENT_VERTEX_ALPHA ||
			mType == MT_TRANSPARENT_ALPHA_CHANNEL ||
			mType == MT_TRANSPARENT_ALPHA_CHANNEL_REF ||
...
std::shared_ptr<Texture2> Material::GetTexture(unsigned int i) const
{
...
void Material::SetTexture(unsigned int i, std::shared_ptr<Texture2> tex)
{
...
bool Material::Update(std::shared_ptr<BlendState>& blendState) const
{
...
bool Material::Update(std::shared_ptr<RasterizerState>& rasterizerState) const
{
...
bool Material::Update(std::shared_ptr<DepthStencilState>& depthStencilState) const
{
