...
void ImageFilter::ImageCleanTransparent(std::shared_ptr<Texture2> const& src, unsigned int threshold)
{
    ...
    for (unsigned int level = 0; level < numLevels; ++level)
    {
        ...
        auto srcData = reinterpret_cast<uint32_t*>(src->GetDataFor(level));
        for (unsigned int v = 0; v < vSize; ++v)
        {
            for (unsigned int u = 0; u < uSize; ++u)
            {
                ...
                SColor srcColor;
                srcColor.GetData(&srcData[u + uSize * v]);
...
void ImageFilter::ImageScaleNNAA(std::shared_ptr<Texture2> const& src,
    const RectangleShape<2, int>& srcRect, std::shared_ptr<Texture2> const& dest)
{
    ...
    Vector2<int> upperLeftCorner = srcRect.GetVertice(RVP_UPPERLEFT);
    double sox = upperLeftCorner[0];
    double soy = upperLeftCorner[1];
    ...
    for (unsigned int level = 0; level < numLevels; ++level)
    {
        ...
        auto srcData = reinterpret_cast<uint32_t*>(src->GetDataFor(level));
        auto destData = reinterpret_cast<uint32_t*>(dest->GetDataFor(level));
        for (dy = 0; dy < dimHeight; ++dy)
        {
            for (dx = 0; dx < dimWidth; ++dx)
            {
                ...
                minsx = std::clamp(minsx, (double)0, sox + sw);
                maxsx = minsx + sw / dimWidth;
                maxsx = std::clamp(maxsx, (double)0, sox + sw);
                if (minsx > maxsx)
                    std::swap(minsx, maxsx);
                minsy = soy + (dy * sh / dimHeight);
                minsy = std::clamp(minsy, (double)0, soy + sh);
                maxsy = minsy + sh / dimHeight;
                maxsy = std::clamp(maxsy, (double)0, soy + sh);
                if (minsy > maxsy)
                ...
                for (sy = floor(minsy); sy < maxsy; sy++)
                {
                    for (sx = floor(minsx); sx < maxsx; sx++)
                    {
                        ...
                        colorPixel.SetData(&srcData[(unsigned int)sx + dimWidth * (unsigned int)sy]);
                        area += pa;
                    ...
                    if (area > 0) 
                    {
                        colorPixel.SetRed((unsigned int)(ra / area + 0.5));
                        colorPixel.SetGreen((unsigned int)(ga / area + 0.5));
                        colorPixel.SetBlue((unsigned int)(ba / area + 0.5));
                        colorPixel.SetAlpha((unsigned int)(aa / area + 0.5));
                    }
                    else 
                    {
                        colorPixel.SetRed(0);
                    ...
                    colorPixel.GetData(&destData[dx + dimWidth * dy]);
                }
            }
        }
    }
