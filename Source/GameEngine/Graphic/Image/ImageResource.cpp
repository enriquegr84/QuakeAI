...
bool ImageResourceLoader::IsALoadableFileExtension(const std::wstring& fileName) const
{
...
{
	std::shared_ptr<ImageResourceExtraData> pExtraData(new ImageResourceExtraData());
	...
	if (IsALoadableFileExtension(file->GetFileName()))
	{
		...
		{
			handle->SetExtra(std::shared_ptr<ImageResourceExtraData>(pExtraData));
			return true;
		}
	}
...
std::shared_ptr<BaseResourceLoader> CreateImageResourceLoader()
{
    return std::shared_ptr<BaseResourceLoader>(new ImageResourceLoader());
}

std::shared_ptr<Texture2> ImageResourceLoader::Load(std::wstring const& fileName, bool wantMipMaps)
{
	int width, height, components;

	unsigned char *imageData = stbi_load(
		ToString(fileName).c_str(), &width, &height, &components, STBI_rgb_alpha);
	...
	std::shared_ptr<Texture2> texture = 
		std::make_shared<Texture2>(gtformat, width, height, wantMipMaps);
