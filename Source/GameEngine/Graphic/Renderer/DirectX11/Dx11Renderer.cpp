	...
	HMODULE softwareModule, UINT flags, D3D_FEATURE_LEVEL minFeatureLevel)
{
	...
	mInputLayouts = std::make_unique<DX11InputLayoutManager>();
...
bool Dx11Renderer::EnableShaders(std::shared_ptr<VisualEffect> const& effect,
...
void Dx11Renderer::DisableShaders(std::shared_ptr<VisualEffect> const& effect,
	DX11VertexShader* dxVShader, DX11GeometryShader* dxGShader, DX11PixelShader* dxPShader)
{
	if (dxGShader)
	{
...
bool Dx11Renderer::Update(std::shared_ptr<Buffer> const& buffer)
{
	if (!buffer->GetData())
	{
...
bool Dx11Renderer::Update(std::shared_ptr<TextureSingle> const& texture)
{
	if (!texture->GetData())
	{
...
bool Dx11Renderer::Update(std::shared_ptr<TextureSingle> const& texture, unsigned int level)
{
	if (!texture->GetData())
	{
		LogWarning("Texture does not have system memory, creating it.");
...
bool Dx11Renderer::Update(std::shared_ptr<TextureArray> const& textureArray)
{
...
bool Dx11Renderer::CopyCpuToGpu(std::shared_ptr<Buffer> const& buffer)
{
    if (!buffer->GetData())
    {
        LogWarning("Buffer does not have system memory, creating it.");
    ...
    DX11Buffer* dxBuffer = static_cast<DX11Buffer*>(Bind(buffer));
    return dxBuffer->CopyCpuToGpu(mDeviceContext);
}

bool Dx11Renderer::CopyCpuToGpu(std::shared_ptr<TextureSingle> const& texture)
{
    if (!texture->GetData())
    {
        LogWarning("Texture does not have system memory, creating it.");
    ...
    DX11Texture* dxTexture = static_cast<DX11Texture*>(Bind(texture));
    return dxTexture->CopyCpuToGpu(mDeviceContext);
}

bool Dx11Renderer::CopyCpuToGpu(std::shared_ptr<TextureSingle> const& texture, unsigned int level)
{
    if (!texture->GetData())
    {
        LogWarning("Texture does not have system memory, creating it.");
    ...
    DX11Texture* dxTexture = static_cast<DX11Texture*>(Bind(texture));
    unsigned int sri = texture->GetIndex(0, level);
    return dxTexture->CopyCpuToGpu(mDeviceContext, sri);
}

bool Dx11Renderer::CopyCpuToGpu(std::shared_ptr<TextureArray> const& textureArray)
{
    if (!textureArray->GetData())
    {
        LogWarning("Texture array does not have system memory, creating it.");
    ...
    DX11TextureArray* dxTextureArray = static_cast<DX11TextureArray*>(
        Bind(textureArray));
    return dxTextureArray->CopyCpuToGpu(mDeviceContext);
}

bool Dx11Renderer::CopyCpuToGpu(std::shared_ptr<TextureArray> const& textureArray, unsigned int item, unsigned int level)
{
    if (!textureArray->GetData())
    {
        LogWarning("Texture array does not have system memory, creating it.");
    ...
    DX11TextureArray* dxTextureArray = static_cast<DX11TextureArray*>(
        Bind(textureArray));
    unsigned int sri = textureArray->GetIndex(item, level);
    return dxTextureArray->CopyCpuToGpu(mDeviceContext, sri);
}

bool Dx11Renderer::CopyGpuToCpu(std::shared_ptr<Buffer> const& buffer)
{
    if (!buffer->GetData())
    {
        LogWarning("Buffer does not have system memory, creating it.");
    ...
    DX11Buffer* dxBuffer = static_cast<DX11Buffer*>(Bind(buffer));
    return dxBuffer->CopyGpuToCpu(mDeviceContext);
}

bool Dx11Renderer::CopyGpuToCpu(std::shared_ptr<TextureSingle> const& texture)
{
    if (!texture->GetData())
    {
        LogWarning("Texture does not have system memory, creating it.");
    ...
    DX11Texture* dxTexture = static_cast<DX11Texture*>(Bind(texture));
    return dxTexture->CopyGpuToCpu(mDeviceContext);
}

bool Dx11Renderer::CopyGpuToCpu(std::shared_ptr<TextureSingle> const& texture, unsigned int level)
{
    if (!texture->GetData())
    {
        LogWarning("Texture does not have system memory, creating it.");
    ...
    DX11Texture* dxTexture = static_cast<DX11Texture*>(Bind(texture));
    unsigned int sri = texture->GetIndex(0, level);
    return dxTexture->CopyGpuToCpu(mDeviceContext, sri);
}

bool Dx11Renderer::CopyGpuToCpu(std::shared_ptr<TextureArray> const& textureArray)
{
    if (!textureArray->GetData())
    {
        LogWarning("Texture array does not have system memory, creating it.");
    ...
    DX11TextureArray* dxTextureArray = static_cast<DX11TextureArray*>(
        Bind(textureArray));
    return dxTextureArray->CopyGpuToCpu(mDeviceContext);
}

bool Dx11Renderer::CopyGpuToCpu(std::shared_ptr<TextureArray> const& textureArray, unsigned int item, unsigned int level)
{
    if (!textureArray->GetData())
    {
        LogWarning("Texture array does not have system memory, creating it.");
    ...
    DX11TextureArray* dxTextureArray = static_cast<DX11TextureArray*>(
        Bind(textureArray));
    unsigned int sri = textureArray->GetIndex(item, level);
    return dxTextureArray->CopyGpuToCpu(mDeviceContext, sri);
}

void Dx11Renderer::CopyGpuToGpu(
    ...
    std::shared_ptr<Buffer> const& buffer1)
{
    ...
    dxBuffer0->CopyGpuToGpu(mDeviceContext, dxBuffer1->GetDXResource());
}
    ...
    std::shared_ptr<TextureSingle> const& texture1)
{
    ...
    dxTexture0->CopyGpuToGpu(mDeviceContext, dxTexture1->GetDXResource());
}
    ...
    unsigned int level)
{
    ...
    dxTexture0->CopyGpuToGpu(mDeviceContext, dxTexture1->GetDXResource(), sri);
}
    ...
    std::shared_ptr<TextureArray> const& textureArray1)
{
    ...
    return dxTextureArray0->CopyGpuToGpu(mDeviceContext, dxTextureArray1->GetDXResource());
}
    ...
    unsigned int item, unsigned int level)
{
    ...
    return dxTextureArray0->CopyGpuToGpu(mDeviceContext, dxTextureArray1->GetDXResource(), sri);
}

uint64_t Dx11Renderer::DrawPrimitive(std::shared_ptr<VertexBuffer> const& vbuffer,
...
void Dx11Renderer::ClearColorBuffer()
{
	...
	for (unsigned int i = 0; i < mNumActiveRTs; ++i)
	{
		if (rtViews[i])
		{
			mDeviceContext->ClearRenderTargetView(rtViews[i], mClearColor.ToArray().data());
			rtViews[i]->Release();
		}
	}
}
...
void Dx11Renderer::SetBlendState(std::shared_ptr<BlendState> const& state)
{
	if (state)
	{
...
void Dx11Renderer::SetDepthStencilState(std::shared_ptr<DepthStencilState> const& state)
{
	if (state)
	{
...
void Dx11Renderer::SetRasterizerState(std::shared_ptr<RasterizerState> const& state)
{
	if (state)
	{
...
void Dx11Renderer::Enable(std::shared_ptr<DrawTarget> const& target)
{
...
void Dx11Renderer::Disable(std::shared_ptr<DrawTarget> const& target)
{
	DX11DrawTarget* dxTarget = static_cast<DX11DrawTarget*>(Bind(target));
	if (dxTarget)
	{
