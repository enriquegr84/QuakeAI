...
HLSLReflection HLSLShaderFactory::CreateFromFile(std::string const& name,
	std::string const& filepath, std::string const& entry,
...
HLSLReflection HLSLShaderFactory::CreateFromByteCode(std::string const& name,
...
ID3DBlob* HLSLShaderFactory::CompileShader(std::string const& filepath,
	std::string const& entry, std::string const& target, 
	unsigned int compileFlags, ProgramDefines const& defines)
{
	...
	std::string umvName = "GE_USE_MAT_VEC";
	std::string umvValue;
	...
	HRESULT hr = D3DCompileFromFile(ToWideString(filepath).c_str(), 
