    ...
    mMeetsRequirements(false)
{
	...
	mInputLayouts = std::make_unique<GL4InputLayoutManager>();
...
bool GL4Renderer::EnableShaders(std::shared_ptr<VisualEffect> const& effect, GLuint program)
{
...
void GL4Renderer::DisableShaders(std::shared_ptr<VisualEffect> const& effect, GLuint program)
{
    Shader* vshader = effect->GetVertexShader().get();
    Shader* pshader = effect->GetPixelShader().get();
    Shader* gshader = effect->GetGeometryShader().get();
...
void GL4Renderer::EnableSBuffers(Shader const* shader, GLuint program)
{
    ...
    for (unsigned acbIndex = 0; acbIndex < atomicCounterBuffers.size(); ++acbIndex)
    {
        ...
        else
        {
            ...
            mGOMutex.lock();
            mGraphicObjects.insert(std::make_pair(rawBuffer.get(), temp));
            mGOMutex.unlock();
...
{
    glClearColor(mClearColor.GetRed(), mClearColor.GetGreen(), mClearColor.GetBlue(), mClearColor.GetAlpha());
...
void GL4Renderer::SetBlendState(std::shared_ptr<BlendState> const& state)
{
    if (state)
    {
...
void GL4Renderer::SetDepthStencilState(std::shared_ptr<DepthStencilState> const& state)
{
    if (state)
    {
...
void GL4Renderer::SetRasterizerState(std::shared_ptr<RasterizerState> const& state)
{
    if (state)
    {
...
void GL4Renderer::Enable(std::shared_ptr<DrawTarget> const& target)
{
    auto gl4Target = static_cast<GL4DrawTarget*>(Bind(target));
    gl4Target->Enable();
}

void GL4Renderer::Disable(std::shared_ptr<DrawTarget> const& target)
{
...
bool GL4Renderer::Update(std::shared_ptr<Buffer> const& buffer)
{
    if (!buffer->GetData())
    {
...
bool GL4Renderer::Update(std::shared_ptr<TextureSingle> const& texture)
{
    if (!texture->GetData())
    {
...
bool GL4Renderer::Update(std::shared_ptr<TextureSingle> const& texture, unsigned int level)
{
    if (!texture->GetData())
    {
        LogWarning("Texture does not have system memory, creating it.");
...
bool GL4Renderer::Update(std::shared_ptr<TextureArray> const& textureArray)
{
...
bool GL4Renderer::Update(std::shared_ptr<TextureArray> const& textureArray, unsigned int item, unsigned int level)
{
    if (!textureArray->GetData())
    {
        LogWarning("Texture array does not have system memory, creating it.");
...
bool GL4Renderer::CopyCpuToGpu(std::shared_ptr<Buffer> const& buffer)
{
    if (!buffer->GetData())
    {
        LogWarning("Buffer does not have system memory, creating it.");
...
bool GL4Renderer::CopyCpuToGpu(std::shared_ptr<TextureSingle> const& texture)
{
    if (!texture->GetData())
    {
        LogWarning("Texture does not have system memory, creating it.");
...
bool GL4Renderer::CopyCpuToGpu(std::shared_ptr<TextureSingle> const& texture, unsigned int level)
{
    if (!texture->GetData())
    {
        LogWarning("Texture does not have system memory, creating it.");
...
bool GL4Renderer::CopyCpuToGpu(std::shared_ptr<TextureArray> const& textureArray)
{
    if (!textureArray->GetData())
    {
        LogWarning("Texture array does not have system memory, creating it.");
...
bool GL4Renderer::CopyCpuToGpu(std::shared_ptr<TextureArray> const& textureArray, unsigned int item, unsigned int level)
{
    if (!textureArray->GetData())
    {
        LogWarning("Texture array does not have system memory, creating it.");
...
bool GL4Renderer::CopyGpuToCpu(std::shared_ptr<Buffer> const& buffer)
{
    if (!buffer->GetData())
    {
        LogWarning("Buffer does not have system memory, creating it.");
...
bool GL4Renderer::CopyGpuToCpu(std::shared_ptr<TextureSingle> const& texture)
{
    if (!texture->GetData())
    {
        LogWarning("Texture does not have system memory, creating it.");
...
bool GL4Renderer::CopyGpuToCpu(std::shared_ptr<TextureSingle> const& texture, unsigned int level)
{
    if (!texture->GetData())
    {
        LogWarning("Texture does not have system memory, creating it.");
...
bool GL4Renderer::CopyGpuToCpu(std::shared_ptr<TextureArray> const& textureArray)
{
    if (!textureArray->GetData())
    {
        LogWarning("Texture array does not have system memory, creating it.");
...
bool GL4Renderer::CopyGpuToCpu(std::shared_ptr<TextureArray> const& textureArray, unsigned int item, unsigned int level)
{
    if (!textureArray->GetData())
    {
        LogWarning("Texture array does not have system memory, creating it.");
...
uint64_t GL4Renderer::DrawPrimitive(std::shared_ptr<VertexBuffer> const& vbuffer,
