...
GLSLShader::GLSLShader(GLSLReflection const& reflector, GraphicObjectType type, uint32_t glslType)
    : Shader(type)
{
        ...
        for (auto const& block : atomicCounterBuffers)
        {
            if (block.referencedBy[glslType])
            {
                ...
                mData[AtomicCounterBufferShaderDataLookup].push_back(
                    Data(GE_RESOURCE, "atomicCounterBuffer" + std::to_string(blockIndex),
