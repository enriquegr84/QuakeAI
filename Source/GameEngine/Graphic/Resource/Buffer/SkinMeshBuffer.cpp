...
SkinMeshBuffer::SkinMeshBuffer()
	: BaseMeshBuffer(0, 0), mVertexType(VT_STANDARD), mBoundingBoxNeedsRecalculated(true)
{
	mMaterial = std::make_shared<Material>();
}

SkinMeshBuffer::SkinMeshBuffer(VertexFormat const& vformat, 
    uint32_t numVertices, uint32_t numPrimitives, size_t indexSize)
	: BaseMeshBuffer(numPrimitives, 0), mVertexType(VT_STANDARD), mBoundingBoxNeedsRecalculated(true)
{
	mVFormat = vformat;
...
void SkinMeshBuffer::RecalculateBoundingBox()
{
    if (!mBoundingBoxNeedsRecalculated)
        return;

    mBoundingBoxNeedsRecalculated = false;
    if (mVertice->GetNumElements())
    {
        mBoundingBox.Reset(Position(0));
        for (unsigned int i = 1; i < mVertice->GetNumElements(); ++i)
            mBoundingBox.GrowToContain(Position(i));
    }
...
void SkinMeshBuffer::CreateVBuffer(unsigned int numVertices)
{
	...
	vbuffer = std::make_shared<VertexBuffer>(mVFormat, numVertices);
	if (vbuffer)
	{
		//Vertex tangent
		{
...
std::shared_ptr<IndexBuffer> SkinMeshBuffer::CreateIBuffer(unsigned int numTriangles, size_t indexSize)
{
