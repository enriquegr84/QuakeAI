...
AnimatedMeshNode::AnimatedMeshNode(const ActorId actorId, PVWUpdater* updater,
	const std::shared_ptr<BaseAnimatedMesh>& mesh) : Node(actorId, NT_ANIMATED_MESH),
	...
	mJointMode(JUOR_NONE), mJointsUsed(false), mLooping(true), mReadOnlyMaterials(false), 
...
//! Sets a new mesh
void AnimatedMeshNode::SetMesh(const std::shared_ptr<BaseAnimatedMesh>& mesh)
{
	if (!mesh)
	...
	if (dynamic_cast<AnimateMeshMD3*>(mMesh.get()))
	{
		...
		{
			mBoundingBox.GrowToContain(mesh->GetBoundingBox());
	...
	SetFrameLoop(0, (int)mMesh->GetFrameCount());
}

//! Get a mesh
const std::shared_ptr<BaseAnimatedMesh>& AnimatedMeshNode::GetMesh(void)
{
	return mMesh;
}
...
std::shared_ptr<BaseMesh> AnimatedMeshNode::GetMeshForCurrentFrame()
{
...
void AnimatedMeshNode::Render(unsigned int& visual, bool isTransparentPass,
	Scene *pScene, std::vector<Transform> interpolations, std::shared_ptr<MD3Mesh> pMesh)
{
	if (pMesh->IsTagMesh())
	{
	...
	for (unsigned int i = 0; i < pMesh->GetMeshBufferCount(); ++i, ++visual)
	{
		...
		{
			const std::shared_ptr<BaseMeshBuffer>& meshBuffer = pMesh->GetMeshBuffer(i);
			if (meshBuffer)
			{
				...
				if (transparent == isTransparentPass)
				{
					...
					std::vector<Transform>::reverse_iterator it;
	...
	{
		std::shared_ptr<MD3Mesh> pChild = pMesh->GetChildren()[n];
...
bool AnimatedMeshNode::Render(Scene* pScene)
{
	...
	if (dynamic_cast<AnimateMeshMD3*>(mCurrentFrameMesh.get()))
	{
		...
		std::vector<Transform> interpolations{ mWorldTransform };
	...
	else
	{
		...
		{
			const std::shared_ptr<BaseMeshBuffer>& meshBuffer = mCurrentFrameMesh->GetMeshBuffer(i);
			if (meshBuffer)
			{
				std::shared_ptr<Material> material = meshBuffer->GetMaterial();
...
BoundingBox<float>& AnimatedMeshNode::GetBoundingBox()
{
    return mBoundingBox;
}
...
std::shared_ptr<BoneNode> AnimatedMeshNode::GetJointNode(const char* jointName)
{
...
//! Returns a pointer to a child node, which has the same transformation as
//! the corresponding joint, if the mesh in this scene node is a skinned mesh.
std::shared_ptr<BoneNode> AnimatedMeshNode::GetJointNode(unsigned int jointID)
{
	if (!mMesh || mMesh->GetMeshType() != MT_SKINNED)
	{
		LogWarning("No mesh, or mesh not of skinned mesh type");
...
//! Gets joint count.
size_t AnimatedMeshNode::GetJointCount() const
{
...
std::shared_ptr<ShadowVolumeNode> AnimatedMeshNode::AddShadowVolumeNode(const ActorId actorId,
...
int AnimatedMeshNode::DetachChild(std::shared_ptr<Node> const& child)
{
...
std::shared_ptr<Visual> const& AnimatedMeshNode::GetVisual(unsigned int i)
{
	if (i >= mVisuals.size())
		return nullptr;
...
size_t AnimatedMeshNode::GetVisualCount() const
{
...
std::shared_ptr<Material> const& AnimatedMeshNode::GetMaterial(unsigned int i)
{
	if (dynamic_cast<AnimateMeshMD3*>(mMesh.get()))
	{
		AnimateMeshMD3* animMeshMD3 = dynamic_cast<AnimateMeshMD3*>(mMesh.get());
...
size_t AnimatedMeshNode::GetMaterialCount() const
{
	if (dynamic_cast<AnimateMeshMD3*>(mMesh.get()))
	{
		AnimateMeshMD3* animMeshMD3 = dynamic_cast<AnimateMeshMD3*>(mMesh.get());
		...
		unsigned int mbCount = 0;
		for (std::shared_ptr<MD3Mesh> mesh : meshes)
			mbCount += (unsigned int)mesh->GetMeshBufferCount();
		return mbCount;
	}
	else return (unsigned int)mMesh->GetMeshBufferCount();
}

//! Sets the texture of the specified layer in all materials of this scene node to the new texture.
/** \param textureLayer Layer of texture to be set. Must be a value smaller than MATERIAL_MAX_TEXTURES.
\param texture New texture to be used. */
void AnimatedMeshNode::SetMaterialTexture(unsigned int textureLayer, std::shared_ptr<Texture2> texture)
{
