...
public:
	...
	AnimatedMeshNode(const ActorId actorId, PVWUpdater* updater,
		const std::shared_ptr<BaseAnimatedMesh>& mesh);
...
private:
	...
	void Render(unsigned int& visual, bool isTransparentPass, Scene *pScene, 
		std::vector<Transform> interpolations, std::shared_ptr<MD3Mesh> pMesh);
	...
	std::shared_ptr<AnimationEndCallBack> mLoopCallBack;
	int mPassCount;
