...
//! constructor
NodeAnimatorFollowSpline::NodeAnimatorFollowSpline(unsigned int time, const std::vector<Vector3<float>>& points,  
...
void NodeAnimatorFollowSpline::AnimateNode(Scene* pScene, Node* node, unsigned int timeMs)
{
	...
	const unsigned int pSize = (unsigned int)mPoints.size();
	if (pSize==0)
	{
		if ( !mLoop )
			mHasFinished = true;
