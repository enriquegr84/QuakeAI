...
//! constructor
NodeAnimatorTexture::NodeAnimatorTexture(const std::vector<std::shared_ptr<Texture2>>& textures, 
	int timePerFrame, bool loop, unsigned int now) : mFinishTime(0), mHasFinished(false), 
	mTimePerFrame(timePerFrame), mStartTime(now), mLoop(loop)
{
	for (unsigned int i=0; i<textures.size(); ++i)
		mTextures.push_back(textures[i]);

	mFinishTime = now + (timePerFrame * (unsigned int)mTextures.size());
}
...
void NodeAnimatorTexture::AnimateNode(Scene* pScene, Node* node, unsigned int timeMs)
{
	...
	if (mTextures.size())
	{
		...
		{
			idx = (unsigned int)mTextures.size() - 1;
			mHasFinished = true;
		}
		else
		{
