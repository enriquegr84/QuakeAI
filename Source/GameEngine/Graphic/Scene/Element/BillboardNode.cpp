...
BillboardNode::BillboardNode(const ActorId actorId, PVWUpdater* updater, 
	const std::shared_ptr<Texture2>& texture, const Vector2<float>& size)
	: Node(actorId, NT_BILLBOARD)
{
	...
	for (unsigned int i = 0; i < GetMaterialCount(); ++i)
	{
		...
		meshBuffer->GetMaterial()->SetTexture(0, texture);
	}
...
BoundingBox<float>& BillboardNode::GetBoundingBox()
{
    return mBoundingBox;
}
...
void BillboardNode::SetColor(const SColorF& overallColor)
{
    mMeshBuffer->GetMaterial()->mDiffuse = overallColor.ToArray();
}
...
std::shared_ptr<Visual> const& BillboardNode::GetVisual(unsigned int i)
{
	return mVisual;
}

//! return amount of visuals of this scene node.
size_t BillboardNode::GetVisualCount() const
{
	return 1;
}
...
std::shared_ptr<Material> const& BillboardNode::GetMaterial(unsigned int i)
{
	return mMeshBuffer->GetMaterial();
}

//! returns amount of materials used by this scene node.
size_t BillboardNode::GetMaterialCount() const
{
	return 1;
}
...
void BillboardNode::SetMaterialTexture(unsigned int textureLayer, std::shared_ptr<Texture2> texture)
{
