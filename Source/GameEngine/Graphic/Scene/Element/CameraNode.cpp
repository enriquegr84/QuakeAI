...
#endif
{
	...
	mCamera->SetFrustum(72.0f, aspectRatio, 0.1f, 3000.0f);
...
void CameraNode::UpdateMatrices()
{
	...
	Vector4<float> up;
#if defined(GE_USE_MAT_VEC)
	up = rotation.GetCol(1);
#else
	up = rotation.GetRow(1);
#endif
...
BoundingBox<float>& CameraNode::GetBoundingBox()
{
    return mBoundingBox;
}
