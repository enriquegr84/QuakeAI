...
//! constructor
LightNode::LightNode(const ActorId actorId, PVWUpdater* updater, const std::shared_ptr<Light>& light)
...
BoundingBox<float>& LightNode::GetBoundingBox()
{
    return mBoundingBox;
}
...
std::shared_ptr<Light> const& LightNode::GetLight() const 
{ 
	return mLight; 
}
...
void LightNode::DoLightRecalc()
{
	...
	GetAbsoluteTransform().GetRotation().Transformation(lightDirection);
...
std::shared_ptr<Material> const& LightNode::GetMaterial(unsigned int i)
{
	return mMaterial;
}

//! returns amount of materials used by this scene node.
size_t LightNode::GetMaterialCount() const
{
	return 1;
}
...
void LightNode::SetMaterialTexture(unsigned int textureLayer, std::shared_ptr<Texture2> texture)
{
