...
void ParticleAttractionAffector::Affect(unsigned int now, Particle* particlearray, unsigned int count)
{
	...
	{
		Vector3<float> direction(mPoint - particlearray[i].mPosition);
		Normalize(direction);
		...
		if( mAffectX )
			particlearray[i].mPosition[0] += direction[0];

		if( mAffectY )
			particlearray[i].mPosition[1] += direction[1];

		if( mAffectZ )
			particlearray[i].mPosition[2] += direction[2];
	}
}