...
int ParticleBoxEmitter::Emitt(unsigned int now, unsigned int timeSinceLastCall, Particle*& outArray)
{
	...
	if (mTime > everyWhatMillisecond)
	{
		...
		{
			particle.mPosition[0] = mBox.mMin[0] + Randomizer::FRand() * extent[0];
			particle.mPosition[1] = mBox.mMin[1] + Randomizer::FRand() * extent[1];
			particle.mPosition[2] = mBox.mMin[2] + Randomizer::FRand() * extent[2];
