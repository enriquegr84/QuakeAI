...
ParticleMeshEmitter::ParticleMeshEmitter(
	const std::shared_ptr<BaseMesh>& mesh, bool useNormalDirection,
	...
	if(mTime > everyWhatMillisecond)
	{
		...
		for(unsigned int i=0; i<amount; ++i)
		{
			if( mEveryMeshVertex )
			{
				...
				{
					const std::shared_ptr<BaseMeshBuffer>& meshBuffer = mParticleMesh->GetMeshBuffer(j);
					for( unsigned int k=0; k<meshBuffer->GetVertice()->GetNumElements(); ++k )
					{
						particle.mPosition = meshBuffer->Position(k);
			...
			else
			{
				...
				const std::shared_ptr<BaseMeshBuffer>& meshBuffer = mParticleMesh->GetMeshBuffer(randomMB);
				if (!meshBuffer->GetVertice()->GetNumElements())
					continue;
				
				unsigned int vertexNumber = Randomizer::Rand() % meshBuffer->GetVertice()->GetNumElements();

				particle.mPosition = meshBuffer->Position(vertexNumber);
				if( mUseNormalDirection )
...
void ParticleMeshEmitter::SetMesh(const std::shared_ptr<BaseMesh>& mesh)
{
	mParticleMesh = mesh;
	...
	mMBCount = (unsigned int)mParticleMesh->GetMeshBufferCount();
	for( unsigned int i = 0; i < mMBCount; ++i )
	{
		const std::shared_ptr<BaseMeshBuffer>& meshBuffer = mParticleMesh->GetMeshBuffer(i);
