...
//! constructor
ParticleRingEmitter::ParticleRingEmitter(const Vector3<float>& center, float radius, float ringThickness, 
	...
	if(mTime > everyWhatMillisecond)
	{
		...
		for(unsigned int i=0; i<amount; ++i)
		{
			...
			Quaternion<float> tgt = Rotation<3, float>(
				AxisAngle<3, float>(-Vector3<float>::Unit(1), Randomizer::FRand() * 360));
