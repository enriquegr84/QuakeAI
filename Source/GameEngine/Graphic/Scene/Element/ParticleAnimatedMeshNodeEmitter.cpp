...
ParticleAnimatedMeshNodeEmitter::ParticleAnimatedMeshNodeEmitter(const ActorId actorId, 
		PVWUpdater* updater, const std::shared_ptr<AnimatedMeshNode>& node, bool useNormalDirection, 
...
int ParticleAnimatedMeshNodeEmitter::Emitt(unsigned int now, unsigned int timeSinceLastCall, Particle*& outArray)
{
	...
	if(mTime > everyWhatMillisecond)
	{
		...
		const std::shared_ptr<BaseMesh>& frameMesh = 
		...
		for(unsigned int i=0; i<amount; ++i)
		{
			if( mEveryMeshVertex )
			{
				for(unsigned int j=0; j<frameMesh->GetMeshBufferCount(); ++j )
				{
					...
					{
						particle.mPosition = frameMesh->GetMeshBuffer(j)->Position(k);
			...
			else
			{
				...
				particle.mPosition = frameMesh->GetMeshBuffer(randomMB)->Position(vertexNumber);
				if( mUseNormalDirection )
				...
				if( mMaxAngleDegrees )
				{
					Quaternion<float> tgt = Rotation<3, float>(
						AxisAngle<3, float>(Vector3<float>::Unit(2), Randomizer::FRand() * mMaxAngleDegrees * (float)GE_C_DEG_TO_RAD));
					mDirection = HProject(Rotate(tgt, HLift(mDirection, 0.f)));
					tgt = Rotation<3, float>(
						AxisAngle<3, float>(Vector3<float>::Unit(0), Randomizer::FRand() * mMaxAngleDegrees * (float)GE_C_DEG_TO_RAD));
					mDirection = HProject(Rotate(tgt, HLift(mDirection, 0.f)));
					tgt = Rotation<3, float>(
...
void ParticleAnimatedMeshNodeEmitter::SetAnimatedMeshNode( const std::shared_ptr<AnimatedMeshNode>& node )
{
	mAnimatedNode = node;
	mAnimatedMesh = 0;
	mBaseMesh = 0;
	...
	mMBCount = (unsigned int)mBaseMesh->GetMeshBufferCount();
...
BoundingBox<float>& ParticleAnimatedMeshNodeEmitter::GetBoundingBox()
{
    return mBoundingBox;
}