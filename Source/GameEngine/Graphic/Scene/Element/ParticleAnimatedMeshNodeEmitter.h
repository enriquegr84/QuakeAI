...
public:
	...
	ParticleAnimatedMeshNodeEmitter(
		...
		const std::shared_ptr<AnimatedMeshNode>& node,
		...
		const SColorF& minStartColor = SColorF(), 
		const SColorF& maxStartColor = SColorF(1.f, 1.f, 1.f, 1.f),
	...
	void SetAnimatedMeshNode( const std::shared_ptr<AnimatedMeshNode>& node );
	...
	const std::shared_ptr<AnimatedMeshNode>& GetAnimatedMeshNode() const { return mAnimatedNode; }
...
private:
	...
	std::shared_ptr<AnimatedMeshNode>		mAnimatedNode;
	std::shared_ptr<BaseAnimatedMesh>		mAnimatedMesh;
	std::shared_ptr<BaseMesh>				mBaseMesh;
	int						mTotalVertices;
	unsigned int			mMBCount;
	int						mMBNumber;
	std::vector<int>		mVertexPerMeshBufferList;
