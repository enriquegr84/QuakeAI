	...
	mMaxParticles(0xffff), mParticlesAreGlobal(true)
{
	...
	if (createDefaultEmitter)
		SetEmitter(std::shared_ptr<ParticleBoxEmitter>(CreateBoxEmitter()));
}
...
bool ParticleSystemNode::PreRender(Scene *pScene)
{
	if (IsVisible())
	{
		UpdateParticles(Timer::GetTime());

		// reallocate arrays, if they are too small
		ReallocateParticleBuffers();

		UpdateParticleBuffers(pScene);
...
BoundingBox<float>& ParticleSystemNode::GetBoundingBox()
{
    return mMeshBuffer->GetBoundingBox();
}

void ParticleSystemNode::UpdateParticles(unsigned int time)
{
	if (mLastEmitTime==0)
	{
		mLastEmitTime = time;
	...
	if (mEmitter)
	{
		...
		if (newParticles && array)
		{
			...
			for (int i = j; i<j + newParticles; ++i)
			{
				...
				Vector4<float> startVector = HLift(mParticles[i].mStartVector, 0.f);
				GetAbsoluteTransform().GetRotation().Transformation(startVector);
				mParticles[i].mStartVector = HProject(startVector);
				if (mParticlesAreGlobal)
				{
					Vector4<float> positionVector = HLift(mParticles[i].mPosition, 0.f);
					GetAbsoluteTransform().GetRotation().Transformation(positionVector);
                    positionVector += GetAbsoluteTransform().GetTranslationW0();
					mParticles[i].mPosition = HProject(positionVector);
				}
			}
		}
	}
	...
	// run affectors
	std::list<std::shared_ptr<BaseParticleAffector>>::iterator ait = mAffectorList.begin();
	for (; ait != mAffectorList.end(); ++ait)
		(*ait)->Affect(now, mParticles.data(), (unsigned int)mParticles.size());

    BoundingBox<float> bBox;
    if (mParticlesAreGlobal)
        bBox.Reset(GetAbsoluteTransform().GetTranslation());
    else
        bBox.Reset(Vector3<float>::Zero());
	...
	for (unsigned int i=0; i<mParticles.size();)
	{
		...
		{
			mParticles[i].mPosition += (mParticles[i].mVector * scale);
            bBox.GrowToContain(mParticles[i].mPosition);
			++i;
		}
	}

    const float m = (mParticleSize[0] > mParticleSize[1] ? mParticleSize[0] : mParticleSize[1]) * 0.5f;
    bBox.mMaxEdge += Vector3<float>{m, m, m};
    bBox.mMinEdge -= Vector3<float>{m, m, m};

    if (mParticlesAreGlobal)
        bBox.Transformation(GetAbsoluteTransform().Inverse());

	mMeshBuffer->GetBoundingBox() = bBox;
}

void ParticleSystemNode::ReallocateParticleBuffers()
{
		...
		mParticles.size() * 2 != mMeshBuffer->GetIndice()->GetNumPrimitives())
	{
		MeshBuffer* meshBuffer = new MeshBuffer(mMeshBuffer->GetVertice()->GetFormat(),
			(unsigned int)mParticles.size() * 4, (unsigned int)mParticles.size() * 2, sizeof(unsigned int));
		for (unsigned int i = 0; i < GetMaterialCount(); ++i)
			meshBuffer->GetMaterial() = GetMaterial(i);
		mMeshBuffer.reset(meshBuffer);
...
void ParticleSystemNode::UpdateParticleBuffers(Scene *pScene)
{
	const std::shared_ptr<CameraNode>& cameraNode = pScene->GetActiveCamera();
	...
	if (mParticles.size() != 0)
	{
		...
		for (unsigned int i = 0; i < mParticles.size(); ++i)
		{
			...
			mMeshBuffer->Position(0 + idx) = particle.mPosition + HProject(horizontal + vertical);
			mMeshBuffer->Color(0, 0 + idx) = particle.mColor.ToArray();
			mMeshBuffer->Position(1 + idx) = particle.mPosition + HProject(horizontal - vertical);
			mMeshBuffer->Color(0, 1 + idx) = particle.mColor.ToArray();
			mMeshBuffer->Position(2 + idx) = particle.mPosition + HProject(-horizontal - vertical);
			mMeshBuffer->Color(0, 2 + idx) = particle.mColor.ToArray();
			mMeshBuffer->Position(3 + idx) = particle.mPosition + HProject(-horizontal + vertical);
...
std::shared_ptr<BaseParticleEmitter> ParticleSystemNode::GetEmitter()
{
	return mEmitter;
}

//! Sets the particle emitter, which creates the particles.
void ParticleSystemNode::SetEmitter(const std::shared_ptr<BaseParticleEmitter>& emitter)
{
	mEmitter = emitter;
}

//! Adds new particle effector to the particle system.
void ParticleSystemNode::AddAffector(const std::shared_ptr<BaseParticleAffector>& affector)
{
	mAffectorList.push_back(affector);
}

//! Get a list of all particle affectors.
const std::list<std::shared_ptr<BaseParticleAffector>>& ParticleSystemNode::GetAffectors() const
{
...
{
	std::list<std::shared_ptr<BaseParticleAffector>>::iterator it = mAffectorList.begin();
...
std::shared_ptr<Visual> const& ParticleSystemNode::GetVisual(unsigned int i)
{
	return mVisual;
}

//! return amount of visuals of this scene node.
size_t ParticleSystemNode::GetVisualCount() const
{
	return 1;
}
...
std::shared_ptr<Material> const& ParticleSystemNode::GetMaterial(unsigned int i)
{
	return mMeshBuffer->GetMaterial();
}

//! returns amount of materials used by this scene node.
size_t ParticleSystemNode::GetMaterialCount() const
{
	return 1;
}
...
void ParticleSystemNode::SetMaterialTexture(unsigned int textureLayer, std::shared_ptr<Texture2> texture)
{
...
ParticleAnimatedMeshNodeEmitter* ParticleSystemNode::CreateAnimatedMeshNodeEmitter(const ActorId actorId,
	const std::shared_ptr<AnimatedMeshNode>& node, bool useNormalDirection, const Vector3<float>& direction, 
...
ParticleMeshEmitter* ParticleSystemNode::CreateMeshEmitter(const std::shared_ptr<BaseMesh>& mesh, 
	bool useNormalDirection, const Vector3<float>& direction, float normalDirectionModifier, int mbNumber, bool everyMeshVertex, 
	unsigned int minParticlesPerSecond, unsigned int maxParticlesPerSecond, const SColorF& minStartColor, const SColorF& maxStartColor,
...
ParticlePointEmitter* ParticleSystemNode::CreatePointEmitter(const Vector3<float>& direction, 
	unsigned int minParticlesPerSecond, unsigned int maxParticlesPerSecond, 
	const SColorF& minStartColor, const SColorF& maxStartColor, 
	unsigned int lifeTimeMin, unsigned int lifeTimeMax, int maxAngleDegrees, 
	const Vector2<float>& minStartSize, const Vector2<float>& maxStartSize )
{
...
ParticleSphereEmitter* ParticleSystemNode::CreateSphereEmitter(const Vector3<float>& center, float radius, 
	const Vector3<float>& direction, unsigned int minParticlesPerSecond, unsigned int maxParticlesPerSecond, 
	const SColorF& minStartColor, const SColorF& maxStartColor, unsigned int lifeTimeMin, unsigned int lifeTimeMax, 
	int maxAngleDegrees, const Vector2<float>& minStartSize, const Vector2<float>& maxStartSize )
{
