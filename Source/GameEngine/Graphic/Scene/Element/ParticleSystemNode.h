...
public:
	...
	std::shared_ptr<BaseParticleEmitter> GetEmitter();

	//! Sets the particle emitter, which creates the particles.
	void SetEmitter(const std::shared_ptr<BaseParticleEmitter>& emitter);

	//! Adds new particle affector to the particle system.
	void AddAffector(const std::shared_ptr<BaseParticleAffector>& affector);

	//! Get a list of all particle affectors.
	const std::list<std::shared_ptr<BaseParticleAffector>>& GetAffectors() const;
	...
	void UpdateParticles(unsigned int time);

	//! Returns type of the scene node
	virtual NodeType GetType() const { return NT_PARTICLE; }
	...
	ParticleAnimatedMeshNodeEmitter* CreateAnimatedMeshNodeEmitter(
		const ActorId actorId, const std::shared_ptr<AnimatedMeshNode>& node, 
		...
		const SColorF& minStartColor = SColorF(), const SColorF& maxStartColor = SColorF(1.f, 1.f, 1.f, 1.f),
	...
	ParticleMeshEmitter* CreateMeshEmitter(
		const std::shared_ptr<BaseMesh>& mesh, bool useNormalDirection = true,
		const Vector3<float>& direction = Vector3<float>{ 0.f, 0.03f, 0.f },
		float normalDirectionModifier = 100.0f, int mbNumber = -1, bool everyMeshVertex = false,
		unsigned int minParticlesPerSecond = 5, unsigned int maxParticlesPerSecond = 10,
		const SColorF& minStartColor = SColorF(), const SColorF& maxStartColor = SColorF(1.f, 1.f, 1.f, 1.f),
...
private:
	...
	void ReallocateParticleBuffers();
	void UpdateParticleBuffers(Scene *pScene);
	...
	std::list<std::shared_ptr<BaseParticleAffector>> mAffectorList;
	std::shared_ptr<BaseParticleEmitter> mEmitter;
