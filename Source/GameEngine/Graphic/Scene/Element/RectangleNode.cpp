...
//! constructor
RectangleNode::RectangleNode(const ActorId actorId, PVWUpdater* updater, const std::shared_ptr<Texture2>& texture,
	float texxScale, float texyScale, float xSize, float ySize, int xPolyCount, int yPolyCount)
:	Node(actorId, NT_BOX), mShadow(0), mSizeX(xSize), mSizeY(ySize), mPolyCountX(xPolyCount), mPolyCountY(yPolyCount)
{
	mPVWUpdater = updater;
...
BoundingBox<float>& RectangleNode::GetBoundingBox()
{
    return mBoundingBox;
}
...
int RectangleNode::DetachChild(std::shared_ptr<Node> const& child)
{
...
std::shared_ptr<ShadowVolumeNode> RectangleNode::AddShadowVolumeNode(const ActorId actorId,
...
std::shared_ptr<Visual> const& RectangleNode::GetVisual(unsigned int i)
{
	return mVisual;
}

//! return amount of visuals of this scene node.
size_t RectangleNode::GetVisualCount() const
{
	return 1;
}
...
std::shared_ptr<Material> const& RectangleNode::GetMaterial(unsigned int i)
{
	return mMaterial;
}

//! returns amount of materials used by this scene node.
size_t RectangleNode::GetMaterialCount() const
{
	return 1;
}
...
void RectangleNode::SetMaterialTexture(unsigned int textureLayer, std::shared_ptr<Texture2> texture)
{
