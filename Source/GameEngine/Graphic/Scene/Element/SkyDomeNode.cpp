...
//! constructor
SkyDomeNode::SkyDomeNode(const ActorId actorId, PVWUpdater* updater, const std::shared_ptr<Texture2>& sky, 
	...
	mTexturePercentage(texturePercentage), mSpherePercentage(spherePercentage), mRadius(radius)
{
	...
	mMeshBuffer = std::make_shared<MeshBuffer>(vformat, 
    ...
    BoundingBox<float> bBox = mMeshBuffer->GetBoundingBox();
    bBox.Reset(0,0,0);
	...
	mVisual = std::make_shared<Visual>(mMeshBuffer->GetVertice(), mMeshBuffer->GetIndice(), effect);
...
void SkyDomeNode::GenerateMesh(const std::shared_ptr<Texture2>& sky)
{
	float azimuth = 0.f;
	...
	const float onepixel = sky ? (1.0f / (sky->GetWidth() * 1.5f)) : 0.0f;
	...
	for (unsigned int k = 0; k <= mHorizontalResolution; ++k)
	{
		...
		for (unsigned int j = 0; j <= mVerticalResolution; ++j)
		{
			...
			vertice.position = Vector3<float>{ 
				cosEr*sinA, mRadius*Function<float>::Sin(elevation), cosEr* cosA };
...
{
	const std::shared_ptr<CameraNode>& camera = pScene->GetActiveCamera();
...
BoundingBox<float>& SkyDomeNode::GetBoundingBox()
{
...
std::shared_ptr<Visual> const& SkyDomeNode::GetVisual(unsigned int i)
{
	return mVisual;
}

//! return amount of visuals of this scene node.
size_t SkyDomeNode::GetVisualCount() const
{
	return 1;
}
...
std::shared_ptr<Material> const& SkyDomeNode::GetMaterial(unsigned int i)
{
	return mMeshBuffer->GetMaterial();
}

//! returns amount of materials used by this scene node.
size_t SkyDomeNode::GetMaterialCount() const
{
	return 1;
}
...
void SkyDomeNode::SetMaterialTexture(unsigned int textureLayer, std::shared_ptr<Texture2> texture)
{
