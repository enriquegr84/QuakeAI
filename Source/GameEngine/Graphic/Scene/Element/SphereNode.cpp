...
SphereNode::SphereNode(const ActorId actorId, PVWUpdater* updater, 
	const std::shared_ptr<Texture2>& texture, float radius, unsigned int polyCountX, unsigned int polyCountY)
:	Node(actorId, NT_SPHERE), mShadow(0), mRadius(radius), mPolyCountX(polyCountX), mPolyCountY(polyCountY)
{
	mPVWUpdater = updater;
...
BoundingBox<float>& SphereNode::GetBoundingBox()
{
    return mBoundingBox;
}
...
int SphereNode::DetachChild(std::shared_ptr<Node> const& child)
{
...
std::shared_ptr<ShadowVolumeNode> SphereNode::AddShadowVolumeNode(const ActorId actorId,
...
std::shared_ptr<Visual> const& SphereNode::GetVisual(unsigned int i)
{
	return mVisual;
}

//! return amount of visuals of this scene node.
size_t SphereNode::GetVisualCount() const
{
	return 1;
}
...
std::shared_ptr<Material> const& SphereNode::GetMaterial(unsigned int i)
{
	return mMaterial;
}

//! returns amount of materials used by this scene node.
size_t SphereNode::GetMaterialCount() const
{
	return 1;
}
...
void SphereNode::SetMaterialTexture(unsigned int textureLayer, std::shared_ptr<Texture2> texture)
{
