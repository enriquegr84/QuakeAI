...
void VolumeLightNode::ConstructLight()
{
	...
	for (unsigned int i = 0; i < GetMaterialCount(); ++i)
	{
		...
		mMeshBuffer->GetMaterial()->SetTexture(0, mTexture);
	}
...
BoundingBox<float>& VolumeLightNode::GetBoundingBox()
{
    return mBoundingBox;
}
...
std::shared_ptr<Material> const& VolumeLightNode::GetMaterial(unsigned int i)
{
	return mMeshBuffer->GetMaterial();
}

//! returns amount of materials used by this scene node.
size_t VolumeLightNode::GetMaterialCount() const
{
	return 1;
}
...
void VolumeLightNode::SetMaterialTexture(unsigned int textureLayer, std::shared_ptr<Texture2> texture)
{
