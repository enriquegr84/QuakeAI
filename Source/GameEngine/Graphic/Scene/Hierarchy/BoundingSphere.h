...
public:
    ...
    // Check intersection of linear component and bound (points of
    ...
    bool Intersect(Vector4<float> const& origin,
        Vector4<float> const& direction, float tmin, float tmax) const;

    // Check intersection of the two stationary spheres.
    bool Intersect(BoundingSphere const& sphere) const;

    // Check intersection of the two moving spheres.  The velocity0 is
    // for 'this' and the velocity1 is the for the input bound.
    bool Intersect(BoundingSphere const& sphere, float tmax,
