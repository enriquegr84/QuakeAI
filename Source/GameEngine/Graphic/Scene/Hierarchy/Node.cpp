...
int Node::AttachChild(std::shared_ptr<Node> const& child)
{
    if (!child)
    {
...
int Node::DetachChild(std::shared_ptr<Node> const& child)
{
    if (child)
    {
        int i = 0;
...
std::shared_ptr<Node> Node::DetachChildAt(int i)
{
...
int Node::AttachParent(std::shared_ptr<Node> const& parent)
{
    LogAssert(!GetParent(), "This node already has a parent.");

    return parent->AttachChild(shared_from_this());
}

int Node::DetachParent()
{
    if (GetParent())
    {
        Node* parent = dynamic_cast<Node*>(GetParent());
        return parent->DetachChild(shared_from_this());
    }
...
std::shared_ptr<Node> Node::SetChild(int i, std::shared_ptr<Node> const& child)
{
    if (child)
    {
    ...
    if (0 <= i && i < numChildren)
    {
		...
		std::shared_ptr<Node> previousChild = mChildren[i];
        if (previousChild)
        {
...
int Node::AttachAnimator(std::shared_ptr<NodeAnimator> const& animator)
{
	if (!animator)
	{
...
int Node::DetachAnimator(std::shared_ptr<NodeAnimator> const& animator)
{
	if (animator)
	{
		int i = 0;
...
std::shared_ptr<NodeAnimator> Node::DetachAnimatorAt(int i)
{
...
std::shared_ptr<NodeAnimator> Node::SetAnimator(int i, std::shared_ptr<NodeAnimator> const& animator)
{
	int const numAnimators = static_cast<int>(mAnimators.size());
	if (0 <= i && i < numAnimators)
	{
		// Remove the animator currently in the slot.
		std::shared_ptr<NodeAnimator> previousAnimator = mAnimators[i];
...
std::shared_ptr<NodeAnimator> Node::GetAnimator(int i)
{
	if (0 <= i && i < static_cast<int>(mAnimators.size()))
	{
		return mAnimators[i];
...
void Node::GetVisibleSet(Culler& culler, std::shared_ptr<Camera> const& camera, bool noCull)
{
    for (auto& child : mChildren)
    {
        if (child)
...
bool Node::OnAnimate(Scene* pScene, unsigned int timeMs)
{
	if (IsVisible())
	{
		...
		while (ait != mAnimators.end())
		{
			...
			const std::shared_ptr<NodeAnimator>& anim = *ait;
			++ait;
...
void Node::SortRenderList(Scene* pScene)
{
	...
	if (!pScene->GetLightManager())
	{
		...
		std::vector<DistanceNodeEntry> SortedLights;
	...
	//SOLID NODES
	{
		...
		std::vector<DefaultNodeEntry> SortedSolids;
	...
	//TRANSPARENT NODES
	{
		...
		std::vector<TransparentNodeEntry> SortedTransparent;
