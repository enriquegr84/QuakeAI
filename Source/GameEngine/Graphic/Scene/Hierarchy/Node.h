...
enum GRAPHIC_ITEM RenderPass
{
	...
	//! PlayerCamera pass. The active view is set up here. The very first pass.
	RP_CAMERA,
...
typedef std::vector<std::shared_ptr<Node>> SceneNodeList;
typedef std::vector<std::shared_ptr<NodeAnimator>> SceneNodeAnimatorList;

typedef std::vector<Node*> SceneNodeRenderList;
typedef std::vector<NodeAnimator*> SceneNodeAnimateList;


// This class represents grouping nodes in a spatial hiearchy.
class GRAPHIC_ITEM Node : public Spatial, public std::enable_shared_from_this<Node>
{
public:
    ...
    int AttachParent(std::shared_ptr<Node> const& parent);

    // Detach parent from this node.
    int DetachParent();
	...
	int AttachAnimator(std::shared_ptr<NodeAnimator> const& animator);

	int DetachAnimator(std::shared_ptr<NodeAnimator> const& animator);

	std::shared_ptr<NodeAnimator> DetachAnimatorAt(int i);

	// Detach all animators from this node.
	void DetachAllAnimators();

	std::shared_ptr<NodeAnimator> SetAnimator(int i, std::shared_ptr<NodeAnimator> const& animator);

	std::shared_ptr<NodeAnimator> GetAnimator(int i);
	...
	virtual std::shared_ptr<Visual> const& GetVisual(unsigned int i){ return nullptr; }

	//! Get amount of visuals used by this scene node.
	/** \return Current amount of visuals of this scene node. */
	virtual size_t GetVisualCount() const { return 0; }
	...
	virtual std::shared_ptr<Material> const& GetMaterial(unsigned int i) { return nullptr; }

	//! Get amount of materials used by this scene node.
	/** \return Current amount of materials of this scene node. */
	virtual size_t GetMaterialCount() const { return 0; }
