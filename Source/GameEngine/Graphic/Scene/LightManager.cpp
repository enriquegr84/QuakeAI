...
{ 
#if defined(_OPENGL_)
	mLight = std::make_shared<Light>(true, false);
#else
	mLight = std::make_shared<Light>(true, true);
#endif
	mLight->mLighting = std::make_shared<Lighting>();
	mLight->mLighting->mAmbient = Renderer::Get()->GetClearColor().ToArray();
...
void LightManager::UpdateCameraLightModelPositions(Node* node, CameraNode* cameraNode)
{
	if (node->IsVisible())
	{
		...
		{
			std::shared_ptr<Visual> visual = node->GetVisual(i);

			if (visual)
			{
				std::shared_ptr<LightingEffect> ltEffect = 
					std::dynamic_pointer_cast<LightingEffect>(visual->GetEffect());
				if (ltEffect)
				{
					ltEffect->SetLighting(mLight->mLighting);
					ltEffect->UpdateLightingConstant();

					ltEffect->SetGeometry(std::make_shared<LightCameraGeometry>());
					...
					for (unsigned int i = 0; i < mSceneLightList->size(); i++)
					{
						...
						std::shared_ptr<Lighting> lighting = ltEffect->GetLighting();
...
void LightManager::OnPreRender(std::vector<Node*> & lightList)
{
...
void LightManager::OnNodePreRender(Node* node)
{
	...
	if (LIGHTS_NEAREST_NODE == mMode)
	{
		...
		std::vector<LightDistanceElement> sortingArray(mSceneLightList->size());
