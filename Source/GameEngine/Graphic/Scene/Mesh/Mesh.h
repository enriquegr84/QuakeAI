...
public:
	...
	mesh buffer. */
	virtual std::shared_ptr<BaseMeshBuffer> GetMeshBuffer(unsigned int nr) const = 0;

	//! Get pointer to a mesh buffer which fits a material
	/** \param material: material to search for
	\return Pointer to the mesh buffer or 0 if there is no such
	mesh buffer. */
	virtual std::shared_ptr<BaseMeshBuffer> GetMeshBuffer(const Material &material) const = 0;
