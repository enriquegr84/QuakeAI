...
#include "Graphic/Scene/Mesh/MeshMD3.h"
#include "Graphic/Scene/Mesh/SkinnedMesh.h"
#include "Graphic/Scene/Mesh/StaticMesh.h"
...
std::shared_ptr<BaseResourceLoader> CreateMeshResourceLoader()
{
	return std::shared_ptr<MeshFileLoader>(new MeshFileLoader());
}

//
// MeshFileLoader::LoadResource				- Chapter 14, page 492
//
bool MeshFileLoader::LoadResource(void *rawBuffer, unsigned int rawSize, const std::shared_ptr<ResHandle>& handle)
{
	std::shared_ptr<MeshResourceExtraData> pExtraData(new MeshResourceExtraData());
	...
	if (IsALoadableFileExtension(file->GetFileName()))
	{
		...
		{
			handle->SetExtra(std::shared_ptr<MeshResourceExtraData>(pExtraData));
			return true;
		}
	}
...
bool MeshFileLoader::IsALoadableFileExtension(const std::wstring& fileName) const
{
	if (fileName.find('.') != std::string::npos)
	{
		std::wstring fileExtension = fileName.substr(fileName.find('.') + 1);
		return fileExtension.compare(L"3d") == 0 || fileExtension.compare(L"3ds") == 0 ||
			...
			fileExtension.find(L"mesh") != std::string::npos || fileExtension.compare(L"mot") == 0 ||
	...
	bool textureMipmaps, TextureType textureType, aiString* textureName)
{
	...
	std::string texture = textureName->data;
	if (texture.substr(0, 1) == "*")
	{
		...
		if (embeddedTexture->mHeight == 0)
		{
			...
			std::shared_ptr<Texture2> meshTexture =
				std::make_shared<Texture2>(gtformat, width, height, textureMipmaps);
			meshTexture->SetName(ToWideString(texture));
		...
		else
		{
			// Create the 2D texture and compute the stride and image size.
			DFType gtformat = DF_R8G8B8A8_UNORM;
			std::shared_ptr<Texture2> meshTexture = std::make_shared<Texture2>(
	...
	{
		int width, height, components;
		std::wstring directory = FileSystem::Get()->GetWorkingDirectory();
		std::string texturePath = textureName->C_Str();
		if (FileSystem::Get()->GetFileDirectory(ToWideString(textureName->C_Str())) == L".")
			texturePath = ToString(directory) + "/" + texturePath;
...
//-------------------------------------------------------------------------------
bool HasAlphaPixels(std::shared_ptr<Texture2> piTexture)
{
	for (unsigned int y = 0; y < piTexture->GetHeight(); ++y)
	{
		for (unsigned int x = 0; x < piTexture->GetWidth(); ++x)
		{
			char* tx = piTexture->GetData() + y * piTexture->GetWidth() + x;
...
void CreateMaterial(const aiScene* pScene, const aiMesh* pMesh, BaseMeshBuffer* meshBuffer, bool generateMipmaps)
{
	...
	if (pMesh->mTextureCoords[0])
	{
		...
		else
		{
			...
			std::shared_ptr<Texture2> diffuseTexture = meshBuffer->GetMaterial()->GetTexture(TT_DIFFUSE);
		...
		/*
		std::shared_ptr<Texture2> normalTexture = meshBuffer->GetMaterial()->GetTexture(TT_NORMALS);
		if (normalTexture)
		{
...
void ReadNodeMesh(const aiScene* pScene,
	const aiNode* pNode, BaseMesh* mesh, std::wstring fileExtension)
{
	if (fileExtension == L"md3")
	{
		AnimateMeshMD3* animMeshMD3 = dynamic_cast<AnimateMeshMD3*>(mesh);

		std::shared_ptr<MD3Mesh> meshMD3;
		if (pScene->mRootNode == pNode)
		{
			meshMD3 = std::make_shared<MD3Mesh>(pNode->mName.C_Str());
				...
				pNode->mMetaData->Get("numanimations", numAnimations) == true)
			{
				...
				if (pScene->HasMaterials())
				{
					for (unsigned int m = 0; m < pNode->mNumMeshes; m++)
					{
						...
						{
							std::shared_ptr<BaseMeshBuffer> meshBuffer = meshMD3->GetMeshBuffer(mb);
	...
	else
	{
		for (unsigned int m = 0; m < pNode->mNumMeshes; m++)
		{
			...
			for (unsigned int ch = 1; ch < pScene->mMeshes[*meshIndex]->GetNumUVChannels(); ch++)
				if (pScene->mMeshes[*meshIndex]->HasTextureCoords(ch))
					vformat.Bind(VA_TEXCOORD, DF_R32G32_FLOAT, ch);
			vformat.Bind(VA_COLOR, DF_R32G32B32A32_FLOAT, 0);
			for (unsigned int ch = 1; ch < pScene->mMeshes[*meshIndex]->GetNumColorChannels(); ch++)
			...
			if (pScene->mMeshes[*meshIndex]->HasBones())
			{
				...
				for (unsigned int b = 0; b < pScene->mMeshes[*meshIndex]->mNumBones; b++)
				{
					...
					joint->mAttachedMeshes.push_back((unsigned int)mesh->GetMeshBufferCount());
					for (unsigned int w = 0; w < bone->mNumWeights; w++)
					{
						BaseSkinnedMesh::Weight weight;
						weight.mBufferId = (unsigned int)mesh->GetMeshBufferCount();
...
// -------------------------------------------------------------------------------
std::shared_ptr<BaseMesh> MeshFileLoader::CreateMesh(BaseReadFile* file)
{
	//save current working directory and restoring it when finished
	std::wstring saveDir = FileSystem::Get()->GetWorkingDirectory();
	...
	const aiScene* pScene = importer.ReadFile(
		ToString(file->GetFileName()).c_str(),
	...
	FileSystem::Get()->ChangeWorkingDirectoryTo(
		FileSystem::Get()->GetFileDirectory(file->GetFileName()));
	...
	if (fileExtension == L"pk3")
	{
		...
		{
			const std::shared_ptr<BaseMeshBuffer>& meshBuffer = mesh->GetMeshBuffer(i);
			if (meshBuffer)
			{
    ...
    else if (mesh->GetMeshType() == MT_NORMAL)
    {
        ...
        normalMesh->RecalculateBoundingBox();
    }

	FileSystem::Get()->ChangeWorkingDirectoryTo(saveDir);
	return std::shared_ptr<BaseMesh>(mesh);
}