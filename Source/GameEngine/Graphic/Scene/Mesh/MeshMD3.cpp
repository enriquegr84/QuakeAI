...
size_t MD3Mesh::GetMeshBufferCount() const
{
	return mBufferInterpol.size();
}

//! returns pointer to a mesh buffer
std::shared_ptr<BaseMeshBuffer> MD3Mesh::GetMeshBuffer(unsigned int nr) const
{
...
std::shared_ptr<BaseMeshBuffer> MD3Mesh::GetMeshBuffer(const Material &material) const
{
...
{
	std::shared_ptr<MeshBuffer> buffer((MeshBuffer*)meshBuffer);
	mBufferInterpol.push_back(buffer);
}

//! create a MeshBuffer for a MD3 MeshBuffer
std::shared_ptr<MeshBuffer> MD3Mesh::CreateMeshBuffer(const std::shared_ptr<MD3MeshBuffer>& source)
{
	...
	std::shared_ptr<MeshBuffer> dest = std::make_shared<MeshBuffer>(vformat, 
...
BoundingBox<float>& MD3Mesh::GetBoundingBox()
{
    return mBoundingBox;
}

int MD3Mesh::AttachChild(std::shared_ptr<MD3Mesh> const& child)
{
	if (!child)
	{
...
int MD3Mesh::DetachChild(std::shared_ptr<MD3Mesh> const& child)
{
	if (child)
	{
		int i = 0;
...
std::shared_ptr<MD3Mesh> MD3Mesh::DetachChildAt(int i)
{
...
std::shared_ptr<MD3Mesh> MD3Mesh::GetTagMesh(std::string tagName)
{
	if (mTagInterpol.mName.find(tagName) != std::string::npos)
		return shared_from_this();

	for (unsigned int n = 0; n < mChildren.size(); n++)
	{
		std::shared_ptr<MD3Mesh> const mesh = mChildren[n]->GetTagMesh(tagName);
		if (mesh) return mesh;
	}
...
size_t MD3Mesh::GetAnimationCount()
{
	return mAnimations.size();
}

std::shared_ptr<MD3Mesh> MD3Mesh::GetRootMesh()
{
	if (mParent == nullptr)
		return shared_from_this();
...
std::shared_ptr<MD3Mesh> MD3Mesh::GetMesh(std::string meshName)
{
	if (mName == meshName)
		return shared_from_this();
	...
	for (unsigned int n = 0; n < mChildren.size(); n++)
	{
		std::shared_ptr<MD3Mesh> const mesh = mChildren[n]->GetMesh(meshName);
		if (mesh) return mesh;
	}
...
void MD3Mesh::GetMeshes(std::vector<std::shared_ptr<MD3Mesh>>& meshes)
{
...
std::shared_ptr<MD3Mesh> MD3Mesh::CreateMesh(std::string parentName, std::string name)
{
	std::shared_ptr<MD3Mesh> rootMesh = GetRootMesh();
	std::shared_ptr<MD3Mesh> mesh = rootMesh->GetMesh(name);
	if (mesh) return mesh;

	if (!parentName.empty())
	{
		std::shared_ptr<MD3Mesh> meshParent = GetMesh(parentName);
		std::shared_ptr<MD3Mesh> meshChild(new MD3Mesh(name));
...
bool MD3Mesh::UpdateMesh(int frame, int detailLevel, int startFrameLoop, int endFrameLoop)
{
	...
	startFrameLoop = std::max(0, startFrameLoop >> mInterPolShift);
	...
	else
	{
		...
		frameB = std::min(frameA + 1, endFrameLoop);
	}
	...
	unsigned int frameA, unsigned int frameB, float interpolate)
{
	...
	mBufferInterpol[meshId]->RecalculateBoundingBox();
}
...
void MD3Mesh::BuildTagArray(unsigned int frameA, unsigned int frameB, float interpolate)
{
	...
	{
		std::shared_ptr<MD3Mesh> mesh = GetChildren()[i];
		if (mesh->IsTagMesh())
		{
...
void MD3Mesh::RecalculateBoundingBox()
{
    mBoundingBox.Reset(0, 0, 0);

    if (mBufferInterpol.empty())
        return;

    mBoundingBox = mBufferInterpol[0]->GetBoundingBox();
    for (unsigned int i = 1; i < mBufferInterpol.size(); ++i)
        mBoundingBox.GrowToContain(mBufferInterpol[i]->GetBoundingBox());
}

/*!
load model
*/
bool MD3Mesh::LoadModel(std::wstring& path)
{
	...
	{
		std::shared_ptr<MD3Mesh> meshMD3 = rootMesh->GetTagMesh(tags[i].mName);
		if (!meshMD3)
		{
			size_t prefixBegin = 0, prefixEnd = 0;
			prefixEnd = mName.find('$', prefixBegin);
			std::string tagName = tags[i].mName;
	...
	for (unsigned int i = 0; i != header.numMeshes; ++i)
	{
		...
		std::shared_ptr<MD3MeshBuffer> buf(new MD3MeshBuffer());
		...
		//! prepare memory
		std::vector<MD3Vertex> vertices = std::vector<MD3Vertex>(meshHeader.numVertices * meshHeader.numFrames);
		buf->mTexCoords = std::vector<MD3TexCoord>(meshHeader.numVertices);
		buf->mFaces = std::vector<MD3Face>(meshHeader.numTriangles);
		...
		for (unsigned int a = 0; a < meshHeader.numTriangles; a++)
		{
			...
			std::swap(face.index[0], face.index[2]);
		}

		//! prepare memory
		buf->mNormals = std::vector<Vector3<float>>(meshHeader.numVertices * meshHeader.numFrames);
		buf->mPositions = std::vector<Vector3<float>>(meshHeader.numVertices * meshHeader.numFrames);
...
//! returns amount of mesh buffers.
size_t AnimateMeshMD3::GetMeshBufferCount() const
{
	return mRootMesh->GetMeshBufferCount();
}

//! returns pointer to a mesh buffer
std::shared_ptr<BaseMeshBuffer> AnimateMeshMD3::GetMeshBuffer(unsigned int nr) const
{
	return mRootMesh->GetMeshBuffer(nr);
}

//! Returns pointer to a mesh buffer which fits a material
std::shared_ptr<BaseMeshBuffer> AnimateMeshMD3::GetMeshBuffer(const Material &material) const
{
	std::shared_ptr<BaseMeshBuffer> meshBuffer = mRootMesh->GetMeshBuffer(material);
	if (meshBuffer)
		return meshBuffer;
...
std::shared_ptr<BaseMesh> AnimateMeshMD3::GetMesh(
...
size_t AnimateMeshMD3::GetFrameCount() const
{
	return 1;
}
