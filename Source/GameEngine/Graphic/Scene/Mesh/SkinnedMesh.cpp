...
size_t SkinnedMesh::GetFrameCount() const
{
...
std::shared_ptr<BaseMesh> SkinnedMesh::GetMesh(int frame, int detailLevel, int startFrameLoop, int endFrameLoop)
{
	...
	Quaternion<float> &rotation, int &rotationHint)
{
	...
	if (joint->mUseAnimationFrom)
	{
		const std::vector<PositionKey> &positionKeys=joint->mUseAnimationFrom->mPositionKeys;
		const std::vector<ScaleKey> &scaleKeys=joint->mUseAnimationFrom->mScaleKeys;
		const std::vector<RotationKey> &rotationKeys=joint->mUseAnimationFrom->mRotationKeys;
...
void SkinnedMesh::SkinJoint(Joint *joint, Joint *parentJoint)
{
	if (joint->mWeights.size())
	{
		...
		Transform jointTransform = 
			joint->mGlobalAnimatedTransform * joint->mGlobalInversedTransform;
		...
		for (unsigned int i=0; i<joint->mWeights.size(); ++i)
		{
            ...
            mSkinningBuffers[weight.mBufferId]->BoundingBoxNeedsRecalculated();
		}
	}
...
//! Gets joint count.
size_t SkinnedMesh::GetJointCount() const
{
...
size_t SkinnedMesh::GetMeshBufferCount() const
{
...
std::shared_ptr<BaseMeshBuffer> SkinnedMesh::GetMeshBuffer(unsigned int nr) const
{
...
std::shared_ptr<BaseMeshBuffer> SkinnedMesh::GetMeshBuffer(const Material &material) const
{
...
BoundingBox<float>& SkinnedMesh::GetBoundingBox()
{
    return mBoundingBox;
}
...
std::vector<std::shared_ptr<SkinMeshBuffer>> &SkinnedMesh::GetMeshBuffers()
{
	return mLocalBuffers;
}


std::vector<SkinnedMesh::Joint*> &SkinnedMesh::GetAllJoints()
{
	return mAllJoints;
}


const std::vector<SkinnedMesh::Joint*> &SkinnedMesh::GetAllJoints() const
{
	return mAllJoints;
}
	...
	if (mHasAnimation && !mPreparedForSkinning)
	{
		...
		for (i=0; i<mAllJoints.size(); ++i)
		{
			...
			for (j=0; j<joint->mWeights.size(); ++j)
			{
				...
				joint->mWeights[j].mStaticPos = { position[0], position[1], position[2] };
				joint->mWeights[j].mStaticNormal = { normal[0], normal[1], normal[2] };
...
void SkinnedMesh::Finalize()
{
	...
	for (i=0; i<mLocalBuffers.size(); ++i)
		mVerticesMoved.push_back(std::map<int, bool>());
	...
	if (mHasAnimation)
	{
		...
		for(i=0;i<mAllJoints.size();++i)
		{
			std::vector<PositionKey> &positionKeys = mAllJoints[i]->mPositionKeys;
			std::vector<ScaleKey> &scaleKeys = mAllJoints[i]->mScaleKeys;
			std::vector<RotationKey> &rotationKeys = mAllJoints[i]->mRotationKeys;
			...
			if (positionKeys.size())
			{
				...
				if (key->mFrame!=0)
				{
					positionKeys.insert(positionKeys.begin(), *key);
					key=&positionKeys[0];//getFirst
					key->mFrame=0;
				}
			...
			if (scaleKeys.size())
			{
				...
				if (key->mFrame!=0)
				{
					scaleKeys.insert(scaleKeys.begin(), *key);
					key =&scaleKeys[0];//getFirst
					key->mFrame=0;
				}
			...
			if (rotationKeys.size())
			{
				...
				if (key->mFrame!=0)
				{
					rotationKeys.insert(rotationKeys.begin(), *key);
    ...
    {
        BoundingBox<float> bb(mLocalBuffers[0]->GetBoundingBox());
        bb.Transformation(mLocalBuffers[0]->GetTransform());

        mBoundingBox.Reset(bb);
        for (unsigned int j = 1; j < mLocalBuffers.size(); ++j)
        {
            bb = mLocalBuffers[j]->GetBoundingBox();
            bb.Transformation(mLocalBuffers[j]->GetTransform());

            mBoundingBox.GrowToContain(bb);
        }
    }
...
void SkinnedMesh::UpdateBoundingBox(void)
{
    ...
    {
        for (unsigned int j = 0; j < mSkinningBuffers.size(); ++j)
        {
            mSkinningBuffers[j]->RecalculateBoundingBox();
            BoundingBox<float> bb = mSkinningBuffers[j]->GetBoundingBox();
            bb.Transformation(mSkinningBuffers[j]->GetTransform());
...
void SkinnedMesh::AddMeshBuffer(BaseMeshBuffer* meshBuffer)
{
	...
	mLocalBuffers.push_back(
		std::shared_ptr<SkinMeshBuffer>(mSkinningBuffers.back()));
}
...
void SkinnedMesh::RecoverJointsFromMesh(std::vector<std::shared_ptr<BoneNode>> &jointChildSceneNodes)
{
	for (unsigned int i=0; i<mAllJoints.size(); ++i)
	{
		std::shared_ptr<BoneNode> node=jointChildSceneNodes[i];
		Joint *joint=mAllJoints[i];
...
void SkinnedMesh::TransferJointsToMesh(const std::vector<std::shared_ptr<BoneNode>> &jointChildSceneNodes)
{
	for (unsigned int i=0; i<mAllJoints.size(); ++i)
	{
		std::shared_ptr<BoneNode> const node=jointChildSceneNodes[i];
		Joint *joint=mAllJoints[i];
...
void SkinnedMesh::TransferOnlyJointsHintsToMesh(const std::vector<std::shared_ptr<BoneNode>> &jointChildSceneNodes)
{
	for (unsigned int i=0; i<mAllJoints.size(); ++i)
	{
		const std::shared_ptr<BoneNode> node=jointChildSceneNodes[i];
		Joint *joint=mAllJoints[i];
...
void SkinnedMesh::AddJoints(std::vector<std::shared_ptr<BoneNode>>& jointChildSceneNodes, Node* node, PVWUpdater* updater)
{
	//Create new joints
	for (unsigned int i=0; i<mAllJoints.size(); ++i)
	{
		jointChildSceneNodes.push_back(std::make_shared<BoneNode>(
			INVALID_ACTOR_ID, updater, i, mAllJoints[i]->mName.c_str()));
	}
	...
	for (unsigned int i=0; i<jointChildSceneNodes.size(); ++i)
	{
		...
		std::shared_ptr<BoneNode> bone=jointChildSceneNodes[i];
