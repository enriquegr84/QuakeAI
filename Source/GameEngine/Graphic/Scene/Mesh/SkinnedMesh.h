...
public:
	...
	struct Joint
	{
		...
		std::vector<unsigned int> mAttachedMeshes;
	...
	virtual std::vector<std::shared_ptr<SkinMeshBuffer>>& GetMeshBuffers() = 0;

	//! exposed for loaders: joints list
	virtual std::vector<Joint*>& GetAllJoints() = 0;

	//! exposed for loaders: joints list
	virtual const std::vector<Joint*>& GetAllJoints() const = 0;
...
public:
	...
	virtual std::shared_ptr<BaseMesh> GetMesh(int frame, int detailLevel = 255,
	...
	//! Gets joint count.
	virtual size_t GetJointCount() const;
	...
	//! exposed for loaders to add mesh buffers
	virtual std::vector<std::shared_ptr<SkinMeshBuffer>> &GetMeshBuffers();

	//! alternative method for adding joints
	virtual std::vector<Joint*> &GetAllJoints();

	//! alternative method for adding joints
	virtual const std::vector<Joint*> &GetAllJoints() const;
	...
	void RecoverJointsFromMesh(std::vector<std::shared_ptr<BoneNode>> &jointChildSceneNodes);

	//! Tranfers the joint data to the mesh
	void TransferJointsToMesh(const std::vector<std::shared_ptr<BoneNode>> &jointChildSceneNodes);

	//! Tranfers the joint hints to the mesh
	void TransferOnlyJointsHintsToMesh(const std::vector<std::shared_ptr<BoneNode>> &jointChildSceneNodes);

	//! Creates an array of joints from this mesh as children of node
	void AddJoints(std::vector<std::shared_ptr<BoneNode>> &jointChildSceneNodes, Node* node, PVWUpdater* updater);
...
private:
	...
	std::vector<SkinMeshBuffer*> mSkinningBuffers;
	std::vector<std::shared_ptr<SkinMeshBuffer>> mLocalBuffers;

	std::vector<Joint*> mAllJoints;
	std::vector<Joint*> mRootJoints;

	std::vector<std::map<int, bool>> mVerticesMoved;
