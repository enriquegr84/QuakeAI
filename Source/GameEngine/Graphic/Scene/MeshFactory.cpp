	...
	unsigned int numZSamples, float xExtent, float zExtent)
{
	...
	MeshDescription desc(MeshTopology::RECTANGLE, numXSamples, numZSamples);
	...
	rectangle.mAxis[1] = { 0.0f, 0.0f, 1.0f };
	rectangle.mExtent = { xExtent, 0, zExtent };
	RectangleMesh<float> mesh(desc, rectangle);

	return std::make_shared<Visual>(vbuffer, ibuffer);
}

std::shared_ptr<Visual> MeshFactory::CreateTriangle(unsigned int numSamples, float xExtent, float zExtent)
{
    ...
    Vector3<float> pos;
    Vector3<float> nor{ 0.0f, 1.0f, 0.0f };
    Vector3<float> tan{ 1.0f, 0.0f, 0.0f };
    Vector3<float> bin{ 0.0f, 0.0f, 1.0f };  // = Cross(nor,tan)
    Vector2<float> tcd;
    pos[2] = 0.0f;
