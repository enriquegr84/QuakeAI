...
// is centered at (0,0,0) and has an up-axis of (0,1,0).  The other axes
// forming the coordinate system are (1,0,0) and (0,0,1).
//
...
public:
	...
	// The rectangle is in the plane y = 0 and is visible to an observer who
	// is on the side of the plane to which the normal (0,1,0) points.  It has
	// corners (-xExtent, 0, -zExtent), (+xExtent, 0, -zExtent),
	// (-xExtent, 0, +zExtent), and (+xExtent, 0, +zExtent).  The mesh has
	// numXSamples vertices in the x-direction and numZSamples vertices in the
	// z-direction for a total of numXSamples*numYSamples vertices.
	std::shared_ptr<Visual> CreateRectangle(unsigned int numXSamples,
		unsigned int numZSamples, float xExtent, float zExtent);

    // The triangle is in the plane y = 0 and is visible to an observer who is
    // on the side of the plane to which the normal (0,1,0) points.  It has
    // vertices (0, 0, 0), (xExtent, 0, 0), and (0, 0, zExtent).  The mesh
    // has numSamples vertices along each of the x- and z-axes for a total
	...
	// is at (0,radius,0) and the south pole is at (0,-radius,0). The mesh has
