...
#include "Element/MeshNode.h"
#include "Element/BillboardNode.h"
#include "Element/AnimatedMeshNode.h"
...
Scene::Scene()
	...
	mBufferUpdater([this](std::shared_ptr<Buffer> const& buffer) { Renderer::Get()->Update(buffer); })
{
...
void Scene::AddToRenderQueue(RenderPass pass, const std::shared_ptr<Node>& node)
{
	if (!node)
		return;
...
std::shared_ptr<BaseDummyTransformationNode> Scene::AddDummyTransformationNode(const std::shared_ptr<Node>& parent, int id)
{
    std::shared_ptr<BaseDummyTransformationNode> node(new DummyTransformationNode(id, &mPVWUpdater));
...
std::shared_ptr<Node> Scene::AddEmptyNode(const std::shared_ptr<Node>& parent, int id)
{
    std::shared_ptr<Node> node(new EmptyNode(id, &mPVWUpdater));
...
std::shared_ptr<MeshNode> Scene::AddMeshNode(const std::shared_ptr<Node>& parent,
	std::shared_ptr<BaseMesh> mesh, int id, bool alsoAddIfMeshPointerZero)
{
	if (!alsoAddIfMeshPointerZero && !mesh)
		return nullptr;

	std::shared_ptr<MeshNode> meshNode(new MeshNode(id, &mPVWUpdater, mesh));

	if (!parent)
		AddSceneNode(id, meshNode);
	else
		parent->AttachChild(meshNode);
...
//! the returned pointer must not be dropped.
std::shared_ptr<Node> Scene::AddRectangleNode(const std::shared_ptr<Node>& parent,
	const std::shared_ptr<Texture2>& texture, float texxScale, float texyScale, 
	float xSize, float ySize, int xPolyCount, int yPolyCount, int id)
{
	std::shared_ptr<Node> node(new RectangleNode(
...
//! the returned pointer must not be dropped.
std::shared_ptr<Node> Scene::AddBoxNode(const std::shared_ptr<Node>& parent, 
	const std::shared_ptr<Texture2>& texture, Vector2<float> texScale, Vector3<float> size, int id)
{
	std::shared_ptr<Node> node(new BoxNode(id, &mPVWUpdater, texture, texScale, size));
	if (!parent) 
		AddSceneNode(id, node);
	else 
		parent->AttachChild(node);
...
//! adds a cloud scene node to the scene graph.
//! the returned pointer must not be dropped.
std::shared_ptr<Node> Scene::AddCloudSystemNode(const std::shared_ptr<Node>& parent, int id)
{
    std::shared_ptr<Node> node(new CloudSystemNode(id, &mPVWUpdater));
    if (!parent)
        AddSceneNode(id, node);
    else
        parent->AttachChild(node);
...
std::shared_ptr<Node> Scene::AddSphereNode(const std::shared_ptr<Node>& parent, 
	const std::shared_ptr<Texture2>& texture, float radius, int polyCount, int id)
{
...
std::shared_ptr<CameraNode> Scene::AddCameraNode(int id, bool makeActive)
{
	std::shared_ptr<CameraNode> node(new CameraNode(id));
	AddSceneNode(id, node);
...
std::shared_ptr<BillboardNode> Scene::AddBillboardNode(const std::shared_ptr<Node>& parent,
	const std::shared_ptr<Texture2>& texture, const Vector2<float>& size, int id)
{
...
std::shared_ptr<Node> Scene::AddParticleSystemNode(
	const std::shared_ptr<Node>& parent, int id, bool withDefaultEmitter)
{
	std::shared_ptr<Node> node(
...
std::shared_ptr<Node> Scene::AddSkyDomeNode(const std::shared_ptr<Node>& parent,
	const std::shared_ptr<Texture2>& sky, unsigned int horiRes, unsigned int vertRes,
	float texturePercentage, float spherePercentage, float radius, int id)
{

	std::shared_ptr<Node> node(new SkyDomeNode(id, &mPVWUpdater, 
...
//! the returned pointer must not be dropped.
std::shared_ptr<Node> Scene::AddStaticMeshNode(const std::shared_ptr<Node>& parent, 
	std::shared_ptr<BaseMesh> mesh, int id, bool alsoAddIfMeshPointerZero)
{
	if (!alsoAddIfMeshPointerZero && !mesh)
		return nullptr;

	std::shared_ptr<Node> node(new StaticMeshNode(id, &mPVWUpdater, mesh));
...
std::shared_ptr<AnimatedMeshNode> Scene::AddAnimatedMeshNode(const std::shared_ptr<Node>& parent,
	std::shared_ptr<BaseAnimatedMesh> mesh, int id, bool alsoAddIfMeshPointerZero)
{
	if (!alsoAddIfMeshPointerZero && !mesh)
		return nullptr;

	std::shared_ptr<AnimatedMeshNode> node(
...
std::shared_ptr<Node> Scene::AddLightNode(const std::shared_ptr<Node>& parent, 
	const std::shared_ptr<Light>& light, int id)
{
	std::shared_ptr<Node> node(new LightNode(id, &mPVWUpdater, light));
...
std::shared_ptr<NodeAnimator> Scene::CreateFlyCircleAnimator(const Vector3<float>& center,
	float radius, float speed, const Vector3<float>& direction, float startPosition, float radiusEllipsoid)
{
	...
	return std::shared_ptr<NodeAnimator>(
...
std::shared_ptr<NodeAnimator> Scene::CreateFlyStraightAnimator(const Vector3<float>& startPoint,
...
std::shared_ptr<NodeAnimator> Scene::CreateTextureAnimator(
	const std::vector<std::shared_ptr<Texture2>>& textures, int timePerFrame, bool loop)
{
	return std::shared_ptr<NodeAnimator>(
...
std::shared_ptr<NodeAnimator> Scene::CreateDeleteAnimator(unsigned int when)
{
	return std::shared_ptr<NodeAnimator>(new NodeAnimatorDelete(Timer::GetTime() + when));
}

//! Creates a follow spline animator.
std::shared_ptr<NodeAnimator> Scene::CreateFollowSplineAnimator(int startTime,
	const std::vector<Vector3<float>>& points, float speed, float tightness, bool loop, bool pingpong)
{
	return std::shared_ptr<NodeAnimator>(
...
bool Scene::AddSceneNode(ActorId id, const std::shared_ptr<Node>& node)
{ 
	...
	std::shared_ptr<Light> pLight = std::dynamic_pointer_cast<Light>(node);
...
bool Scene::RemoveSceneNode(ActorId id)
{
	...
	std::shared_ptr<Node> node = GetSceneNode(id);
	if (!node)
		return false;
...
std::shared_ptr<Node> Scene::GetSceneNode(ActorId id)
{
...
std::shared_ptr<Node> Scene::GetSceneNode(const Vector4<float>& start, 
	const Vector4<float>& direction, const std::shared_ptr<Node>& root)
{
	std::shared_ptr<Node> best = 0;
	float dist = FLT_MAX;

	GetPickedNode((root == 0) ? mRoot : root, start, direction, dist, best);
	return best;
}

//! recursive method for going through all scene nodes
void Scene::GetPickedNode(const std::shared_ptr<Node>& node, 
	const Vector4<float>& start, const Vector4<float>& direction,
	float& outbestdistance, std::shared_ptr<Node>& outbestnode)
{
	const SceneNodeList& children = node->GetChildren();

	SceneNodeList::const_iterator it = children.begin();
	for (; it != children.end(); ++it)
	{
		std::shared_ptr<Node> current = *it;

		if (current->IsVisible() && current->GetType() == NT_ANIMATED_MESH)
		{
			if (current->GetAbsoulteBound().Intersect(
				start, direction, 0.0f, std::numeric_limits<float>::max()))
			{
				// use sphere bound center to calculate distance with ray
				const float toIntersectionSq = 
					LengthSq(current->GetAbsoulteBound().GetCenter() - start);
			...
			GetPickedNode(current, start, direction, outbestdistance, outbestnode);
		}
	}
}

void Scene::SetActiveCamera(const std::shared_ptr<CameraNode>& camera)
{
	mCamera = camera;
...
{
    std::shared_ptr<EventDataNewRenderComponent> pCastEventData = 
		std::static_pointer_cast<EventDataNewRenderComponent>(pEventData);

    ActorId actorId = pCastEventData->GetActorId();
    std::shared_ptr<Node> pSceneNode(pCastEventData->GetSceneNode());

    // FUTURE WORK: Add better error handling here.	
    if (!pSceneNode || FAILED(pSceneNode->OnRestore(this)))
    {
        LogError(std::string("Failed to restore scene node to the scene for actorid ") + std::to_string(actorId));
        return;
    }
}
...
{
    std::shared_ptr<EventDataModifiedRenderComponent> pCastEventData = 
		std::static_pointer_cast<EventDataModifiedRenderComponent>(pEventData);
	...
	std::shared_ptr<Node> pSceneNode = GetSceneNode(actorId);
    // FUTURE WORK: Add better error handling here.		
...
{
    std::shared_ptr<EventDataDestroyActor> pCastEventData = 
		std::static_pointer_cast<EventDataDestroyActor>(pEventData);
...
void Scene::SyncActorDelegate(BaseEventDataPtr pEventData)
{
	...
	std::shared_ptr<Node> pNode = GetSceneNode(actorId);
	if (pNode)
	{
		std::shared_ptr<Actor> pGameActor(GameLogic::Get()->GetActor(actorId).lock());
