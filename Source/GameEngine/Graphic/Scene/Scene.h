...
typedef std::map<ActorId, std::shared_ptr<Node> > SceneNodeActorMap;
typedef std::vector<Node*> SceneNodeRenderList;
...
public:
    ...
    virtual std::shared_ptr<BaseDummyTransformationNode> AddDummyTransformationNode(
        const std::shared_ptr<Node>& parent, int id = -1);
    ...
    std::shared_ptr<Node> AddEmptyNode(const std::shared_ptr<Node>& parent, int id = -1);

	//! Adds an mesh scene node to the scene graph.
	/** Can be used for doing advanced transformations
	or structuring the scene graph.
	\return Pointer to the created scene node. */
	std::shared_ptr<MeshNode> AddMeshNode( 
		const std::shared_ptr<Node>& parent, std::shared_ptr<BaseMesh> mesh,
	...
	std::shared_ptr<Node> AddRectangleNode(
		const std::shared_ptr<Node>& parent, const std::shared_ptr<Texture2>& texture, 
	...
	pointer should not be dropped. See IReferenceCounted::drop()
	for more information. */
	std::shared_ptr<Node> AddBoxNode(const std::shared_ptr<Node>& parent, 
		const std::shared_ptr<Texture2>& texture, Vector2<float> texScale, 
		Vector3<float> size = { 10.0f, 10.0f, 10.0f }, int id = -1);


    //! Adds a cloud scene node
    /**\param parent: Parent of the scene node. Can be 0 if no parent.
    \param id: Id of the node. This id can be used to identify the scene node.
    \return Pointer to the created test scene node. This
    pointer should not be dropped. See IReferenceCounted::drop()
    for more information. */
    std::shared_ptr<Node> AddCloudSystemNode(const std::shared_ptr<Node>& parent, int id = -1);
	...
	pointer should not be dropped. See IReferenceCounted::drop()
	for more information. */
	std::shared_ptr<Node> AddSphereNode(
		const std::shared_ptr<Node>& parent, const std::shared_ptr<Texture2>& texture, 
	...
	This pointer should not be dropped. See IReferenceCounted::drop() for more information. */
	std::shared_ptr<Node> AddVolumeLightNode(const std::shared_ptr<Node>& parent,
	...
	This pointer should not be dropped. See IReferenceCounted::drop() for more information. */
	std::shared_ptr<CameraNode> AddCameraNode(int id = -1, bool makeActive = true);
	...
	std::shared_ptr<BillboardNode> AddBillboardNode(
		const std::shared_ptr<Node>& parent, const std::shared_ptr<Texture2>& texture,
	...
	\return Pointer to the created scene node.
	This pointer should not be dropped. See IReferenceCounted::drop() for more information. */
	std::shared_ptr<Node> AddParticleSystemNode(
		const std::shared_ptr<Node>& parent, int id = -1, bool withDefaultEmitter = true);
	...
	This pointer should not be dropped. See IReferenceCounted::drop() for more information. */
	std::shared_ptr<Node> AddSkyDomeNode(
		const std::shared_ptr<Node>& parent, const std::shared_ptr<Texture2>& sky, 
	...
	\return Pointer to the created scene node.
	This pointer should not be dropped. See IReferenceCounted::drop() for more information. */
	std::shared_ptr<Node> AddStaticMeshNode(
		const std::shared_ptr<Node>& parent, std::shared_ptr<BaseMesh> mesh, 
		int id = -1, bool alsoAddIfMeshPointerZero = false);
	...
	\return Pointer to the created scene node.
	This pointer should not be dropped. See IReferenceCounted::drop() for more information. */
	std::shared_ptr<AnimatedMeshNode> AddAnimatedMeshNode(
		const std::shared_ptr<Node>& parent, std::shared_ptr<BaseAnimatedMesh> mesh, 
		int id = -1, bool alsoAddIfMeshPointerZero = false);
	...
	This pointer should not be dropped. See IReferenceCounted::drop() for more information. */
	std::shared_ptr<Node> AddLightNode(
		const std::shared_ptr<Node>& parent, const std::shared_ptr<Light>& light, int id = -1);
	...
	If you no longer need the animator, you should call ISceneNodeAnimator::drop().
	See IReferenceCounted::drop() for more information. */
	std::shared_ptr<NodeAnimator> Scene::CreateFlyCircleAnimator(
		const Vector3<float>& center = Vector3<float>{ 0.f, 0.f, 0.f }, 
		float radius = 100.f, float speed = 0.001f,
		const Vector3<float>& direction = Vector3<float>{ 0.f, 1.f, 0.f }, 
		float startPosition = 0.f, float radiusEllipsoid = 0.f);
	...
	If you no longer need the animator, you should call ISceneNodeAnimator::drop().
	See IReferenceCounted::drop() for more information. */
	std::shared_ptr<NodeAnimator> Scene::CreateFlyStraightAnimator(
		const Vector3<float>& startPoint, const Vector3<float>& endPoint, 
		unsigned int timeForWay, bool loop = false, bool pingpong = false);
	...
	std::shared_ptr<NodeAnimator> Scene::CreateTextureAnimator(
		const std::vector<std::shared_ptr<Texture2>>& textures, int timePerFrame, bool loop = true);
	...
	std::shared_ptr<NodeAnimator> Scene::CreateFollowSplineAnimator(int startTime, 
		const std::vector<Vector3<float>>& points, float speed = 1.0f, 
		float tightness = 0.5f, bool loop = true, bool pingpong = false);
	...
	This pointer should not be dropped. See IReferenceCounted::drop() for more information. */
	virtual const std::shared_ptr<RootNode>& GetRootNode() { return mRoot; }

	bool RemoveSceneNode(ActorId id);

	bool AddSceneNode(ActorId id, const std::shared_ptr<Node>& node);
	...
	This pointer should not be dropped. See IReferenceCounted::drop() for more information. */
	std::shared_ptr<Node> GetSceneNode(ActorId id);

	//! Returns the nearest scene node which collides with a 3d ray
	std::shared_ptr<Node> GetSceneNode(const Vector4<float>& start, 
		const Vector4<float>& direction, const std::shared_ptr<Node>& root = 0);
	...
	This pointer should not be dropped. See IReferenceCounted::drop() for more information. */
	const std::shared_ptr<CameraNode>& GetActiveCamera() const { return mCamera; }
	...
	void SetLightManager(const std::shared_ptr<LightManager>& lightManager) { mLightManager = lightManager; }

	const std::shared_ptr<LightManager>& GetLightManager() { return mLightManager; }
	...
	void AddToRenderQueue(RenderPass renderPass, const std::shared_ptr<Node>& node);
...
protected:
	...
	void GetPickedNode(const std::shared_ptr<Node>& node,
...
public:
	...
	virtual bool AddChild(ActorId id, const std::shared_ptr<Node>& kid) { return AddSceneNode(id, kid); }
};
