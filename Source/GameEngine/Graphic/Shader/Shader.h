...
class ShaderResourceExtraData : public BaseResourceExtraData
{
    std::shared_ptr<VisualProgram> mShaderProgram;

public:
    virtual std::wstring ToString() { return L"ShaderResourceExtraData"; }
    std::shared_ptr<VisualProgram>& GetProgram(void) { return mShaderProgram; }
};

class ShaderResourceLoader : public BaseResourceLoader
{
public:
    ...
    virtual bool LoadResource(void* rawBuffer, unsigned int rawSize, const std::shared_ptr<ResHandle>& handle);
    virtual bool MatchResourceFormat(std::wstring name) { return IsALoadableFileExtension(name); }

protected:

    bool IsALoadableFileExtension(const std::wstring& filename) const;
};
...
protected:
    ...
    struct Data
    {
        Data(GraphicObjectType inType, std::string const& inName, int32_t inBindPoint,
...
{
	std::vector<Data>& data = mData[T::mShaderDataLookup];
...
{
	std::vector<Data> const& data = mData[T::mShaderDataLookup];
    if (0 <= handle && handle < static_cast<int>(data.size()))
    {
...
{
	std::vector<Data>& data = mData[ConstantBuffer::mShaderDataLookup];
    if (0 <= handle && handle < static_cast<int>(data.size()))
    {
        auto& d = data[handle];
        if (IsValid(d, object.get()))
...
{
	std::vector<Data>& data = mData[TextureBuffer::mShaderDataLookup];
    if (0 <= handle && handle < static_cast<int>(data.size()))
    {
        auto& d = data[handle];
        if (IsValid(d, object.get()))
