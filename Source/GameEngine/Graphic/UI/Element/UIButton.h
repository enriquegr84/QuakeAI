...
public:
	...
	//! Initialize button
	virtual void OnInit(bool noclip = false, bool foregroundImage = false) = 0;
	...
	virtual const std::shared_ptr<BaseUIFont>& GetOverrideFont(void) const = 0;

	//! Get the font which is used right now for drawing
	/** Currently this is the override font when one is set and the
	font of the active skin otherwise */
	virtual std::shared_ptr<BaseUIFont> GetActiveFont() const = 0;
	...
	virtual void SetImage(const std::shared_ptr<Texture2>& image = 0) = 0;

	//! Sets a background image for the button when it is in normal state.
	/** \param image: Texture containing the image to be displayed
	\param pos: Position in the texture, where the image is located */
	virtual void SetImage(const std::shared_ptr<Texture2>& image, const RectangleShape<2, int>& sourceRect) = 0;

    //! Sets a background image for the button when it is in normal state.
    /** \param image: Texture containing the image to be displayed
    \param pos: Position in the texture, where the image is located */
    virtual void SetImage(UIButtonImageState state,
        const std::shared_ptr<Texture2>& image = 0, const RectangleShape<2, int>& sourceRect = RectangleShape<2, int>()) = 0;

	//! Sets a background image for the button when it is in pressed state.
	/** If no images is specified for the pressed state via
	setPressedImage(), this image is also drawn in pressed state.
	\param image: Image to be displayed */
	virtual void SetPressedImage(const std::shared_ptr<Texture2>& image = 0) = 0;

	//! Sets an image which should be displayed on the button when it is in pressed state.
	/** \param image: Texture containing the image to be displayed
	\param pos: Position in the texture, where the image is located */
	virtual void SetPressedImage(const std::shared_ptr<Texture2>& image, const RectangleShape<2, int>& sourceRect) = 0;

    //! Sets a foreground image which should be displayed on the button.
    //! \param image : Image to be displayed
    virtual void SetForegroundImage(const std::shared_ptr<Texture2>& image) = 0;

	//! Sets the sprite bank used by the button
	virtual void SetSpriteBank(const std::shared_ptr<BaseUISpriteBank>& sprites = 0) = 0;

	//! Sets the animated sprite for a specific button state
	/** \param index: mNumber of the sprite within the sprite bank, use -1 for no sprite
    ...
    virtual void SetFromState() = 0;

    //! Set element properties from a style
    virtual void SetFromStyle(const Style& style) = 0;

    //! Set the styles used for each state
    virtual void SetStyles(const std::array<Style, Style::NUM_STATES>& styles) = 0;
};
...
public:
	...
	//! initialize button
	virtual void OnInit(bool noclip=false, bool foregroundImage=false);
	...
	//! Get the font which is used right now for drawing
	virtual std::shared_ptr<BaseUIFont> GetActiveFont() const;
	...
	//! Sets an image which should be displayed on the button when it is in normal state.
	virtual void SetImage(const std::shared_ptr<Texture2>& image=0);

	//! Sets an image which should be displayed on the button when it is in normal state.
	virtual void SetImage(const std::shared_ptr<Texture2>& image, const RectangleShape<2, int>& sourceRect);

    //! Sets the image which should be displayed on the button when it is in its normal state.
    virtual void SetImage(UIButtonImageState state, 
        const std::shared_ptr<Texture2>& image = 0, const RectangleShape<2, int>& sourceRect = RectangleShape<2, int>());

	//! Sets an image which should be displayed on the button when it is in pressed state.
	virtual void SetPressedImage(const std::shared_ptr<Texture2>& image=0);

	//! Sets an image which should be displayed on the button when it is in pressed state.
	virtual void SetPressedImage(const std::shared_ptr<Texture2>& image, const RectangleShape<2, int>& sourceRect);

    //! Sets a foreground image which should be displayed on the button.
    virtual void SetForegroundImage(const std::shared_ptr<Texture2>& image);

    //! Sets the new caption of this element.
    virtual void SetText(const wchar_t* text);

	//! Sets the sprite bank used by the button
	virtual void SetSpriteBank(const std::shared_ptr<BaseUISpriteBank>& sprites=0);
    ...
    //! Set element properties from a style corresponding to the button state
    virtual void SetFromState();

    //! Set element properties from a style
    virtual void SetFromStyle(const Style& style);

    //! Set the styles used for each state
    virtual void SetStyles(const std::array<Style, Style::NUM_STATES>& styles);
...
protected:
    ...
    UIButtonImageState GetImageState(bool pressed);
    UIButtonImageState GetImageState(bool pressed, const ButtonImage* images);
...
private:
    ...
    std::array<Style, Style::NUM_STATES> mStyles;
	...
	std::shared_ptr<BaseUIFont> mOverrideFont;

    std::shared_ptr<BaseUIStaticText> mStaticText;

    std::shared_ptr<BaseUIImage> mForegroundImage;
