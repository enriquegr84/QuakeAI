...
class BaseUIElement : public std::enable_shared_from_this<BaseUIElement>
{
    friend class BaseUI;
...
public:
    ...
    void SetParent(const std::shared_ptr<BaseUIElement>& parent)
    {
        if (parent)
        {
    ...
    const std::shared_ptr<BaseUIElement>& GetParent() const
    {
        return mParent;
    }
    ...
    void SetRelativePosition(const RectangleShape<2, int>& r)
    {
        if (mParent)
        {
            ...
            {
                float corner = mScaleRect.GetVertice(RVP_UPPERLEFT)[0];
                int scale = mDesiredRect.GetVertice(RVP_UPPERLEFT)[0] / dimension[0];
                mScaleRect.mExtent[0] = corner - scale;
                mScaleRect.mCenter[0] = corner + (mDesiredRect.mExtent[0] / 2);
            }
            if (mAlignRight == UIA_SCALE)
            {
                float corner = mScaleRect.GetVertice(RVP_LOWERRIGHT)[0];
                int scale = mDesiredRect.GetVertice(RVP_LOWERRIGHT)[0] / dimension[0];
                mScaleRect.mExtent[0] = scale - corner;
                mScaleRect.mCenter[0] = corner - (int)round(mDesiredRect.mExtent[0] / 2.f);
            }
            if (mAlignTop == UIA_SCALE)
            {
                float corner = mScaleRect.GetVertice(RVP_UPPERLEFT)[1];
                int scale = mDesiredRect.GetVertice(RVP_UPPERLEFT)[1] / dimension[1];
                mScaleRect.mExtent[1] = corner - scale;
                mScaleRect.mCenter[1] = corner + (mDesiredRect.mExtent[1] / 2);
            }
            if (mAlignBottom == UIA_SCALE)
            {
                float corner = mScaleRect.GetVertice(RVP_LOWERRIGHT)[1];
                int scale = mDesiredRect.GetVertice(RVP_LOWERRIGHT)[1] / dimension[1];
    ...
    void SetRelativePositionProportional(const RectangleShape<2, float>& r)
    {
        ...
        Vector2<int> upperLeftCorner{
            (int)floor(dimension[0] * r.GetVertice(RVP_UPPERLEFT)[0]),
            (int)floor(dimension[1] * r.GetVertice(RVP_UPPERLEFT)[1]) };
        Vector2<int> lowerRightCorner{
            (int)floor(dimension[0] * r.GetVertice(RVP_LOWERRIGHT)[0]),
            (int)floor(dimension[1] * r.GetVertice(RVP_LOWERRIGHT)[1]) };
        std::array<Vector2<int>, 2U> axis;
    ...
    */
    std::shared_ptr<BaseUIElement> GetElementFromPoint(const Vector2<int>& point)
    {
        std::shared_ptr<BaseUIElement> target = 0;

        // we have to search from back to front, because later children
        // might be drawn over the top of earlier ones.

        std::list<std::shared_ptr<BaseUIElement>>::reverse_iterator it = mChildren.rbegin();
        if (IsVisible())
        {
    ...
    virtual void AddChild(const std::shared_ptr<BaseUIElement>& child)
    {
        AddChildToEnd(child);
        if (child)
    ...
    virtual void RemoveChildren(bool cascade)
    {
        ...
        while (!mChildren.empty())
        {
            std::list<std::shared_ptr<BaseUIElement>>::iterator it = mChildren.begin();
            (*it)->mParent = nullptr;
            if (cascade) (*it)->RemoveChildren(true);
            (*it) = nullptr;
            mChildren.erase(it);
        }
    }
    ...
    void SetTabOrder(int index)
    {
        ...
        {
            mTabOrder = 0;
            std::shared_ptr<BaseUIElement> el = GetTabGroup();
            while (mTabGroup && el && el->mParent)
                el = el->mParent;

            const std::shared_ptr<BaseUIElement>& first = 0, closest = 0;
            if (el)
            {
    ...
    std::shared_ptr<BaseUIElement> GetTabGroup()
    {
        std::shared_ptr<BaseUIElement> ret(shared_from_this());
    ...
    */
    ...
    virtual bool BringToFront(const std::shared_ptr<BaseUIElement>& element)
    {
        std::list<std::shared_ptr<BaseUIElement>>::iterator it = mChildren.begin();
        for (; it != mChildren.end(); ++it)
        {
    ...
    /** \return True if successful, false if not. */
    virtual bool SendToBack(const std::shared_ptr<BaseUIElement>& element)
    {
        std::list<std::shared_ptr<BaseUIElement>>::iterator it = mChildren.begin();
    ...
    virtual const std::list<std::shared_ptr<BaseUIElement>>& GetChildren() const
    {
        return mChildren;
    }
    ...
    virtual std::shared_ptr<BaseUIElement> GetElementFromId(int id, bool searchchildren = false) const
    {
        std::shared_ptr<BaseUIElement> e = 0;

        std::list<std::shared_ptr<BaseUIElement>>::const_iterator it = mChildren.begin();
        for (; it != mChildren.end(); ++it)
        {
            if ((*it)->GetID() == id)
                return (*it);
    ...
    bool IsChild(BaseUIElement* child) const
    {
        if (!child)
            return false;
        do
    ...
    bool GetNextElement(int startOrder, bool reverse, bool group,
        std::shared_ptr<BaseUIElement> first, std::shared_ptr<BaseUIElement> closest,
...
protected:
    ...
    void AddChildToEnd(const std::shared_ptr<BaseUIElement>& child)
    {
        if (child)
        {
    ...
    void RecalculateAbsolutePosition(bool recursive)
    {
        ...
        if (mParent)
        {
            ...
            if (mNoClip)
            {
                std::shared_ptr<BaseUIElement> p(shared_from_this());
                while (p->mParent)
                    p = p->mParent;
        ...
        switch (mAlignLeft)
        {
            ...
            case UIA_CENTER:
                ...
                break;
            case UIA_SCALE:
                int vertice = mDesiredRect.GetVertice(RVP_UPPERLEFT)[0];
                int newVertice = (int)round(mScaleRect.GetVertice(RVP_UPPERLEFT)[0] * fw);
                mDesiredRect.mExtent[0] += (vertice - newVertice);
                mDesiredRect.mCenter[0] = newVertice + mDesiredRect.mExtent[0] / 2;
                break;
        }
        ...
        switch (mAlignRight)
        {
            ...
            case UIA_CENTER:
                ...
                break;
            case UIA_SCALE:
                int vertice = mDesiredRect.GetVertice(RVP_LOWERRIGHT)[0];
                int newVertice = (int)round(mScaleRect.GetVertice(RVP_LOWERRIGHT)[0] * fw);
                mDesiredRect.mExtent[0] += (newVertice - vertice);
                mDesiredRect.mCenter[0] = newVertice - (int)round(mDesiredRect.mExtent[0] / 2.f);
                break;
        }
        ...
        switch (mAlignTop)
        {
            ...
            case UIA_CENTER:
                ...
                break;
            case UIA_SCALE:
                int vertice = mDesiredRect.GetVertice(RVP_UPPERLEFT)[1];
                int newVertice = (int)round(mScaleRect.GetVertice(RVP_UPPERLEFT)[1] * fh);
                mDesiredRect.mExtent[1] += (vertice - newVertice);
                mDesiredRect.mCenter[1] = newVertice + mDesiredRect.mExtent[1] / 2;
                break;
        }
        ...
        switch (mAlignBottom)
        {
            ...
            case UIA_CENTER:
                ...
                break;
            case UIA_SCALE:
                int vertice = mDesiredRect.GetVertice(RVP_LOWERRIGHT)[1];
                int newVertice = (int)round(mScaleRect.GetVertice(RVP_LOWERRIGHT)[1] * fh);
                mDesiredRect.mExtent[1] += (newVertice - vertice);
                mDesiredRect.mCenter[1] = newVertice - (int)round(mDesiredRect.mExtent[1] / 2.f);
                break;
        }
        ...
        mAbsoluteRect.mCenter += parentAbsolute.GetVertice(RVP_UPPERLEFT);

        if (!mParent)
            parentAbsoluteClip = mAbsoluteRect;

        mAbsoluteClippingRect = mAbsoluteRect;
        mAbsoluteClippingRect.ClipAgainst(parentAbsoluteClip);
...
protected:
    ...
    std::list<std::shared_ptr<BaseUIElement>> mChildren;

    //! Pointer to the parent
    std::shared_ptr<BaseUIElement> mParent;
