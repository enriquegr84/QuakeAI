...
void UIListBox::OnInit(bool clip)
{
	if (mUI && mUI->GetSkin())
	{
		mSelectedText.reset(new UIStaticText(mUI, -1, mRelativeRect, L"", false, false));
		...
		mScrollBar.reset(new UIScrollBar(mUI, -1, mRelativeRect, false, true));
...
size_t UIListBox::GetItemCount() const
{
	return mItems.size();
}
...
{
    if (xPos < mAbsoluteRect.GetVertice(RVP_UPPERLEFT)[0] ||
        yPos < mAbsoluteRect.GetVertice(RVP_UPPERLEFT)[1] ||
        xPos > mAbsoluteRect.GetVertice(RVP_LOWERRIGHT)[0] ||
        yPos > mAbsoluteRect.GetVertice(RVP_LOWERRIGHT)[1])
    {
        return -1;
    }
    ...
    int verticalPos = mAbsoluteRect.GetVertice(RVP_UPPERLEFT)[1];
	int item = ((yPos - verticalPos - 1) + mScrollBar->GetPosition()) / mItemHeight;
...
bool UIListBox::OnEvent(const Event& evt)
{
	if (IsEnabled())
	{
			...
			case ET_KEY_INPUT_EVENT:
					...
					evt.mKeyInput.mKey == KEY_PRIOR))
				{
					...
					if (mSelected >= (int)mItems.size())
						mSelected = (int)mItems.size() - 1;	// will set Selected to -1 for empty listboxes which is correct
				...
				else if (evt.mKeyInput.mPressedDown && evt.mKeyInput.mChar)
				{
					...
					if (now - mLastKeyTime < 500)
					{
						...
						if (!(mKeyBuffer.size() == 1 && mKeyBuffer[0] == evt.mKeyInput.mChar))
						{
							mKeyBuffer += L" ";
							mKeyBuffer[mKeyBuffer.size()-1] = evt.mKeyInput.mChar;
						}
					}
					else
					{
						mKeyBuffer = L" ";
						mKeyBuffer[0] = evt.mKeyInput.mChar;
					}
					mLastKeyTime = now;
			...
			case ET_UI_EVENT:
				switch(evt.mUIEvent.mEventType)
				{
					case UIEVT_SCROLL_BAR_CHANGED:
						if (evt.mUIEvent.mCaller == mScrollBar.get())
							return true;
						break;
					...
					switch(evt.mMouseInput.mEvent)
					{
						...
						case MIE_MOUSE_MOVED:
							if (mSelecting || mMoveOverSelect)
							{
								if (IsPointInside(p))
								{
									SelectNew(evt.mMouseInput.Y, true);
									return true;
								}
							}
						default:
...
void UIListBox::SelectNew(int ypos, bool onlyHover)
{
	...
	mSelected = GetItemAt(mAbsoluteRect.GetVertice(RVP_UPPERLEFT)[0], ypos);
...
void UIListBox::Draw()
{
	...
	frameRect.mCenter[1] -= mScrollBar->GetPosition();
	...
	{
		if (frameRect.GetVertice(RVP_LOWERRIGHT)[1] <= mAbsoluteRect.GetVertice(RVP_LOWERRIGHT)[1] &&
			frameRect.GetVertice(RVP_LOWERRIGHT)[1] >= mAbsoluteRect.GetVertice(RVP_UPPERLEFT)[1])
		{
			...
			if (mFont)
			{
				...
				if ( i==mSelected )
				{
                    ...
                    selectedRect.mCenter -= mAbsoluteRect.GetVertice(RVP_UPPERLEFT);
...
unsigned int UIListBox::AddItem(const wchar_t* text, int icon)
{
	...
	return (unsigned int)(mItems.size() - 1);
}


void UIListBox::SetSpriteBank(const std::shared_ptr<BaseUISpriteBank>& bank)
{
	if ( bank == mIconBank )
		return;
...
void UIListBox::RecalculateScrollPosition()
{
	if (!mAutoScroll)
		return;

	const int selPos = (mSelected == -1 ? mTotalItemHeight : mSelected * mItemHeight) - mScrollBar->GetPosition();

	if (selPos < 0)
	{
		mScrollBar->SetPosition(mScrollBar->GetPosition() + selPos);
	}
