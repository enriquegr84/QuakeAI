...
struct UISprite
{
	...
	std::vector<UISpriteFrame> mFrames;
...
public:
	...
	virtual std::vector<RectangleShape<2, int>>& GetPositions() = 0;

	//! Returns the array of animated sprites within the sprite bank
	virtual std::vector<UISprite>& GetSprites() = 0;
	...
	virtual std::shared_ptr<Texture2> GetTexture(unsigned int index) const = 0;

	//! Adds a texture to the sprite bank
	virtual void AddTexture(std::shared_ptr<Texture2> texture) = 0;

	//! Changes one of the textures in the sprite bank
	virtual void SetTexture(unsigned int index, std::shared_ptr<Texture2> texture) = 0;
	...
	virtual int AddTextureAsSprite(std::shared_ptr<Texture2> texture) = 0;
    ...
    */
	...
	virtual void Draw2DSpriteBatch(const std::vector<unsigned int>& indices, 
...
public:
	...
	virtual std::vector<RectangleShape<2, int>>& GetPositions();
	virtual std::vector<UISprite>& GetSprites();

	virtual size_t GetTextureCount() const;
	virtual std::shared_ptr<Texture2> GetTexture(unsigned int index) const;
	virtual void AddTexture(std::shared_ptr<Texture2> texture);
	virtual void SetTexture(unsigned int index, std::shared_ptr<Texture2> texture);

	//! Add the texture and use it for a single non-animated sprite.
	virtual int AddTextureAsSprite(std::shared_ptr<Texture2> texture);
	...
	virtual void Draw2DSprite(unsigned int index, const std::shared_ptr<Visual>& visual, const RectangleShape<2, int>& destRect, 
...
protected:
	...
	struct DrawBatch
	{
		std::vector<Vector2<int>> mPositions;
		std::vector<RectangleShape<2, int>> mSourceRects;
		unsigned int mTextureNumber;
	};

	BaseUI* mUI;

	std::vector<UISprite> mSprites;
	std::vector<RectangleShape<2, int>> mRectangles;
	std::vector<std::shared_ptr<Texture2>> mTextures;
};
