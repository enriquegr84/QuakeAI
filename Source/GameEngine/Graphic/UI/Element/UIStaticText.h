...
public:
	...
	//! constructor
	UIStaticText(BaseUI* ui, int id, RectangleShape<2, int> rectangle, 
        const wchar_t* text, bool border, bool background = false);
...
private:
	...
	std::shared_ptr<BaseUIFont> mLastBreakFont; // stored because: if skin changes, line break must be recalculated.

	BaseUI* mUI;

	std::vector<EnrichedString> mBrokenText;
};
