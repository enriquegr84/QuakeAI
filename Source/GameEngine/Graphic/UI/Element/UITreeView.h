...
*/
class BaseUITreeViewNode : public std::enable_shared_from_this<BaseUITreeViewNode>
{
public:
	//! returns the owner (tree view) of this node
	virtual std::shared_ptr<BaseUIElement> GetOwner() const = 0;

	//! Returns the parent node of this node.
	/** For the root node this will return 0. */
	virtual std::shared_ptr<BaseUITreeViewNode> GetParent() const = 0;
	...
	//! returns the children item count
	...
	\return The new node
	*/
	virtual std::shared_ptr<BaseUITreeViewNode> AddChildBack(
	...
	\return The new node
	*/
	virtual std::shared_ptr<BaseUITreeViewNode> AddChildFront(
		const wchar_t* text, const wchar_t* icon = 0, int imageIndex = -1, 
		int selectedImageIndex = -1, void* data = 0, void* data2 = 0) = 0;
	...
	*/
	virtual std::shared_ptr<BaseUITreeViewNode> InsertChildAfter(
		std::shared_ptr<BaseUITreeViewNode> other, const wchar_t* text, const wchar_t* icon = 0,
	...
	\return The new node or 0 if other is no child node from this
	*/
	virtual std::shared_ptr<BaseUITreeViewNode> InsertChildBefore(
		std::shared_ptr<BaseUITreeViewNode> other, const wchar_t* text, const wchar_t* icon = 0,
		int imageIndex = -1, int selectedImageIndex = -1, void* data = 0, void* data2 = 0) = 0;

	//! Return the first child node from this node.
	/** \return The first child node or 0 if this node has no children. */
	virtual std::shared_ptr<BaseUITreeViewNode> GetFrontChild() const = 0;

	//! Return the last child node from this node.
	/** \return The last child node or 0 if this node has no children. */
	virtual std::shared_ptr<BaseUITreeViewNode> GetBackChild() const = 0;
...
public:
	...
	virtual std::shared_ptr<BaseUITreeViewNode> GetRoot() const = 0;

	//! returns the selected node of the tree or 0 if none is selected
	virtual std::shared_ptr<BaseUITreeViewNode> GetSelected() const = 0;
	...
	*/
	virtual void SetIconFont(std::shared_ptr<BaseUIFont> font) = 0;
	...
	virtual std::shared_ptr<BaseUITreeViewNode> GetLastEventNode() const = 0;
};
...
public:
	//! constructor
	UITreeViewNode(std::shared_ptr<BaseUIElement> owner, std::shared_ptr<BaseUITreeViewNode> parent);

	//! destructor
	~UITreeViewNode();

	//! returns the owner (tree view) of this node
	virtual std::shared_ptr<BaseUIElement> GetOwner() const;

	//! Returns the parent node of this node.
	virtual std::shared_ptr<BaseUITreeViewNode> GetParent() const;
	...
	virtual size_t GetChildrenCount() const { return mChildren.size(); }
	...
	virtual std::shared_ptr<BaseUITreeViewNode> GetFrontChild() const;

	//! Return the last child note from this node.
	virtual std::shared_ptr<BaseUITreeViewNode> GetBackChild() const;

	//! Returns the preverse sibling node from this node.
	virtual std::shared_ptr<BaseUITreeViewNode> GetPrevNode() const;

	//! Returns the next sibling node from this node.
	virtual std::shared_ptr<BaseUITreeViewNode> GetNextNode() const;

	//! Returns the next visible (expanded, may be out of scrolling) node from this node.
	virtual std::shared_ptr<BaseUITreeViewNode> GetNextVisible() const;

	//! Deletes a child node.
	virtual bool DeleteChild(std::shared_ptr<BaseUITreeViewNode> child );

	//! Moves a child node one position up.
	virtual bool MoveChildUp(std::shared_ptr<BaseUITreeViewNode> child );

	//! Moves a child node one position down.
	virtual bool MoveChildDown(std::shared_ptr<BaseUITreeViewNode> child );
...
private:
	...
	std::shared_ptr<BaseUIElement> mOwner;
	std::shared_ptr<BaseUITreeViewNode> mParent;
	...
	void* mData2;
	bool mExpanded;
	std::list<std::shared_ptr<BaseUITreeViewNode>> mChildren;
};
...
public:
	...
	//! returns the root node (not visible) from the tree.
	virtual std::shared_ptr<BaseUITreeViewNode> GetRoot() const { return mRoot; }

	//! returns the selected node of the tree or 0 if none is selected
	virtual std::shared_ptr<BaseUITreeViewNode> GetSelected() const { return mSelected; }
	...
	virtual void SetIconFont(std::shared_ptr<BaseUIFont> font);
	...
	//! Returns the node which is associated to the last event.
	virtual std::shared_ptr<BaseUITreeViewNode> GetLastEventNode() const { return mLastEventNode; }
...
private:
	...
	std::shared_ptr<BaseUITreeViewNode> mRoot;
	std::shared_ptr<BaseUITreeViewNode> mSelected;
	...
	std::shared_ptr<BaseUIFont> mIconFont;
	std::shared_ptr<BaseUIScrollBar> mScrollBarH;
	std::shared_ptr<BaseUIScrollBar> mScrollBarV;
	std::shared_ptr<BaseUITreeViewNode> mLastEventNode;
	bool mLinesVisible;
	bool mSelecting;
	bool mClip;
	bool mDrawBack;
