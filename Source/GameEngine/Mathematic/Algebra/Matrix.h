...
public:
	...
	// Transformation
	void Transformation(Vector<NumRows, Real>& vec) const;
    void Transformation(Vector<NumRows, Real> const& in, Vector<NumRows, Real>& out) const;

    void Translation(Vector<NumRows, Real>& vec) const;
    void Translation(Vector<NumRows, Real> const& in, Vector<NumRows, Real>& out) const;
...
template <int NumRows, int NumCols, typename Real>
void Matrix<NumRows, NumCols, Real>::SetRow(int r, Vector<NumCols, Real> const& vec)
{
...
template <int NumRows, int NumCols, typename Real>
void Matrix<NumRows, NumCols, Real>::SetCol(int c, Vector<NumRows, Real> const& vec)
{
	Real const* in = reinterpret_cast<Real const*>(&vec);
...
template <int NumRows, int NumCols, typename Real>
void Matrix<NumRows, NumCols, Real>::Transformation(Vector<NumRows, Real>& vec) const
{
    Vector<NumRows, Real> tmp;
    Real* vInOut = reinterpret_cast<Real*>(&vec);
    for (int r = 0; r < NumRows; ++r)
    {
        tmp[r] = (Real)0;
        for (int c = 0; c < NumCols; ++c)
        {
            tmp[r] += vInOut[c] * mTable[c * NumCols + r];
        }
    }
...
template <int NumRows, int NumCols, typename Real>
void Matrix<NumRows, NumCols, Real>::Translation(
    Vector<NumRows, Real> const& in, Vector<NumRows, Real>& out) const
{
    Real* vOut = reinterpret_cast<Real*>(&out);
    Real const* vIn = reinterpret_cast<Real const*>(&in);
    for (int r = 0; r < NumRows; ++r)
    {
        vOut[r] = vIn[r] + mTable[r * NumCols + (NumCols - 1)];
    }
}

template <int NumRows, int NumCols, typename Real>
void Matrix<NumRows, NumCols, Real>::Translation(Vector<NumRows, Real>& vec) const
{
    Real* vInOut = reinterpret_cast<Real*>(&vec);
    for (int r = 0; r < NumRows; ++r)
    {
        vInOut[r] = vInOut[r] + mTable[r * NumCols + (NumCols - 1)];
    }
}
...
template <int NumRows, int NumCols, typename Real>
Matrix<NumRows, NumCols, Real> Matrix<NumRows, NumCols, Real>::Unit(int r, int c)
{
    Matrix M;
    M.MakeUnit(r, c);
    return M;
