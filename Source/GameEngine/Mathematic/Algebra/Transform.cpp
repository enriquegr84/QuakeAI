...
void Transform::Set2DRotationCenter(float radAngle)
{
    const float c = cosf(radAngle);
    const float s = sinf(radAngle);
    mMatrix[0] = c;
    mMatrix[1] = s;
    ...
    mMatrix[8] = (0.5f * (s - c) + 0.5f);
    mMatrix[9] = (-0.5f * (s + c) + 0.5f);
}

//! Set transformation translation
void Transform::Set2DTranslation(float x, float y)
{
    mMatrix[8] = x;
    mMatrix[9] = y;
}
...
void Transform::Get2DTranslation(float& x, float& y) const
{
    x = (float)mMatrix[8];
    y = (float)mMatrix[9];
}

//! Set transformation translation, using a transposed representation
void Transform::Set2DTranslationTransposed(float x, float y)
{
    mMatrix[2] = x;
    mMatrix[6] = y;
}
...
void Transform::Set2DScale(float sx, float sy)
{
    mMatrix[0] = sx;
    mMatrix[5] = sy;
}
...
void Transform::Get2DScale(float& sx, float& sy) const
{
    sx = mMatrix[0];
    sy = mMatrix[5];
}

void Transform::Set2DScaleCenter(float sx, float sy)
{
    mMatrix[0] = sx;
    mMatrix[5] = sy;
    mMatrix[8] = (0.5f - 0.5f * sx);
    mMatrix[9] = (0.5f - 0.5f * sy);
}
...
Transform operator*(Transform const& A, Transform const& B)
{
    ...
    if (A.IsRSMatrix() && B.IsRSMatrix())
    {
        ...
        if (A.IsUniformScale())
        {
            ...
            Vector4<float> transform = B.GetTranslationW0();
			A.GetRotation().Transformation(transform);
        ...
        if (B.IsUniformScale())
        {
            ...
            Vector4<float> transform = A.GetTranslationW0();
			B.GetRotation().Transformation(transform);
...
#if defined(GE_USE_MAT_VEC)
    Vector4<float> transform = B.GetTranslationW0();
	matMA.Transformation(transform);
	product.SetTranslation(transform + A.GetTranslationW1());
#else
    Vector4<float> transform = A.GetTranslationW0();
	matMB.Transformation(transform);
