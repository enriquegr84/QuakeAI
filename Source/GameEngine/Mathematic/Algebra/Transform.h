...
public:
    ...
    void Get2DScale(float& sx, float& sy) const;

    //! Set transformation scale, and recenter at (0.5,0.5)
    /** Doesn't clear other elements than those affected.
    \param sx Scale factor on x axis
    \param sy Scale factor on y axis */
    void Set2DScaleCenter(float sx, float sy);
...
inline Matrix4x4<float> operator*(Matrix4x4<float> const& A, Transform const& B);
inline Matrix4x4<float> operator*(Transform const& A, Matrix4x4<float> const& B);
