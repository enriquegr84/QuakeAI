...
template <int N, typename Real>
Real LengthSq(Vector<N, Real> const& v, bool robust = false);
...
public:
    ...
    friend Real LengthSq<N, Real>(Vector const& v, bool robust);
...
bool Vector<N, Real>::operator<(Vector const& vec) const
{
	std::array<Real, N> thisTuple;
	std::array<Real, N> otherTuple;
	for (int i = 0; i < N; ++i)
	{
...
Real Length(Vector<N, Real> const& v, bool robust)
{
	if (robust)
	{
		Real maxAbsComp = (Real)fabs(v.mTuple[0]);
		for (int i = 1; i < N; ++i)
		{
			Real absComp = (Real)fabs(v.mTuple[i]);
		...
		if (maxAbsComp > (Real)0)
		{
			...
			length = (Real)(maxAbsComp * sqrt(Dot(scaled, scaled)));
		}
		else length = (Real)0;
		return length;
	}
	else return (Real)sqrt(Dot(v, v));
}


template <int N, typename Real> inline
Real LengthSq(Vector<N, Real> const& v, bool robust)
{
    if (robust)
    {
        Real maxAbsComp = (Real)fabs(v.mTuple[0]);
        ...
        {
            Vector<N, Real> scaled = v / maxAbsComp;
            length = (Real)(maxAbsComp * Dot(scaled, scaled));
        }
        else length = (Real)0;
        return length;
    }
    else return (Real)Dot(v, v);
}
