...
template <typename Real>
Vector2<Real> Rotate(double degrees, Vector2<Real> const& v,
    const Vector2<Real>& center = {0, 0});
...
template <typename Real>
Vector2<Real> Rotate(double degrees, Vector2<Real> const& in, const Vector2<Real>& center)
{
    Vector2<Real> r = in;

    degrees *= GE_C_DEG_TO_RAD;
    const Real cs = (Real)cos(degrees);
    const Real sn = (Real)sin(degrees);
