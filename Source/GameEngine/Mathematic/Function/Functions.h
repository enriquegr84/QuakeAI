...
public:
    ...
    inline static float Min(const float a, const float b, const float c)
    {
        return a < b ? std::min(a, c) : std::min(b, c);
    }

    inline static float Max(const float a, const float b, const float c)
    {
        return a < b ? std::max(b, c) : std::max(a, c);
    }
	...
	template <int N>
	inline static std::array<float, N> Lerp(std::array<float, N> v0, std::array<float, N> v1, float intp)
	{
		intp = std::clamp(intp, 0.f, 1.f);
		const float inv = 1.0f - intp;

		std::array<float, N> lerp = std::array<float, N>();
		for (int i = 0; i < N; i++)
...
public:
    ...
    inline static double Min(const double a, const double b, const double c)
    {
        return a < b ? std::min(a, c) : std::min(b, c);
    }

    inline static double Max(const double a, const double b, const double c)
    {
        return a < b ? std::max(b, c) : std::max(a, c);
    }
	...
	template <int N>
	inline static std::array<double, N> Lerp(std::array<double, N> v0, std::array<double, N> v1, double intp)
	{
		intp = clamp(intp, 0.f, 1.f);
		const double inv = 1.0f - intp;

		std::array<double, N> lerp = std::array<double, N>();
		for (int i = 0; i < N; i++)
			lerp[i] = v1[i] * inv + v0[i] * intp;
