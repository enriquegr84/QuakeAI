...
template <typename Real>
unsigned int SingularValueDecomposition<Real>::Solve(Real const* input, int sortType)
{
    if (mNumRows > 0)
    {
...
template <typename Real>
void SingularValueDecomposition<Real>::GetSingularValues(Real* singularValues) const
{
...
template <typename Real>
void SingularValueDecomposition<Real>::GetUColumn(int index, Real* uColumn) const
{
    if (0 <= index && index < mNumRows)
    {
        ...
        for (auto itRev = mLGivens.rbegin(); itRev != mLGivens.rend(); itRev++)
        {
            auto const& givens = (*itRev);
...
template <typename Real>
void SingularValueDecomposition<Real>::GetVColumn(int index, Real* vColumn) const
{
    if (0 <= index && index < mNumCols)
    {
        ...
        // Apply the Givens rotations.
        for (auto itRev = mRGivens.rbegin(); itRev != mRGivens.rend(); itRev++)
        {
            auto const& givens = (*itRev);
...
template <typename Real>
void SingularValueDecomposition<Real>::GetSinCos(Real x, Real y, Real& cs, Real& sn)
{
...
template <typename Real>
bool SingularValueDecomposition<Real>::DiagonalEntriesNonzero(int imin, int imax, Real threshold)
{
