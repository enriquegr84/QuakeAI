...
//
void NetSocket::Send(std::shared_ptr<BasePacket> pkt, bool clearTimeOut)
{
	if (clearTimeOut)
		mTimeOut = 0;
...
void NetSocket::HandleOutput() 
{
	...
	do 
	{
		...
		std::shared_ptr<BasePacket> pkt = *i;
	...
	while (newData > hdrSize)
	{
		...
		if (mIsBinaryProtocol)
		{
			...
			if (newData >= packetSize)
			{
				...
				//test[packetSize+3]=0;
                //LogInformation(test);
				std::shared_ptr<BinaryPacket> pkt(
		...
		else
		{
			...
			{
				*(cr+1) = 0;
				std::shared_ptr<TextPacket> pkt(new TextPacket(&mRecvBuf[mRecvBegin]));
				mInList.push_back(pkt);
...
//
bool BaseSocketManager::Send(int sockId, std::shared_ptr<BasePacket> packet)
{
...
void GameServerListenSocket::HandleInput() 
{
	...
	if (new_sock != INVALID_SOCKET)
	{
        ...
        std::shared_ptr<EventDataRemoteClient> pEvent(new EventDataRemoteClient(sockId, ipAddress));
...
void RemoteEventSocket::HandleInput()
{
	...
	while (!mInList.empty())
	{
		std::shared_ptr<BasePacket> packet = *mInList.begin();
		mInList.pop_front();
		if (!strcmp(packet->GetType(), BinaryPacket::Type))
		{
			...
			switch(type)
			{
				...
				case NMS_PLAYERLOGINOK:
				{
                    ...
                    std::shared_ptr<EventDataNetworkPlayerActorAssignment> pEvent(
...
void RemoteEventSocket::CreateEvent(std::istrstream &in)
{
    ...
    {
		LogError("ERROR Unknown event type from remote: " + std::to_string(eventType));
    }
}
...
void NetworkEventForwarder::ForwardEvent(BaseEventDataPtr pEventData)
{
	...
	std::shared_ptr<BinaryPacket> eventMsg(
...
{ 
	...
	std::shared_ptr<BinaryPacket> gvidMsg(
		new BinaryPacket(out.rdbuf()->str(), (u_long)out.pcount()));
...
{
    std::shared_ptr<EventDataNewActor> pCastEventData = 
		std::static_pointer_cast<EventDataNewActor>(pEventData);
	ActorId actorId = pCastEventData->GetActorId();

	std::shared_ptr<Actor> pActor(GameLogic::Get()->GetActor(actorId).lock());
