...
// Collision Radius
#define DEFAULT_FALL_SPEED 1.25f
...
btKinematicCharacterController::btKinematicCharacterController(btPairCachingGhostObject* ghostObject, btConvexShape* convexShape, btScalar stepHeight, const btVector3& up)
{
	...
	m_fallSpeed = DEFAULT_FALL_SPEED;			// Terminal velocity of a sky diver in m/s.
	m_jumpSpeed = 10.0;     // ?
...
bool btKinematicCharacterController::checkPenetration(btCollisionWorld* collisionWorld)
{
	...
	for (int i = 0; i < m_ghostObject->getOverlappingPairCache()->getNumOverlappingPairs(); i++)
	{
		...
		for (int j = 0; j < m_manifoldArray.size(); j++)
		{
			...
			for (int p = 0; p < manifold->getNumContacts(); p++)
			{
				...
				{
					//check for stairs
					btVector3 collisionHeight = (pt.getPositionWorldOnB() - minAabb) * m_upAxis;
					if (collisionHeight.length() > m_stepHeight * 0.75f)
					{
						// TODO: cause problems on slopes, not sure if it is needed
						//if (dist < maxPen)
						//{
...
void btKinematicCharacterController::stepUp(btCollisionWorld* world)
{
	...
	//printf("\n pre step up position %f %f %f step height %f vertical offset %f", m_currentPosition[0], m_currentPosition[1], m_currentPosition[2], stepHeight, m_verticalOffset);
	m_targetPosition = m_currentPosition + m_up * (stepHeight) + m_jumpAxis * ((m_verticalOffset > 0.f ? m_verticalOffset : 0.f));
	m_currentPosition = m_targetPosition;
	//printf("\n step up position %f %f %f", m_currentPosition[0], m_currentPosition[1], m_currentPosition[2]);
...
void btKinematicCharacterController::stepForwardAndStrafe(btCollisionWorld* collisionWorld, const btVector3& walkMove)
{
	...
	//printf("\nstep forward walk %f %f %f", walkMove[0], walkMove[1], walkMove[2]);
	...
	while (fraction > btScalar(0.01) && maxIter-- > 0)
	{
		...
		if (callback.hasHit() && m_ghostObject->hasContactResponse() && needsCollision(m_ghostObject, callback.m_hitCollisionObject))
		{
			...
			//printf("\nstep forward hit %f %f %f", m_targetPosition[0], m_targetPosition[1], m_targetPosition[2]);
		...
		{
			m_currentPosition = m_targetPosition;
			//printf("\nstep forward update %f %f %f", m_targetPosition[0], m_targetPosition[1], m_targetPosition[2]);
		}
	}
}
...
void btKinematicCharacterController::stepDown(btCollisionWorld* collisionWorld, btScalar dt)
{
	...
	//printf("\n pre step down m_targetPosition %f %f %f", m_targetPosition[0], m_targetPosition[1], m_targetPosition[2]);

	btVector3 step_drop = m_up * (m_currentStepOffset + downVelocity);
	m_targetPosition -= step_drop;

	btVector3 playerPosition = m_ghostObject->getWorldTransform().getOrigin();

	//printf("\n velocity calculation");
	//printf("\n step down step offset %f down velocity %f", m_currentStepOffset, downVelocity);
	//printf("\n step down step_drop %f %f %f", step_drop[0], step_drop[1], step_drop[2]);
	//printf("\n step down player position %f %f %f", playerPosition[0], playerPosition[1], playerPosition[2]);
	//printf("\n step down player minaabb %f %f %f", minAabb[0], minAabb[1], minAabb[2]);
	//printf("\n step down m_currentPosition %f %f %f", m_currentPosition[0], m_currentPosition[1], m_currentPosition[2]);
	//printf("\n step down m_targetPosition %f %f %f", m_targetPosition[0], m_targetPosition[1], m_targetPosition[2]);
	...
	while (1)
	{
		...
		if (m_useGhostObjectSweepTest)
		{
			...
			if (!callback.hasHit() && m_ghostObject->hasContactResponse())
			{
				...
				if (!callback2.hasHit() && step_drop.length())
				{
					btVector3 from = playerPosition;
					btVector3 to = playerPosition - 3 * step_drop;
					btCollisionWorld::ClosestRayResultCallback closestResults(from, to);
					closestResults.m_flags |= btTriangleRaycastCallback::kF_FilterBackfaces;

					collisionWorld->rayTest(from, to, closestResults);
					//printf("\n step down from %f %f %f", from[0], from[1], from[2]);
					//printf("\n step down to %f %f %f", to[0], to[1], to[2]);
					if (closestResults.hasHit())
					{
						callback2.m_closestHitFraction = closestResults.m_closestHitFraction;
						callback2.m_hitCollisionObject = closestResults.m_collisionObject;
						callback2.m_hitNormalWorld = closestResults.m_hitNormalWorld;
						callback2.m_hitPointWorld = closestResults.m_hitPointWorld;
						//printf("\n collision hit %f %f %f", callback2.m_hitPointWorld.x(), callback2.m_hitPointWorld.y(), callback2.m_hitPointWorld.z());
					}
				}
			}
		}
		...
		if (downVelocity2 > 0.0 && downVelocity2 < stepHeight && has_hit == true && runonce == false && (m_wasOnGround || !m_wasJumping))
		{
			...
			//printf("\n velocity calculation when falling a small amount");
			//printf("\n step down step_drop %f %f %f", step_drop[0], step_drop[1], step_drop[2]);
			//printf("\n step down m_targetPosition %f %f %f", m_targetPosition[0], m_targetPosition[1], m_targetPosition[2]);
	...
	if ((m_ghostObject->hasContactResponse() && (callback.hasHit() && needsCollision(m_ghostObject, callback.m_hitCollisionObject))) || runonce == true)
	{
		...
		m_fallSpeed = DEFAULT_FALL_SPEED;
...
void btKinematicCharacterController::playerStep(btCollisionWorld* collisionWorld, btScalar dt)
{
	...
	//	printf("walkSpeed=%f\n",walkSpeed);
	// printf("\n pre step up vertical vel %f vertical offset %f", m_verticalVelocity, m_verticalOffset);
	stepUp(collisionWorld);
	//printf("\n step up pos %f %f %f", m_currentPosition[0], m_currentPosition[1], m_currentPosition[2]);
	...
	//printf("\n step forward pos %f %f %f", m_currentPosition[0], m_currentPosition[1], m_currentPosition[2]);
	stepDown(collisionWorld, dt);
	//printf("\n step down pos %f %f %f", m_currentPosition[0], m_currentPosition[1], m_currentPosition[2]);
...
{
	//ground detection through contact manifolds
	...
	for (int i = 0; i < m_ghostObject->getOverlappingPairCache()->getNumOverlappingPairs(); i++)
	{
		...
		bool collides = (
			obj0->getBroadphaseHandle()->m_collisionFilterGroup &
			obj1->getBroadphaseHandle()->m_collisionFilterMask) != 0;
		collides = collides && (
			obj1->getBroadphaseHandle()->m_collisionFilterGroup &
		...
		for (int j = 0; j < manifoldArray.size(); j++)
		{
			...
			for (int p = 0; p < manifold->getNumContacts(); p++)
			{
				...
				btScalar dotUp = m_upAxis.dot(pt.m_normalWorldOnB);

				//check for slopes and flat floor
				if (dotUp < -0.2f && !m_wasJumping)
				{
					//printf("\n dotUp %f detected", dotUp);
					return true;
				}
				else
				{
