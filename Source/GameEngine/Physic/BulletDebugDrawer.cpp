...
void BulletDebugDrawer::ReadSettings(tinyxml2::XMLElement *pRoot)
{
	...
	if (pNode)
	{
		...
		{
	        std::string attribute(pNode->Attribute("debug_draw_wireframe"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_DrawWireframe;
		}

		if (pNode->Attribute("debug_draw_aabb"))
		{
	        std::string attribute(pNode->Attribute("debug_draw_aabb"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_DrawAabb;
		}

		if (pNode->Attribute("debug_draw_features_text"))
		{
	        std::string attribute(pNode->Attribute("debug_draw_features_text"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_DrawFeaturesText;
		}

		if (pNode->Attribute("debug_draw_contactpoints"))
		{
	        std::string attribute(pNode->Attribute("debug_draw_contactpoints"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_DrawContactPoints;
		}

		if (pNode->Attribute("debug_no_deactivation"))
		{
	        std::string attribute(pNode->Attribute("debug_no_deactivation"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_NoDeactivation;
		}

		if (pNode->Attribute("debug_no_help_text"))
		{
	        std::string attribute(pNode->Attribute("debug_no_help_text"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_NoHelpText;
		}

		if (pNode->Attribute("debug_draw_text"))
		{
	        std::string attribute(pNode->Attribute("debug_draw_text"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_DrawText;
		}

		if (pNode->Attribute("debug_profile_timings"))
		{
	        std::string attribute(pNode->Attribute("debug_profile_timings"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_ProfileTimings;
		}

		if (pNode->Attribute("debug_enable_sat_comparison"))
		{
	        std::string attribute(pNode->Attribute("debug_enable_sat_comparison"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_EnableSatComparison;
		}

		if (pNode->Attribute("debug_disable_bullet_lcp"))
		{
	        std::string attribute(pNode->Attribute("debug_disable_bullet_lcp"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_DisableBulletLCP;
		}

		if (pNode->Attribute("debug_enable_ccd"))
		{
	        std::string attribute(pNode->Attribute("debug_enable_ccd"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_EnableCCD;
		}

		if (pNode->Attribute("debug_draw_constraints"))
		{
	        std::string attribute(pNode->Attribute("DrawConstraints"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_DrawConstraints;
		}

		if (pNode->Attribute("debug_draw_constraint_limits"))
		{
	        std::string attribute(pNode->Attribute("debug_draw_constraint_limits"));
			if (attribute == "true") debugModes |= btIDebugDraw::DBG_DrawConstraintLimits;
		}

		if (pNode->Attribute("debug_fast_wireframe"))
		{
	        std::string attribute(pNode->Attribute("debug_fast_wireframe"));
	...
	const btVector3& from, const btVector3& to, const btVector3& lineColor)
{
	...
	vFrom[0] = (float)from.x();
	vFrom[1] = (float)from.y();
	vFrom[2] = (float)from.z();

	vTo[0] = (float)to.x();
	vTo[1] = (float)to.y();
	vTo[2] = (float)to.z();

	Vector4<float> color{ (float)lineColor.x(), (float)lineColor.y(), (float)lineColor.z(), 1.f };
...
void BulletDebugDrawer::Render()
{
	...
	std::shared_ptr<VertexBuffer> vbuffer =
		std::make_shared<VertexBuffer>(vformat, (int)mVertices.size()*2);
	vbuffer->SetUsage(Resource::DYNAMIC_UPDATE);
	std::shared_ptr<IndexBuffer> ibuffer =
		std::make_shared<IndexBuffer>(IP_POLYSEGMENT_DISJOINT, (int)mVertices.size());
	std::shared_ptr<Visual> visual = std::make_shared<Visual>(vbuffer, ibuffer, mEffect);

	int vertexCount = 0;
	Vertex* vertex = vbuffer->Get<Vertex>();
	std::map<Vertex*, Vertex*>::iterator itVertex = mVertices.begin();
