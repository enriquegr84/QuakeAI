...
void BspConverter::CreateCurvedSurfaceBezier(BspLoader& bspLoader, BSPSurface* surface, bool isConvexSurface)
{
	unsigned int j, k;
	...
	if (isConvexSurface)
	{
		...
		for (S3DVertex2TCoords vertex : bezier.vertices)
			vertices.push_back(btVector3(vertex.vPosition.x, vertex.vPosition.y, vertex.vPosition.z));
