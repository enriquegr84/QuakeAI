...
public:
	...
	{
		return color(
			std::min(getAlpha() + other.getAlpha(), 255u),
...
inline void copy(S3DVertex2TCoords * dest, const BSPVertice * source)
{
	...
	unsigned char r = std::min(source->color[0] * 4, 255);
	unsigned char g = std::min(source->color[1] * 4, 255);
	unsigned char b = std::min(source->color[2] * 4, 255);
...
struct SBezier
{
	...
	std::vector<S3DVertex2TCoords> vertices;
	...
	void tesselate(int level)
	{
		...
		const unsigned int idx = (unsigned int)vertices.size();
...
private:
	...
	std::vector<S3DVertex2TCoords> column[3];
...
public: //easier for conversion
	...
	int mNumModels;
	std::vector<BSPModel> mDModels;

	int mNumShaders;
	std::vector<BSPShader> mDShaders;

	int mEntDataSize;
	std::vector<char> mDentData;

	int mNumLeafs;
	std::vector<BSPLeaf> mDLeafs;

	int mNumPlanes;
	std::vector<BSPPlane> mDPlanes;

	int mNumNodes;
	std::vector<BSPNode> mDNodes;

	int mNumLeafSurfaces;
	std::vector<int> mDLeafSurfaces;

	int mNumLeafBrushes;
	std::vector<int> mDLeafBrushes;

	int mNumBrushes;
	std::vector<BSPBrush> mDBrushes;

	int mNumBrushsides;
	std::vector<BSPBrushSide> mDBrushsides;

	int mNumLightBytes;
	std::vector<unsigned char> mLightBytes;

	int mNumGridPoints;
	std::vector<unsigned char> mGridData;

	int mNumVisBytes;
	std::vector<unsigned char> mVisBytes;

	int mNumDrawIndexes;
	std::vector<int> mDrawIndexes;

	int mNumDrawSurfaces;
	std::vector<BSPSurface> mDrawSurfaces;

	int mNumDrawVertices;
	std::vector<BSPVertice> mDrawVertices;
