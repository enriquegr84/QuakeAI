...
bool PhysicResourceLoader::IsALoadableFileExtension(const std::wstring& fileName) const
{
	...
	void *rawBuffer, unsigned int rawSize, const std::shared_ptr<ResHandle>& handle)
{
	...
	std::shared_ptr<BspResourceExtraData> pExtraData(new BspResourceExtraData());
	...
	if (IsALoadableFileExtension(file->GetFileName()))
	{
		...
		if (bspLoader.LoadBSPFile(rawBuffer))
		{
			...
			handle->SetExtra(std::shared_ptr<BspResourceExtraData>(pExtraData));
			return true;
		}
	}
...
std::shared_ptr<BaseResourceLoader> CreatePhysicResourceLoader()
{
    return std::shared_ptr<BaseResourceLoader>(new PhysicResourceLoader());
}