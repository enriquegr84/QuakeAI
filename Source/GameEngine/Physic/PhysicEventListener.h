...
class EventDataPhysCollision : public EventData
{
    ...
    std::list<Vector3<float>> mCollisionPoints;
...
public:
	...
	explicit EventDataPhysCollision(ActorId actorA, ActorId actorB,
		Vector3<float> sumNormalForce, Vector3<float> sumFrictionForce, std::list<Vector3<float>> collisionPoints )
	:	mActorA(actorA),
		mActorB(actorB),
    ...
    const std::list<Vector3<float>>& GetCollisionPoints(void) const
    {
