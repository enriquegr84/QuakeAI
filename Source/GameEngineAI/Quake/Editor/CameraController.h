...
class CameraController : public BaseMouseHandler, public BaseKeyboardHandler
{
public:
	CameraController(const std::shared_ptr<CameraNode>& camera,
	...
	const std::shared_ptr<CameraNode>& GetCamera() { return mCamera; }
	void SetCamera(const std::shared_ptr<CameraNode>& newCamera) { mCamera = newCamera; }

	bool OnMouseMove(const Vector2<int>& mousePos, const int radius);
	bool OnMouseButtonDown(const Vector2<int>& mousePos, const int radius, const std::string& buttonName);
	bool OnMouseButtonUp(const Vector2<int>& mousePos, const int radius, const std::string& buttonName);

    bool OnKeyDown(const Event::KeyInput& input) { mKey[input.mKey] = true; return true; }
    bool OnKeyUp(const Event::KeyInput& input) { mKey[input.mKey] = false; return true; }
