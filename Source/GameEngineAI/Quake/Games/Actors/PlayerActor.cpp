...
unsigned int PlayerActor::GetFreeHudID()
{
...
unsigned int PlayerActor::AddHud(HudElement* toadd)
{
...
HudElement* PlayerActor::GetHud(unsigned int id)
{
	if (id < mHud.size())
		return mHud[id];
...
HudElement* PlayerActor::RemoveHud(unsigned int id)
{
...
void PlayerActor::BeginWeaponChange()
{
	...
	mState.weaponTime += 0.2f;
	StartTorsoAnim(TORSO_DROP);
	EventManager::Get()->TriggerEvent(
		std::make_shared<EventDataChangeWeapon>(GetId()));
}
...
bool PlayerActor::IsChangingWeapon()
{
	return mAction.weaponSelect != WP_NONE ? 
		mAction.weaponSelect != mState.weapon : false;
}
...
void PlayerActor::PlayerSpawn()
{
	...
	// increment the spawncount so the visual will detect the respawn
	...
	EventManager::Get()->TriggerEvent(
		std::make_shared<EventDataSpawnActor>(GetId(), Transform()));
}
...
void PlayerActor::UpdateWeapon(unsigned long deltaMs)
{
	...
	*/
	...
	if (mState.weaponTime > 0)
		mState.weaponTime -= deltaMs / 1000.f;
	...
	mState.weaponTime += addTime / 1000.f;
}

void PlayerActor::UpdateMovement(const Vector3<float>& velocity, const Vector3<float>& fall)
{
	...
	if (mAction.actionType & ACTION_JUMP)
	{
		...
		EventManager::Get()->TriggerEvent(
			std::make_shared<EventDataJumpActor>(GetId(), velocity, fall));
	}
	...
	if (mAction.actionType & ACTION_RUN)
	{
		...
		EventManager::Get()->TriggerEvent(
			std::make_shared<EventDataMoveActor>(GetId(), velocity, fall));
	}
	else
	{
		// if not trying to move
		...
		EventManager::Get()->TriggerEvent(
			std::make_shared<EventDataFallActor>(GetId(), fall));
	}
}
