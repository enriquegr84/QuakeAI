...
// Local
//---------------------------------------------------------------------------------------------------------------------
Local::Local() : BaseMenu()
{
    mSelectedGame = Settings::Get()->Get("selected_game");
...
std::string Local::GetForm()
{
    ...
    if (!mSelectedGame.empty())
    {
        levels = GameLogic::Get()->GetLevelManager()->GetLevels();
        for (std::vector<Level*>::iterator it = levels.begin(); it != levels.end(); ++it)
            form += ToString((*it)->GetName()) + ",";
    }

    if (levels.size()) form = form.substr(0, form.size() - 1);
    form += ";" + std::to_string(levels.size()) + "]";
    ...
    std::vector<Subgame> games = GetAvailableGames();
    for (const auto& game : games)
    {
        form += "image_button[" +
            std::to_string(box.mCenter[0]) + "," + std::to_string(box.mCenter[1]) + ";" +
            std::to_string(box.mExtent[0]) + "," + std::to_string(box.mExtent[1]) + ";" +
            game.mIconPath + ";game_btnbar_" + game.mId + ";;true;false]tooltip[game_btnbar_" + 
            game.mId + ";" + game.mName + "]";
    ...
    form += "image_button[" +
        std::to_string(box.mCenter[0]) + "," + std::to_string(box.mCenter[1]) + ";" +
        std::to_string(box.mExtent[0]) + "," + std::to_string(box.mExtent[1]) + ";" +
        "art/quake/textures/plus.png;game_open_cdb;;true;false]"
...
bool Local::Handle(std::string name, BaseUIElement* element)
{
    ...
    if (name == "play")
    {
        ...
        if (button)
        {
            ...
            if (selection)
            {
                ...
                std::string levelId = ToString(levels[selection - 1]->GetId().c_str());

                std::vector<WorldSpec> worldSpecs = GetAvailableWorlds();
                for (WorldSpec worldSpec : worldSpecs)
                {
                    if (worldSpec.mName != levelId)
                        continue;

                    Settings conf;
                    std::string confPath = worldSpec.mPath + "/world.qk";
                    bool succeeded = conf.ReadConfigFile(confPath.c_str());
                    if (!succeeded)
                        return false;

                    Settings::Get()->Set("selected_world", conf.Get("world_name"));

                    //Update last game
                    Settings::Get()->Set("selected_game", mSelectedGame);
    ...
    {
        std::shared_ptr<EventDataOpenContentStore> pOpenContentStore(
            new EventDataOpenContentStore());
    ...
    {
        /*
        QuakeMainMenuUI* mainMenu = reinterpret_cast<QuakeMainMenuUI*>(mUI);
    ...
    if (name.find("game_btnbar_") != std::string::npos)
    {
        ...
        for (const auto& game : games)
        {
            if (name == "game_btnbar_" + game.mId)
            {
                Settings::Get()->SetInt("mainmenu_last_selected_world", 1);
                Settings::Get()->Set("selected_game", game.mId);
                mSelectedGame = game.mId;

                std::shared_ptr<EventDataChangeGameSelection> pChangeGameSelection(
                    new EventDataChangeGameSelection(game.mId, game.mName, game.mRelativePath));
