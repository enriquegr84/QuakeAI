...
#define	CHAR_OFFSET_WIDTH   32
#define	CHAR_OFFSET_HEIGHT  48
...
private:
    void DrawScoreStat(Vector2<int> pos, int step, uint16_t drawdir, SColor bgColor,
        const std::string& score, int item, Vector2<int> offset, Vector2<int> size = Vector2<int>());
    void DrawStatbar(
        Vector2<int> pos, uint16_t corner, uint16_t drawdir,
        const std::string& texture, int number, int item,
        Vector2<int> offset, Vector2<int> size = Vector2<int>());
    ...
    float mHudScaling; // cached quake setting
    float mScaleFactor;
