...
void AIFinder::Destroy(void)
{
	...
	Concurrency::parallel_for_each(mNodes.begin(), mNodes.end(), [&](auto& plan)
	//for (ActorToAIPlanNodeMap::iterator it = mNodes.begin(); it != mNodes.end(); ++it)
	{
		Concurrency::parallel_for_each(plan.second.begin(), plan.second.end(), [&](auto& planNode)
	...
	std::map<PathingActorVec, float>& actorsPathPlans, unsigned int pathingType)
{
	...
	while (!mOpenSet.empty())
	{
		...
		const PathingActorMap& neighbors = planNode->GetPathingActor() ?
		...
		Concurrency::parallel_for_each(neighbors.begin(), neighbors.end(), [&](auto& neighbor)
		//for (auto it = neighbors.begin(); it != neighbors.end(); ++it)
		{
			PathingActor* pActorToEvaluate = neighbor.second;
	...
	PlayerData& otherPlayerData, const PathingArcVec& otherPlayerPathPlan, float otherPlayerPathOffset)
{
	...
	if (playerData.plan.weight - playerPathOffset < otherPlayerData.plan.weight - otherPlayerPathOffset)
	{
		...
		CalculateVisibility(threatLevel,
	...
	else
	{
		...
		CalculateVisibility(threatLevel, playerData.plan.node, playerPathOffset, playerVisibleTime, playerPathPlan, playerVisibility,
			otherPlayerData.plan.node, otherPlayerPathOffset, otherPlayerVisibleTime, otherPlayerPathPlan, otherPlayerVisibility);
	}
	...
	CalculateHeuristic(evaluation, threatLevel, playerData, otherPlayerData);
}
	...
	Concurrency::concurrent_unordered_map<unsigned long long, PathingArcVec>& otherClusterNodePathPlans)
{
	...
	{
		threatLevel = 4; // maximum threat level if there is at least one close visible node
		...
		for (size_t maxClusters = 30; maxClusters <= 90; maxClusters += 30)
		{
			...
			unsigned short visibleClustersCount = 0;
	...
	Concurrency::concurrent_unordered_map<unsigned long long, PathingArcVec>& otherClusterNodePathPlans)
{
	...
	std::map<PathingCluster*, PathingArcVec> clusterPaths, jumpClusterPaths;
	std::map<PathingCluster*, float> clusterPathWeights, jumpClusterPathWeights;
	clusterNodeStart->GetClusters(AT_MOVE, 150, clusterPaths, clusterPathWeights);
	for (auto& clusterPath : clusterPaths)
		clusterNodes[clusterPath.first->GetTarget()->GetCluster()] = clusterPath.first->GetTarget();
	//we will only consider jumps which are not reachable on moving
	clusterNodeStart->GetClusters(AT_JUMP, 150, jumpClusterPaths, jumpClusterPathWeights);
	for (auto& jumpClusterPath : jumpClusterPaths)
	{
		if (clusterNodes.find(jumpClusterPath.first->GetTarget()->GetCluster()) == clusterNodes.end())
		{
	...
	otherClusterNodeStart->GetClusters(AT_MOVE, 150, otherClusterPaths, otherClusterPathWeights);
	for (auto& otherClusterPath : otherClusterPaths)
		otherClusterNodes[otherClusterPath.first->GetTarget()->GetCluster()] = otherClusterPath.first->GetTarget();
	//we will only consider jumps which are not reachable on moving
	otherClusterNodeStart->GetClusters(AT_JUMP, 150, jumpOtherClusterPaths, jumpOtherClusterPathWeights);
	...
	if (visibleClusters.size())
	{
		threatLevel = 1; // lower threat level if there is at least one visible node
	...
	else threatLevel = 0; // no threat level if there is no visible node
	...
	Concurrency::concurrent_unordered_map<unsigned long long, PathingArcVec>& otherClusterNodePathPlans)
{
	...
	unsigned int skipPathingClusters = 120;
	std::unordered_map<PathingNode*, std::unordered_map<PathingCluster*, unsigned int>> pathingClusterNodes;
	std::multimap<float, PathingCluster*, std::less<float>> closestClusterPathWeights;
	for (auto& clusterPathWeight : clusterPathWeights)
		closestClusterPathWeights.insert({ clusterPathWeight.second, clusterPathWeight.first });
	...
	auto itCluster = closestClusterPathWeights.begin();
	if (closestClusterPathWeights.size() > 120)
	{
		itCluster = std::next(closestClusterPathWeights.begin(),
			std::min((size_t)skipPathingClusters, closestClusterPathWeights.size()));
	}
	...
	auto itOtherCluster = otherClosestClusterPathWeights.begin();
	if (otherClosestClusterPathWeights.size() > 120)
	{
		itOtherCluster = std::next(otherClosestClusterPathWeights.begin(),
			std::min((size_t)skipPathingClusters, otherClosestClusterPathWeights.size()));
	}
	...
	Concurrency::concurrent_unordered_map<unsigned long long, float>& actorPathPlanClusterHeuristics)
{
	...
	//for (auto& bestClusterPath : bestClusterPaths)
	{
		...
		//lets try to add surrounding clusters
		std::unordered_set<PathingNode*> pathingClusterNodes;
		std::map<PathingCluster*, PathingArcVec> pathingClusters;
		std::multimap<float, PathingCluster*, std::less<float>> pathingClusterWeights;
		...
		for (auto itCluster = pathingClusterWeights.begin(); itCluster != pathingClusterWeights.end(); ++itCluster)
		{
			...
			pathingClusterNodes.insert(pathingClusterNodeEnd);
		...
		pathingClusters.clear();
		pathingClusterWeights.clear();
		actorPathNode->GetClusters(AT_JUMP, 30, pathingClusters, pathingClusterWeights);
		for (auto itCluster = pathingClusterWeights.begin(); itCluster != pathingClusterWeights.end(); ++itCluster)
		{
			PathingNode* pathingClusterNodeEnd = graph->FindClusterNode((*itCluster).second->GetTarget()->GetCluster());
			if (clusterNodeEnd == pathingClusterNodeEnd)
			...
			if (pathingClusterNodes.find(pathingClusterNodeEnd) != pathingClusterNodes.end())
				continue;
...
bool QuakeAIManager::SimulatePlayerDecision(const PlayerData& playerDataIn, PlayerData& playerDataOut,
	const PlayerData& otherPlayerDataIn, PlayerData& otherPlayerDataOut, unsigned short& threatLevel,
	const std::map<ActorId, float>& gameItems, ActorId playerEvaluation, EvaluationType evaluation)
{
	PathingNode* clusterNodeStart = playerDataIn.plan.node;
...
bool QuakeAIManager::MakeAIGuessing(PlayerView& aiView)
{
	...
	if (aiSimulation.data.plan.node)
	{
		...
		for (auto const& aiGuessItem : aiView.data.items)
		{
			...
			if (itemPickup)
				if (!aiSimulation.data.plan.node->IsVisibleNode(itemPickup->GetNode()) && gameItems[aiGuessItem.first] <= 0.f)
		...
		for (auto const& humanGuessItem : playerGuessView.items)
		{
			...
			if (itemPickup)
				if (!aiSimulation.data.plan.node->IsVisibleNode(itemPickup->GetNode()) && gameItems[humanGuessItem.first] <= 0.f)
...
bool QuakeAIManager::MakeAIGuessingDecision(PlayerView& aiView)
{
	...
	if (playerGuessSimulation.data.plan.node)
	{
		...
		for (auto const& aiGuessItem : playerGuessView.guessItems[mPlayers[GV_AI]])
		{
			const AIAnalysis::ActorPickup* itemPickup = mGameActorPickups.at(aiGuessItem.first);
			if (itemPickup)
				if (!playerGuessSimulation.data.plan.node->IsVisibleNode(itemPickup->GetNode()) && gameItems[aiGuessItem.first] <= 0.f)
					gameItems[aiGuessItem.first] = aiGuessItem.second;
		}
	}
...
bool QuakeAIManager::MakeHumanGuessing(PlayerView& playerView)
{
	...
	if (playerSimulation.data.plan.node)
	{
		...
		for (auto const& humanGuessItem : playerView.data.items)
		{
			const AIAnalysis::ActorPickup* itemPickup = mGameActorPickups.at(humanGuessItem.first);
			if (itemPickup)
				if (!playerSimulation.data.plan.node->IsVisibleNode(itemPickup->GetNode()) && gameItems[humanGuessItem.first] <= 0.f)
					gameItems[humanGuessItem.first] = humanGuessItem.second;
		}
		...
		for (auto const& aiGuessItem : aiGuessView.items)
		{
			const AIAnalysis::ActorPickup* itemPickup = mGameActorPickups.at(aiGuessItem.first);
			if (itemPickup)
				if (!playerSimulation.data.plan.node->IsVisibleNode(itemPickup->GetNode()) && gameItems[aiGuessItem.first] <= 0.f)
					gameItems[aiGuessItem.first] = aiGuessItem.second;
		}
	}
...
bool QuakeAIManager::MakeHumanGuessingDecision(PlayerView& playerView)
{
	...
	if (aiGuessSimulation.data.plan.node)
	{
		...
		for (auto const& humanGuessItem : aiGuessView.guessItems[mPlayers[GV_HUMAN]])
		{
			const AIAnalysis::ActorPickup* itemPickup = mGameActorPickups.at(humanGuessItem.first);
			if (itemPickup)
				if (!aiGuessSimulation.data.plan.node->IsVisibleNode(itemPickup->GetNode()) && gameItems[humanGuessItem.first] <= 0.f)
					gameItems[humanGuessItem.first] = humanGuessItem.second;
		}
	}
	...
	while (true)
	{
		if (GameLogic::Get()->GetState() == BGS_RUNNING)
		{
			...
			if (isAIGuessing)
			{
				...
				unsigned int diffTime = Timer::GetRealTime() - time;
				std::stringstream ss;
				ss << "\n ai close guessing total elapsed time " << diffTime;// << " threat level " << aiView.threat;
				PrintInfo(ss.str());
				printf(ss.str().c_str());
	...
	while (true)
	{
		if (GameLogic::Get()->GetState() == BGS_RUNNING)
		{
			...
			if (isAIDecision)
			{
				...
				unsigned int diffTime = Timer::GetRealTime() - time;
				std::stringstream ss;
				ss << "\n ai aware decision total elapsed time " << diffTime;// << " threat level " << aiView.threat;
				PrintInfo(ss.str());
				printf(ss.str().c_str());
	...
	while (true)
	{
		if (GameLogic::Get()->GetState() == BGS_RUNNING)
		{
			...
			if (isHumanDecision)
			{
				...
				unsigned int diffTime = Timer::GetRealTime() - time;
				std::stringstream ss;
				ss << "\n human close guessing total elapsed time " << diffTime;// << " threat level " << playerView.threat;
				PrintInfo(ss.str());
				printf(ss.str().c_str());
	...
	while (true)
	{
		if (GameLogic::Get()->GetState() == BGS_RUNNING)
		{
			...
			if (isHumanDecision)
			{
				...
				unsigned int diffTime = Timer::GetRealTime() - time;
				std::stringstream ss;
				ss << "\n human aware decision total elapsed time " << diffTime;// << " threat level " << playerView.threat;
				PrintInfo(ss.str());
				printf(ss.str().c_str());
...
void QuakeAIManager::GetPlayerInput(const AIAnalysis::PlayerInput& playerInput, PlayerData& playerData)
{
	...
	playerData.planWeight = playerInput.planWeight;
...
void QuakeAIManager::SetPlayerInput(AIAnalysis::PlayerInput& playerInput, const PlayerData& playerData, const PlayerData& playerDataOffset)
{
	...
	playerInput.planWeight = playerDataOffset.planWeight;
	...
	if (pPlayerActor)
	{
		...
		if (pPhysicComponent)
		{
			...
			if (mPathingGraph)
			{
				...
				spawnNode = mPathingGraph->FindClosestNode(spawnTransform.GetTranslation(), false);
				...
				for (std::shared_ptr<PlayerActor> pOtherPlayerActor : playerActors)
				{
					...
					PathingNode* guessSpawnNode = mPathingGraph->FindClosestNode(guessSpawnTransform.GetTranslation(), false);
...
void QuakeAIManager::CalculateHeuristic(EvaluationType evaluation, unsigned short threatLevel, PlayerData& playerData, PlayerData& otherPlayerData)
{
	float heuristic = 0.f;
	int maxDamage = threatLevel > 1 ? 500 : 400;
	...
	if (playerData.weapon != WP_NONE)
	{
		if (playerData.damage[playerData.weapon - 1] >= maxDamage)
		{
	...
	{
		if (otherPlayerData.damage[otherPlayerData.weapon - 1] >= maxDamage)
		{
	...
	if (CalculatePlayerStatus(playerData) > 0.3f)
	{
		if (CalculatePlayerStatus(otherPlayerData) > 0.3f)
		{
			//remains the same
			playerDamage = playerMaxDamage > maxDamage ? (float)maxDamage : (float)playerMaxDamage;
			otherPlayerDamage = otherPlayerMaxDamage > maxDamage ? (float)maxDamage : (float)otherPlayerMaxDamage;
		}
		else if (CalculatePlayerStatus(otherPlayerData) > 0.15f)
		{
			if (CalculatePlayerWeaponStatus(playerData) > 0.f)
			{
				playerDamage = playerMaxDamage > maxDamage ? (float)maxDamage : (float)playerMaxDamage;
				otherPlayerDamage = otherPlayerMaxDamage * 0.25f > maxDamage ? (float)maxDamage : (float)otherPlayerMaxDamage * 0.25f;
			}
			else if (CalculatePlayerWeaponStatus(otherPlayerData) > 0.3f)
			{
				//remains the same
				playerDamage = playerMaxDamage > maxDamage ? (float)maxDamage : (float)playerMaxDamage;
				otherPlayerDamage = otherPlayerMaxDamage > maxDamage ? (float)maxDamage : (float)otherPlayerMaxDamage;
	...
	else if (CalculatePlayerStatus(playerData) > 0.15f)
	{
		if (CalculatePlayerStatus(otherPlayerData) > 0.3f)
		{
			if (CalculatePlayerWeaponStatus(otherPlayerData) > 0.f)
			{
				playerDamage = playerMaxDamage * 0.25f > maxDamage ? (float)maxDamage : (float)playerMaxDamage * 0.25f;
				otherPlayerDamage = otherPlayerMaxDamage > maxDamage ? (float)maxDamage : (float)otherPlayerMaxDamage;
			}
			else if (CalculatePlayerWeaponStatus(playerData) > 0.3f)
			{
				//remains the same
				playerDamage = playerMaxDamage > maxDamage ? (float)maxDamage : (float)playerMaxDamage;
				otherPlayerDamage = otherPlayerMaxDamage > maxDamage ? (float)maxDamage : (float)otherPlayerMaxDamage;
	...
	float heuristicDamage = (playerDamage - otherPlayerDamage) / (float)maxDamage;
			...
			switch (weapon)
			{
				case WP_LIGHTNING:
					...
					for (itVisibility = visibility.begin(); itVisibility != visibility.end(); itVisibility++)
					{
						...
						if ((*itVisibility).first < weaponTime)
							continue;

						// lets assign a firing threshold
						if ((*itVisibility).second.moveTime > 0.1f)
						{
							if ((*itVisibility).second.moveDistance <= 800)
							{
				...
				case WP_RAILGUN:
					...
					for (itVisibility = visibility.begin(); itVisibility != visibility.end(); itVisibility++)
					{
						...
						if ((*itVisibility).second.moveTime >= 0.1f)
						{
							...
							shotCount = shotCount > ammo ? ammo : shotCount;
							ammo -= shotCount;
							if ((*itVisibility).second.moveDistance > 500)
								playerData.damage[weapon - 1] += (damage + 50) * shotCount;
							else if ((*itVisibility).second.moveDistance < 300)
								playerData.damage[weapon - 1] += (damage - 50) * shotCount;
							else
								playerData.damage[weapon - 1] += damage * shotCount;
						}
...
void QuakeAIManager::CalculateVisibility(unsigned short threatLevel,
	...
	const PathingArcVec& otherPlayerPathPlan, std::map<float, VisibilityData>& otherPlayerVisibility)
{
	...
	for (; otherPathIndex < otherPlayerPathPlan.size(); otherPathIndex++)
	{
		...
		const PathingNodeVec& otherTransitionNodes = otherCurrentArc->GetTransition()->GetNodes();
		const std::vector<Vector3<float>>& otherTransitionPositions = otherCurrentArc->GetTransition()->GetPositions();
	...
	for (; itPathPlan != playerPathPlan.end(); itPathPlan++)
	{
		...
		const PathingNodeVec& transitionNodes = currentArc->GetTransition()->GetNodes();
		const std::vector<Vector3<float>>& transitionPositions = currentArc->GetTransition()->GetPositions();
	...
	float weightLimit = threatLevel > 1 ? 4.f : 3.f;
	...
	if (otherCurrentArc != NULL)
	{
		...
		{
			currentArc = *itPathPlan;
			std::reference_wrapper<const PathingNodeVec> transitionNodes = currentArc->GetTransition()->GetNodes();
			std::reference_wrapper<const PathingNodeVec> otherTransitionNodes = otherCurrentArc->GetTransition()->GetNodes();
			std::reference_wrapper<const std::vector<Vector3<float>>> transitionPositions = currentArc->GetTransition()->GetPositions();
			std::reference_wrapper<const std::vector<Vector3<float>>> otherTransitionPositions = otherCurrentArc->GetTransition()->GetPositions();
			...
			for (; index < transitionPositions.get().size(); index++)
			{
				...
				while (totalArcWeight <= totalWeight)
				{
					...
					if (otherIndex + 1 >= otherTransitionPositions.get().size())
					{
				...
				totalWeight += currentWeight;
				// set weightlimit; any time further is likely to be unrealistic simulation
	...
	float visibleWeightLimit = threatLevel > 1 ? 2.f : 1.5f;
	if (totalVisibleWeight < visibleWeightLimit)
	{
			...
			/*if (RayCollisionDetection(currentNode->GetPosition(), otherCurrentNode->GetPosition()) == NULL)*/
		{
			...
			visibleWeightLimit = threatLevel > 1 ? 1.f : 0.75f;
			totalVisibleWeight = totalVisibleWeight < visibleWeightLimit ? 2.f : 1.f;
...
float QuakeAIManager::CalculatePathingWeight(const PlayerData& playerData)
{
	...
	if (playerData.plan.path.size())
	{
		...
		const std::vector<float>& transitionWeights = currentArc->GetTransition()->GetWeights();
		const std::vector<Vector3<float>>& transitionPositions = currentArc->GetTransition()->GetPositions();
	...
	WeaponType& playerWeapon, WeaponType& otherPlayerWeapon, unsigned long long& playerClusterCode, unsigned long long& otherPlayerClusterCode)
{
	...
	if (otherPlayerClusterCode != ULLONG_MAX && otherPlayerGuessingHeuristics.find(ULLONG_MAX) != otherPlayerGuessingHeuristics.end())
	{
		...
		if (abs(otherPlayerGuessingHeuristics.at(ULLONG_MAX) - otherPlayerClusterHeuristic) < 0.05f)
		{
	...
	WeaponType& playerWeapon, WeaponType& otherPlayerWeapon, unsigned long long& playerClusterCode, unsigned long long& otherPlayerClusterCode)
{
	...
	{
		//we keep the current plan if the heuristic is close to the best other player heuristic
		if (abs(otherPlayerDecisionHeuristics.at(ULLONG_MAX) - otherPlayerClusterHeuristic) < 0.05f)
		{
			otherPlayerClusterCode = ULLONG_MAX;
	...
	for (std::shared_ptr<PlayerActor> pPlayerActor : playerActors)
	{
		...
		for (std::shared_ptr<PlayerActor> pOtherPlayerActor : playerActors)
		{
			...
			if (playerGuessView.isUpdated)
			{
				...
				if (playerGuessView.data.plan.path.empty() || playerGuessView.threat > 3)
				...
				if (playerGuessView.data.plan.id != -1 && playerGuessView.data.IsWeaponSelectable(playerGuessView.simulation.weapon))
...
//jump pathing generation via physics simulation
void QuakeAIManager::CreatePathingJump(ActorId playerId, NodePlan& pathPlan, std::shared_ptr<PathingGraph>& graph)
{
	std::shared_ptr<BaseGamePhysic> gamePhysics = GameLogic::Get()->GetGamePhysics();
	...
	if (PathingNode* pStartNode = graph->FindClosestNode(transform.GetTranslation(), false))
	{
		if (PathingNode* pEndNode = SimulateJump(pStartNode, transform, graph))
		{
			int skipArc = AT_MOVE;
			PathPlan* plan = graph->FindPath(pStartNode, pEndNode, skipArc);
			if (plan)
			{
				pathPlan.ResetPathPlan(plan->GetArcs());
				pathPlan.node = pStartNode;
				delete plan;
			}

			SimulateStanding(INVALID_ACTOR_ID, pStartNode->GetPosition(), graph);
		}
	}
	mOpenSet.clear();
...
//run pathing generation via physics simulation
void QuakeAIManager::CreatePathingRun(ActorId playerId, NodePlan& pathPlan, std::shared_ptr<PathingGraph>& graph)
{
	std::shared_ptr<BaseGamePhysic> gamePhysics = GameLogic::Get()->GetGamePhysics();
	...
	if (PathingNode* pStartNode = graph->FindClosestNode(transform.GetTranslation(), false))
	{
		if (PathingNode* pEndNode = SimulateMove(pStartNode, transform, graph))
		{
			int skipArc = AT_JUMP;
			PathPlan* plan = graph->FindPath(pStartNode, pEndNode, skipArc);
			if (plan)
...
//fall pathing generation via physics simulation
void QuakeAIManager::CreatePathingFall(ActorId playerId, NodePlan& pathPlan, std::shared_ptr<PathingGraph>& graph)
{
	std::shared_ptr<BaseGamePhysic> gamePhysics = GameLogic::Get()->GetGamePhysics();
	...
	if (PathingNode* pStartNode = graph->FindClosestNode(transform.GetTranslation(), false))
	{
		if (PathingNode* pEndNode = SimulateFall(pStartNode, transform, graph))
		{
			int skipArc = AT_JUMP;
			PathPlan* plan = graph->FindPath(pStartNode, pEndNode, skipArc);
			if (plan)
...
PathingNode* QuakeAIManager::CreatePathingNode(ActorId playerId, std::shared_ptr<PathingGraph>& graph)
{
	...
	PathingNode* newNode = nullptr;
	if (mOpenSet.size())
	{
		newNode = mOpenSet.front();
		...
		for (auto const& pathArc : pathNode->GetArcs())
		{
			...
			if (pathTransition)
			{
				...
				for (const Vector3<float>& pathPosition : pathTransition->GetPositions())
					nodes.push_back(graph->FindClosestNode(pathPosition, false));
				for (const Vector3<float>& pathPosition : pathTransition->GetPositions())
...
void QuakeAIManager::CreateClusters(std::shared_ptr<PathingGraph>& graph, unsigned int totalClusters)
{
	...
	for (ActorId actor : searchActors)
	{
		...
		PathingNode* pNode = graph->FindClosestNode(transformComponent->GetPosition(), false);
	...
	for (Clustering kCluster : kmeans.GetClusters())
	{
		...
		PathingNode* clusterCenterNode = graph->FindClosestNode(
			Vector3<float>{kCluster.GetCenter(0), kCluster.GetCenter(1), kCluster.GetCenter(2)}, false);
	...
	PathingNode* pNode, const Transform& target, std::shared_ptr<PathingGraph>& graph)
{
	...
	std::vector<std::pair<Vector3<float>, float>> nodePositions;
	while (!gamePhysics->OnGround(mPlayerActor->GetId()) && totalTime <= 10.f)
	{
		totalTime += mSimulationStep;

		transform = gamePhysics->GetTransform(mPlayerActor->GetId());
		nodePositions.push_back({ transform.GetTranslation(), mSimulationStep });
		gamePhysics->OnUpdate(mSimulationStep);
	}
	transform = gamePhysics->GetTransform(mPlayerActor->GetId());

	//we store the falling position on ground
	std::vector<std::pair<Transform, bool>> interpolations;
	gamePhysics->GetInterpolations(mPlayerActor->GetId(), interpolations);
	for (auto const& interpolation : interpolations)
	{
		if (interpolation.second) // check we are on ground
		{
			transform = interpolation.first;
			break;
		}
	...
	if (deltaTime > 0.f)
	{
		...
		nodePositions.push_back({ transform.GetTranslation(), deltaTime });
	}
	...
	if (pNode != pEndNode)
	{
		if (pEndNode != NULL && pNode->FindArc(pEndNode) == NULL)
		{
			if (pEndNode->GetActorId() == INVALID_ACTOR_ID || pNode->GetActorId() != pEndNode->GetActorId())
			{
				...
				{
					totalTime += nodePositions[idx].second;

					if (Length(nodePositions[idx].first - position) >= FLOATING_DISTANCE)
					{
						nodes.push_back(pNode);
						weights.push_back(totalTime);
						positions.push_back(nodePositions[idx].first);

						totalTime = 0.f;
						position = nodePositions[idx].first;
					}
				}
	...
	PathingNode* pNode, const Transform& target, std::shared_ptr<PathingGraph>& graph)
{
	...
	float push = mPushSpeed[AXIS_Y] + direction[AXIS_Y] * 0.004f;
...
PathingNode* QuakeAIManager::SimulateMove(PathingNode* pNode, Transform transform, std::shared_ptr<PathingGraph>& graph)
{
	std::shared_ptr<BaseGamePhysic> gamePhysics = GameLogic::Get()->GetGamePhysics();
				...
				if (Length(diff) >= GROUND_DISTANCE)
				{
					...
					if (!collision.has_value() || actorId != INVALID_ACTOR_ID)
					{
							...
							!CheckPenetration(mPlayerActor->GetId(), (*itMove).first.GetTranslation()))
						{
							...
							if (totalTime >= 0.02f && totalTime <= 0.2f)
							{
...
PathingNode* QuakeAIManager::SimulateJump(PathingNode* pNode, Transform transform, std::shared_ptr<PathingGraph>& graph)
{
	std::shared_ptr<BaseGamePhysic> gamePhysics = GameLogic::Get()->GetGamePhysics();
...
void QuakeAIManager::SimulateFall(PathingNode* pNode, std::shared_ptr<PathingGraph>& graph)
{
	...
	for (int angle = 0; angle < 360; angle += 5)
	{
		...
		float deltaTime = 0.f;
		while (gamePhysics->OnGround(mPlayerActor->GetId()) && deltaTime <= 0.1f)
		{
			deltaTime += mSimulationStep;
		...
		if (pNode != pEndNode && pEndNode && visitedNodes.find(pEndNode) == visitedNodes.end())
		{
			...
			deltaTime = 0.f;
			transform = gamePhysics->GetTransform(mPlayerActor->GetId());
			std::vector<std::pair<Vector3<float>, float>> nodePositions { { transform.GetTranslation(), deltaTime } };
			while (gamePhysics->OnGround(mPlayerActor->GetId()) && deltaTime <= 0.1f)
			{
				deltaTime += mSimulationStep;
...
PathingNode* QuakeAIManager::SimulateFall(PathingNode* pNode, Transform transform, std::shared_ptr<PathingGraph>& graph)
{
	std::shared_ptr<BaseGamePhysic> gamePhysics = GameLogic::Get()->GetGamePhysics();
