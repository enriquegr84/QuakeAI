...
//----------------------------------------------------------------------------
QuakeApp::QuakeApp()
	: GameApplication("Quake", 0, 0, 1024, 600, { 0.392f, 0.584f, 0.929f, 1.0f })
{
}
...
bool QuakeApp::OnInitialize()
{
	if (!GameApplication::OnInitialize())
		return false;

	std::string config = Application::ApplicationPath + "../../Assets/Config/Quake/quake.conf";
	Settings::Get()->ReadConfigFile(config.c_str());
	return true;
}

void QuakeApp::OnTerminate()
{
...
// QuakeApp::LoadGame
//
bool QuakeApp::LoadGame()
{
	// Read the game options and see what the current game
	// needs to be - all of the game graphics are initialized by now, too...
	return GameLogic::Get()->LoadGame(Settings::Get()->Get("selected_world").c_str());
}

//
// QuakeLogic::AddView
//
void QuakeApp::AddView(const std::shared_ptr<BaseGameView>& pView)
{
...
//
void QuakeApp::RemoveView(const std::shared_ptr<BaseGameView>& pView)
{
...
{
	REGISTER_EVENT(EventDataSplashDamage);
	REGISTER_EVENT(EventDataTeleportActor);
	REGISTER_EVENT(EventDataSpawnActor);
	REGISTER_EVENT(EventDataPushActor);
	REGISTER_EVENT(EventDataJumpActor);
	REGISTER_EVENT(EventDataMoveActor);
	REGISTER_EVENT(EventDataFallActor);
