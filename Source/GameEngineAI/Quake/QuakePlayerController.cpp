	...
	: mTarget(target), mEnabled(true)
{
	...
	mMaxPushSpeed = Vector3<float>{12.f, 12.f, 28.f };
	mMaxJumpSpeed = Vector3<float>{12.f, 12.f, 16.f };
	mMaxFallSpeed = Vector3<float>{30.f, 30.f, 60.f };
	mMaxMoveSpeed = 300.f;
			...
			if (pPhysicComponent)
			{
				...
				if (pPhysicComponent->OnGround())
				{
					...
					if (pPlayerActor->GetAction().triggerPush != INVALID_ACTOR_ID)
					{
						...
						float push = mPushSpeed[AXIS_Y] + direction[AXIS_Y] * 0.05f;
						direction[AXIS_Y] = 0;
						Normalize(direction);
					...
					else if (mEnabled)
					{
						if (mMouseRButtonDown)
						{
							...
							//printf("\n character jumping %f %f %f elpased %u", velocity[0], velocity[1], velocity[2], deltaMs);
							pPlayerActor->GetAction().actionType |= ACTION_JUMP;
						}
						else
						{
