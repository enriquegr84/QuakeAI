...
GameDemoCameraController::GameDemoCameraController(const std::shared_ptr<CameraNode>& camera,
...
void GameDemoCameraController::OnUpdate(unsigned int timeMs, unsigned long deltaMs)
{
	...
	{
		mPitch = std::max(-89.f, std::min(89.f, mPitch));
	...
	if (mKey[KEY_KEY_W] || mKey[KEY_KEY_S])
	{
		...
		// the camera.
		atWorld = Vector4<float>::Unit(2); // forward vector
	...
	if (mKey[KEY_SPACE] || mKey[KEY_KEY_C] || mKey[KEY_KEY_X])
	{
		...
		upWorld = Vector4<float>::Unit(1); // up vector
#if defined(GE_USE_MAT_VEC)
