...
// End obsolete stuff
	ACTIVEOBJECT_TYPE_ENTITY = 7,
// Special type, not stored as a static object
	ACTIVEOBJECT_TYPE_PLAYER = 100,
// Special type, only exists as VAO
...
{
	ActiveObjectMessage(uint16_t aId, bool aReliable=true, const std::string& aData="") :
		id(aId), reliable(aReliable), data(aData)
	{}
...
/*
	Parent class for LogicActiveObject and VisualActiveObject
*/
class ActiveObject
{
public:
	ActiveObject(ActorId id = INVALID_ACTOR_ID) : mId(id) { }

	ActorId GetId() const { return mId; }

	void SetId(ActorId id) { mId = id; }
	 ...
	 */
	virtual bool GetCollisionBox(BoundingBox<float>* toset) const = 0;
	 ...
	 */
	virtual bool GetSelectionBox(BoundingBox<float>* toset) const = 0;


	virtual bool CollideWithObjects() const = 0;


	virtual void SetAttachment(int parentId, const std::string& bone, Vector3<float> position,
			Vector3<float> rotation, bool forceVisible) {}
	virtual void GetAttachment(int* parentId, std::string* bone, Vector3<float>* position,
			Vector3<float>* rotation, bool* forceVisible) const {}
	virtual void ClearChildAttachments() {}
	virtual void ClearParentAttachment() {}
	virtual void AddAttachmentChild(int childId) {}
	virtual void RemoveAttachmentChild(int childId) {}

protected:
	ActorId mId; // 0 is invalid, "no id"
};
