...
bool Content::Handle(std::string name, BaseUIElement* element)
{
    ...
    {
        Level* selectedLevel = GameLogic::Get()->GetLevelManager()->GetLevel((unsigned int)mSelectedLevel-1);

        std::shared_ptr<EventDataDeleteContentStore> pDeleteContentStore(
            new EventDataDeleteContentStore(ToString(selectedLevel->GetName())));
