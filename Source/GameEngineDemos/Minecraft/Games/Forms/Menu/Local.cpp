...
bool Local::Handle(std::string name, BaseUIElement* element)
{
    ...
    {
        std::shared_ptr<EventDataOpenContentStore> pOpenContentStore(new EventDataOpenContentStore());
        ...
        for (std::vector<Level*>::iterator it = levels.begin(); it != levels.end(); ++it)
        {
            if (name == "game_btnbar_" + ToString((*it)->GetId()))
            {
                ...
                std::shared_ptr<EventDataChangeGameSelection> pChangeGameSelection(
                    new EventDataChangeGameSelection(ToString(mLevelId), ToString(mLevel)));
