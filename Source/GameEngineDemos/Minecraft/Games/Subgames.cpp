...
Subgame FindSubgame(const std::string& id)
{
    ...
    std::string gameBase = "/../../Assets/Art/Minecraft/";
	...
	for (const GameFindPath& findPath : findPaths) 
    {
        ...
        {
			gamePath = tryPath;
			gameRelativePath = "Art/Minecraft/games/" + id;
    ...
    std::string gameModPath = share + "/../../Assets/Actors/Minecraft/Mods/" + id;

	// Find mod directories
    std::set<std::string> modsPaths;
	if (!userGame)
		modsPaths.insert(share + "/../../Assets/Actors/Minecraft/Mods/");
    ...
    std::string menuIconPath = GetImagePath(gamePath + "/menu/icon.png");
...
{
	std::wstring path = 
		FileSystem::Get()->GetWorkingDirectory() + L"/../../Assets/Art/Minecraft/games/";
...
std::vector<WorldSpec> GetAvailableWorlds()
{
    ...
    std::string share = ToString(FileSystem::Get()->GetWorkingDirectory());
	worldspaths.insert(share + "/../../Assets/Art/Minecraft/games");
	for (const std::string& worldspath : worldspaths) 
    {
		LogInformation("Searching world in " + worldspath + " : ");

		const std::set<char> ignore = { '.' };
		std::vector<std::wstring> dirs;
		FileSystem::Get()->GetFileList(dirs, ToWideString(worldspath), true);
		for (const std::wstring& dir : dirs)
		{
			std::string fileName = ToString(FileSystem::Get()->GetFileName(dir));
			if (ignore.count(fileName[0]))
			...
			if (FileSystem::Get()->ExistDirectory(dir))
			{
				std::string fullpath = ToString(dir) + "/map";
				std::string name = GetWorldName(fullpath, ToString(dir));
    ...
    const std::string& name, const Subgame& game, bool createWorld)
{
    ...
    for (const auto& name : gameSettings->GetNames()) 
    {
        if (name.compare(0, 7, "secure.") != 0)
            continue;

        LogWarning("Secure setting " + name + " isn't allowed, so was ignored.");
        gameSettings->Remove(name);
    }

	LogInformation(L"Initializing world at " + finalPath);

	//fs::CreateAllDirs(finalPath);
	...
	if (!FileSystem::Get()->ExistFile(worldPath))
    {
		...
		conf.SetBool("creative_mode", Settings::Get()->GetBool("creative_mode"));
		conf.SetBool("enable_damage", Settings::Get()->GetBool("enable_damage"));
	...
	if (!FileSystem::Get()->ExistFile(mapMetaPath)) 
    {
		...
		params.ReadParams(Settings::Get());
