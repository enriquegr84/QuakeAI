...
#ifndef ANIMATEDOBJECTMESH_H
#define ANIMATEDOBJECTMESH_H
...
#include "Graphic/Scene/Element/ShadowVolumeNode.h"

enum GRAPHIC_ITEM JointAnimationUpdateOnRender
{
	//! do nothing
	JAUOR_NONE = 0,
...
**/
class AnimationObjectEndCallBack
{
public:
	...
	virtual void OnAnimationEnd(AnimatedObjectMeshNode* node) = 0;
};
...
class AnimatedObjectMeshNode : public Node
{
public:

	//! Constructor
	AnimatedObjectMeshNode(const ActorId actorId, unsigned int shaderId, 
		VisualEnvironment* env, PVWUpdater* updater, const std::shared_ptr<BaseAnimatedMesh>& mesh);

	//! Destructor
	virtual ~AnimatedObjectMeshNode();
	...
	void SetAnimationEndCallback(AnimationObjectEndCallBack* callback = 0);
	...
	std::shared_ptr<BoneNode> GetJointNode(const char* jointName);

	//! same as GetJointNode(const char* jointName), but based on id
	std::shared_ptr<BoneNode> GetJointNode(unsigned int jointID);

	//! Gets joint count.
	unsigned int GetJointCount() const;

	//! Set the joint update mode (0-unused, 1-get joints only, 2-set joints only, 3-move and set)
	void SetJointMode(JointAnimationUpdateOnRender mode);
	...
	std::shared_ptr<ShadowVolumeNode> AddShadowVolumeNode(const ActorId actorId, 
		Scene* pScene, const std::shared_ptr<BaseMesh>& shadowMesh = 0,
		bool zfailmethod=true, float infinity=10000.0f);

	//! Sets a new mesh
	void SetMesh(const std::shared_ptr<BaseAnimatedMesh>& mesh);

	//! Returns the current mesh
	const std::shared_ptr<BaseAnimatedMesh>& GetMesh(void);
...
private:
	...
	std::shared_ptr<BaseMesh> GetMeshForCurrentFrame();
	...
	std::shared_ptr<BaseMesh> mCurrentFrameMesh;
	std::shared_ptr<BaseAnimatedMesh> mMesh;
	...
	std::shared_ptr<AnimationObjectEndCallBack> mLoopCallBack;
	int mPassCount;

	std::shared_ptr<ShadowVolumeNode> mShadow;

	//0-unused, 1-get joints only, 2-set joints only, 3-move and set
	JointAnimationUpdateOnRender mJointMode;
	bool mJointsUsed;

	std::vector<std::shared_ptr<BoneNode>> mJointChildSceneNodes;
	std::vector<Transform> mPretransitingSave;
};
