...
#ifndef MINECRAFTACTORFACTORY_H
#define MINECRAFTACTORFACTORY_H
...
*/
class MinecraftActorFactory : public ActorFactory
{

public:
	MinecraftActorFactory(void);

    std::vector<std::shared_ptr<Actor>> CreateMods(LogicEnvironment* env,
        const wchar_t* modResource, const Transform* initialTransform);

    std::shared_ptr<LogicPlayer> CreatePlayer(BaseItemManager* itemMgr, const char* name,
		const wchar_t* actorResource, tinyxml2::XMLElement* overrides, const Transform* initialTransform);
    ...
    virtual std::shared_ptr<ActorComponent> CreateComponent(
