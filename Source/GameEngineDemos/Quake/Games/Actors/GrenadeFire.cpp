...
void GrenadeFire::Apply(std::weak_ptr<Actor> pActor)
{
	std::shared_ptr<Actor> pStrongActor(pActor);
	if (pStrongActor)
	{
		LogInformation("Applying grenade firing to actor id " + std::to_string(pStrongActor->GetId()));
	}
}
		...
		if (deltaMs >= mExplosionTime)
		{
			...
			if (pTransformComponent)
			{
				...
				EventManager::Get()->QueueEvent(
					std::make_shared<QuakeEventDataSplashDamage>(mOwner->GetId(), location));
			}
		}
