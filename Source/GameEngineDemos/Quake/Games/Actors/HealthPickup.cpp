...
void HealthPickup::Apply(std::weak_ptr<Actor> pActor)
{
	std::shared_ptr<Actor> pStrongActor(pActor);
    if (pStrongActor)
    {
        LogInformation("Applying health pickup to actor id " + std::to_string(pStrongActor->GetId()));
    }
}
...
void HealthPickup::Update(float deltaMs)
{
	...
	if (mRespawnTime)
	{
		...
		if (deltaMs >= mRespawnTime)
		{
			...
			std::shared_ptr<Node> node = pScene->GetSceneNode(mOwner->GetId());
