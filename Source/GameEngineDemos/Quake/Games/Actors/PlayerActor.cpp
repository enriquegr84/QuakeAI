...
void PlayerActor::BeginWeaponChange()
{
	...
	EventManager::Get()->TriggerEvent(
		std::make_shared<QuakeEventDataChangeWeapon>(GetId()));
}
...
void PlayerActor::PlayerSpawn()
{
	...
	EventManager::Get()->TriggerEvent(
		std::make_shared<QuakeEventDataSpawnActor>(GetId()));
}
...
void PlayerActor::UpdateWeapon(unsigned long deltaMs)
{
	...
	*/
	...
	// fire weapon
	EventManager::Get()->TriggerEvent(
		std::make_shared<QuakeEventDataFireWeapon>(GetId()));
...
void PlayerActor::UpdateMovement(const Vector3<float>& velocity)
{
	...
	if (mAction.actionType & ACTION_JUMP)
	{
		...
		EventManager::Get()->TriggerEvent(
			std::make_shared<QuakeEventDataJumpActor>(GetId(), velocity));
	}
	...
	if (mAction.actionType & ACTION_RUN)
	{
		...
		EventManager::Get()->TriggerEvent(
			std::make_shared<QuakeEventDataMoveActor>(GetId(), velocity));
	}
	else
	{
		// if not trying to move
		...
		EventManager::Get()->TriggerEvent(
			std::make_shared<QuakeEventDataFallActor>(GetId(), velocity));
	}
}
