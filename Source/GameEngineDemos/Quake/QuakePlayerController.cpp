...
void QuakePlayerController::OnUpdate(unsigned int timeMs, unsigned long deltaMs)
{
	...
	if (pPlayerActor->GetAction().triggerTeleporter != INVALID_ACTOR_ID)
	{
		...
		EventManager::Get()->TriggerEvent(
			std::make_shared<QuakeEventDataTeleportActor>(actorId));
	}
	else
	{
		...
		if (pPhysicComponent)
		{
			...
			if (pPhysicComponent->OnGround())
			{
				...
				else if (mEnabled)
				{
					...
					{
						upWorld = Vector4<float>::Unit(AXIS_Y);
			...
			{
				EventManager::Get()->TriggerEvent(
					std::make_shared<QuakeEventDataRotateActor>(actorId, mAbsoluteTransform));
