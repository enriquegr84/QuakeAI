...
/** SoundFetcher                                                          */
/******************************************************************************/
void MenuSoundFetcher::FetchSounds(const std::string& name,
	std::set<std::string>& dstPaths, std::set<std::string>& dstDatas)
{
	...
	std::string soundBase = share + "/../../Assets/Art/Quake/audio/";
...
{
	dstPaths.insert(base + "/sound/" + name + ".ogg");
	dstPaths.insert(base + "/sound/" + name + ".0.ogg");
	dstPaths.insert(base + "/sound/" + name + ".1.ogg");
	dstPaths.insert(base + "/sound/" + name + ".2.ogg");
	dstPaths.insert(base + "/sound/" + name + ".3.ogg");
	dstPaths.insert(base + "/sound/" + name + ".4.ogg");
	dstPaths.insert(base + "/sound/" + name + ".5.ogg");
	dstPaths.insert(base + "/sound/" + name + ".6.ogg");
	dstPaths.insert(base + "/sound/" + name + ".7.ogg");
	dstPaths.insert(base + "/sound/" + name + ".8.ogg");
	dstPaths.insert(base + "/sound/" + name + ".9.ogg");
}

void GameSoundFetcher::FetchSounds(const std::string& name,
	std::set<std::string>& dstPaths, std::set<std::string>& dstDatas)
{
	if (mFetched.count(name))
		return;
...
void QuakeMainMenuUI::UpdateMenuUI(std::wstring gameId)
{
	ClearMenuUI(L"overlay");
	ClearMenuUI(L"background");
	ClearMenuUI(L"header");
...
bool QuakeMainMenuUI::SetMenuUI(std::wstring id, std::wstring gameId)
{
	if (gameId.empty())
		return false;
	...
	std::wstring path =
		previousCWD + L"/../../Assets/Art/Quake/games/" + gameId + L"/menu";
	...
	path = L"Art/Quake/games/" + gameId + L"/menu/" + fileName;
	if (SetTexture(id, path, false, 16))
		return true;
...
{
	std::shared_ptr<QuakeEventDataOpenGameSelection> pCastEventData =
		std::static_pointer_cast<QuakeEventDataOpenGameSelection>(pEventData);

	std::shared_ptr<Local> local = std::make_shared<Local>();
	mQuakeMainMenuUI->SetMenu(local);
	mQuakeMainMenuUI->GetFormMenu()->SetForm(local->GetForm());
}

void QuakeMainMenuView::ChangeGameSelectionDelegate(BaseEventDataPtr pEventData)
{
	std::shared_ptr<QuakeEventDataChangeGameSelection> pCastEventData =
		std::static_pointer_cast<QuakeEventDataChangeGameSelection>(pEventData);

	mQuakeMainMenuUI->UpdateMenuUI(ToWideString(pCastEventData->GetGameId()));
	mQuakeMainMenuUI->SetTitleText(ToWideString(pCastEventData->GetGameName()));
	mQuakeMainMenuUI->GetFormMenu()->SetForm(mQuakeMainMenuUI->GetMenu()->GetForm());
}
...
void QuakeMainMenuView::RegisterAllDelegates(void)
{
	...
	pGlobalEventManager->AddListener(
		MakeDelegate(this, &QuakeMainMenuView::OpenContentStoreDelegate),
		QuakeEventDataOpenContentStore::skEventType);
	pGlobalEventManager->AddListener(
		MakeDelegate(this, &QuakeMainMenuView::DeleteContentStoreDelegate),
		QuakeEventDataDeleteContentStore::skEventType);

	pGlobalEventManager->AddListener(
		MakeDelegate(this, &QuakeMainMenuView::OpenGameSelectionDelegate),
		QuakeEventDataOpenGameSelection::skEventType);
	pGlobalEventManager->AddListener(
		MakeDelegate(this, &QuakeMainMenuView::ChangeGameSelectionDelegate),
		QuakeEventDataChangeGameSelection::skEventType);
}
	...
	float dTime, int percent, bool drawClouds)
{
	...
	if ((percent >= 0) && (percent <= 100))
	{
		std::string texturePath =
			ToString(FileSystem::Get()->GetWorkingDirectory()) + "/../../Assets/Art/Quake/textures/";
...
bool QuakeUI::OnRender(double time, float elapsedTime)
{
	...
	std::shared_ptr<PlayerActor> player(std::dynamic_pointer_cast<PlayerActor>(
		GameLogic::Get()->GetActor(mGameView->GetActorId()).lock()));
	if (player->GetState().hudFlags & HUD_FLAG_CROSSHAIR_VISIBLE)
		mHud->DrawCrosshair(L"art/quake/gfx/2d/crosshair1.png");
...
void DrawLoadScreen(const std::wstring& text, std::shared_ptr<BaseUI> ui,
	std::shared_ptr<Visual> visual, std::shared_ptr<BlendState> blendState, float dTime, int percent)
{
	Vector2<unsigned int> screenSize = Renderer::Get()->GetScreenSize();
	...
	{
		std::shared_ptr<Texture2> progressImg;
		if (FileSystem::Get()->ExistFile(ToWideString("progress_bar.png")))
		{
			std::shared_ptr<ResHandle> resHandle =
				ResCache::Get()->GetHandle(&BaseResource(ToWideString("progress_bar.png")));
			if (resHandle)
			{
				std::shared_ptr<ImageResourceExtraData> resData =
					std::static_pointer_cast<ImageResourceExtraData>(resHandle->GetExtra());
		...
		std::shared_ptr<Texture2> progressImgBG;
		if (FileSystem::Get()->ExistFile(ToWideString("progress_bar_bg.png")))
		{
			std::shared_ptr<ResHandle> resHandle =
				ResCache::Get()->GetHandle(&BaseResource(ToWideString("progress_bar_bg.png")));
			if (resHandle)
			{
				std::shared_ptr<ImageResourceExtraData> resData =
					std::static_pointer_cast<ImageResourceExtraData>(resHandle->GetExtra());
...
void TextureUpdateProgress(void* args, unsigned int progress, unsigned int maxProgress)
{
	...
	if (doDraw)
	{
		...
		DrawLoadScreen(strm.str(), targs->ui, targs->visual, targs->blendState, 
			0, 72 + (uint16_t)((18. / 100.) * (double)targs->lastPercent));
	}
}
...
void QuakeHumanView::Step(float dTime)
{
	...
	*/
	...
	if (mRemoveSoundsCheckTimer >= 2.32)
	{
		...
		// Sync to logic
		if (!removeIds.empty())
			EventManager::Get()->TriggerEvent(std::make_shared<QuakeEventDataRemoveSounds>(removeIds));
	}
}
...
bool QuakeHumanView::LoadGameDelegate(tinyxml2::XMLElement* pLevelData)
{
	...
	EventManager::Get()->TriggerEvent(std::make_shared<QuakeEventDataGameInit>());
	...
	mGameCameraController.reset(new QuakeCameraController(mCamera, 0, GE_C_HALF_PI, false));
	mKeyboardHandler = mGameCameraController;
	mMouseHandler = mGameCameraController;
	mCamera->ClearTarget();

	EventManager::Get()->TriggerEvent(std::make_shared<QuakeEventDataGameReady>(GetActorId()));
...
bool QuakeHumanView::GetGameContent()
{
	...
	while (System::Get()->OnRun())
	{
		...
		int progress = (int)(30 + 35 + 0.5f);
		DrawLoadScreen(ToWideString(message.str()),
			mGameUI, mVisual, mBlendState, dTime, progress);
	}
...
void QuakeHumanView::AfterContentReceived()
{
	...
	DrawLoadScreen(text, mGameUI, mVisual, mBlendState, 0, 70);

	// Rebuild shaders
	LogInformation("- Rebuilding shaders");
	text = L"Rebuilding shaders...";
	DrawLoadScreen(text, mGameUI, mVisual, mBlendState, 0, 71);

	// Update node aliases
	LogInformation("- Updating node aliases");
	text = L"Initializing nodes...";
	DrawLoadScreen(text, mGameUI, mVisual, mBlendState, 0, 72);
	...
	text = L"Done!";
	DrawLoadScreen(text, mGameUI, mVisual, mBlendState, 0, 100);
	...
	*/
	do
	{
		...
		std::shared_ptr<UIForm>& formUI =
			std::static_pointer_cast<UIForm>(mGameUI->GetFormUI());
		//formUI->QuitForm();
...
bool QuakeHumanView::LoadMedia(const std::string& filePath, bool fromMediaPush)
{
	...
	if (!name.empty())
	{
		...
		if (resHandle)
		{
			std::shared_ptr<SoundResourceExtraData> resData =
			...
			std::string basePath = ToString(FileSystem::Get()->GetWorkingDirectory()) + "/../../Assets/";
			mSoundMgr->LoadSoundFile(ToString(fileName.c_str()), basePath + filePath);
	...
	const char* modelExt[] = { ".bps", ".pk3", ".md3", ".skin", NULL };
...
void QuakeHumanView::SetControlledActor(ActorId actorId)
{
	...
	mGamePlayerController.reset(new QuakePlayerController(mPlayer, 0.f, 0.f));
	mKeyboardHandler = mGamePlayerController;
	mMouseHandler = mGamePlayerController;
}
...
{
	std::shared_ptr<QuakeEventDataShowForm> pCastEventData =
		std::static_pointer_cast<QuakeEventDataShowForm>(pEventData);
	...
	if (!showForm.empty())
	{
		...
		{
			formUI.reset(new UIForm(mGameUI.get(), -1, rectangle, formSrc, textDst, formPr));
...
{
	std::shared_ptr<QuakeEventDataInitChat> pCastEventData =
		std::static_pointer_cast<QuakeEventDataInitChat>(pEventData);
...
{
	std::shared_ptr<QuakeEventDataUpdateChat> pCastEventData =
		std::static_pointer_cast<QuakeEventDataUpdateChat>(pEventData);
...
{
	std::shared_ptr<QuakeEventDataPlaySoundAt> pCastEventData =
		std::static_pointer_cast<QuakeEventDataPlaySoundAt>(pEventData);
...
{
	std::shared_ptr<QuakeEventDataPlaySoundType> pCastEventData =
		std::static_pointer_cast<QuakeEventDataPlaySoundType>(pEventData);
...
{
	std::shared_ptr<QuakeEventDataStopSound> pCastEventData =
		std::static_pointer_cast<QuakeEventDataStopSound>(pEventData);
...
{
	std::shared_ptr<QuakeEventDataFadeSound> pCastEventData =
		std::static_pointer_cast<QuakeEventDataFadeSound>(pEventData);
...
{
	std::shared_ptr<QuakeEventDataChangeVolume> pCastEventData =
		std::static_pointer_cast<QuakeEventDataChangeVolume>(pEventData);
...
{
	std::shared_ptr<QuakeEventDataHandleMedia> pCastEventData =
		std::static_pointer_cast<QuakeEventDataHandleMedia>(pEventData);
...
{
	std::shared_ptr<QuakeEventDataChangeMenu> pCastEventData =
		std::static_pointer_cast<QuakeEventDataChangeMenu>(pEventData);
...
{
    std::shared_ptr<QuakeEventDataGameplayUIUpdate> pCastEventData =
		std::static_pointer_cast<QuakeEventDataGameplayUIUpdate>(pEventData);
...
{
    std::shared_ptr<QuakeEventDataSetControlledActor> pCastEventData =
		std::static_pointer_cast<QuakeEventDataSetControlledActor>(pEventData);
...
{
	std::shared_ptr<QuakeEventDataDeadActor> pCastEventData =
		std::static_pointer_cast<QuakeEventDataDeadActor>(pEventData);
...
void QuakeHumanView::ToggleUpdatePlayerCamera()
{
...
void QuakeHumanView::ShowPauseMenu()
{
	...
	std::shared_ptr<LocalFormHandler> textDst = std::make_shared<LocalFormHandler>(mPlayer->GetId(), "MT_PAUSE_MENU");
