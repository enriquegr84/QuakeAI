...
protected:
	...
	std::shared_ptr<QuakeMainMenuUI> mQuakeMainMenuUI;
...
struct LocalFormHandler : public TextDestination
{
	...
	void GotText(const StringMap& fields)
	{
		if (mFormName == "MT_PAUSE_MENU")
		{
			...
			{
				std::shared_ptr<QuakeEventDataChangeVolume> pEvent(
					new QuakeEventDataChangeVolume());
			...
			{
				std::shared_ptr<QuakeEventDataChangeMenu> pEvent(
					new QuakeEventDataChangeMenu());
				BaseEventManager::Get()->TriggerEvent(pEvent);
				return;
			}
...
private:
		...
		const std::string& base, const std::string& name)
	{
		...
		dstPaths.insert(base + "\\sounds\\" + name + ".0.ogg");
		dstPaths.insert(base + "\\sounds\\" + name + ".1.ogg");
		dstPaths.insert(base + "\\sounds\\" + name + ".2.ogg");
		dstPaths.insert(base + "\\sounds\\" + name + ".3.ogg");
		dstPaths.insert(base + "\\sounds\\" + name + ".4.ogg");
		dstPaths.insert(base + "\\sounds\\" + name + ".5.ogg");
		dstPaths.insert(base + "\\sounds\\" + name + ".6.ogg");
		dstPaths.insert(base + "\\sounds\\" + name + ".7.ogg");
		dstPaths.insert(base + "\\sounds\\" + name + ".8.ogg");
		dstPaths.insert(base + "\\sounds\\" + name + ".9.ogg");
	}
public:
	void FetchSounds(const std::string& name,
		std::set<std::string>& dstPaths, std::set<std::string>& dstDatas)
