...
bool TriangleDemoApp::CreateScene()
{
	...
	std::shared_ptr<ColorEffect> effect = std::make_shared<ColorEffect>(
		ProgramFactory::Get()->CreateFromFiles(path.front(), path.back(), ""));
...
//----------------------------------------------------------------------------
bool TriangleDemoApp::OnEvent(const Event& evt)
{
	bool result = 0;
	switch (evt.mEventType)
	{
		case ET_KEY_INPUT_EVENT:
		{
			if (evt.mKeyInput.mPressedDown)
				OnKeyDown(evt.mKeyInput);
			else
				OnKeyUp(evt.mKeyInput);
		}
		break;
		...
		case ET_MOUSE_INPUT_EVENT:
		{
			switch (evt.mMouseInput.mEvent)
			{
				case MIE_MOUSE_MOVED:
					OnMouseMotion(	evt.mMouseInput.mEvent,
									evt.mMouseInput.mButtonStates,
									evt.mMouseInput.X,
									evt.mMouseInput.Y);
					break;
				...
				case MIE_LMOUSE_LEFT_UP:
					OnMouseClick(	evt.mMouseInput.mEvent,
									evt.mMouseInput.mButtonStates,
									evt.mMouseInput.X,
									evt.mMouseInput.Y);
					break;
...
bool TriangleDemoApp::OnKeyDown(const Event::KeyInput& input)
{
	return true;
}

bool TriangleDemoApp::OnKeyUp(const Event::KeyInput& input)
{
	return true;
}
